...
// name.
func (client StoragePoolsClient) Get(location string, storageSubSystem string, storagePool string) (result StoragePool, err error) {
	if err := validation.Validate([]validation.Validation{}); err != nil {
		return result, validation.NewErrorWithValidationError(err, "fabric.StoragePoolsClient", "Get")
	}

	req, err := client.GetPreparer(location, storageSubSystem, storagePool)
	if err != nil {
...
func (client StoragePoolsClient) GetPreparer(location string, storageSubSystem string, storagePool string) (*http.Request, error) {
...
// parameter.
func (client StoragePoolsClient) List(location string, storageSubSystem string, filter string) (result StoragePoolList, err error) {
	if err := validation.Validate([]validation.Validation{}); err != nil {
		return result, validation.NewErrorWithValidationError(err, "fabric.StoragePoolsClient", "List")
	}

	req, err := client.ListPreparer(location, storageSubSystem, filter)
	if err != nil {
...
func (client StoragePoolsClient) ListPreparer(location string, storageSubSystem string, filter string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"location":         autorest.Encode("path", location),
		"storageSubSystem": autorest.Encode("path", storageSubSystem),
		"subscriptionId":   autorest.Encode("path", client.SubscriptionID),
...
func (client StoragePoolsClient) ListNextResults(lastResults StoragePoolList) (result StoragePoolList, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "fabric.StoragePoolsClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "fabric.StoragePoolsClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "fabric.StoragePoolsClient", "List", resp, "Failure responding to next results request")
	}
...
func (client StoragePoolsClient) ListComplete(location string, storageSubSystem string, filter string, cancel <-chan struct{}) (<-chan StoragePool, <-chan error) {
	resultChan := make(chan StoragePool)
	...
	go func() {
		...
		list, err := client.List(location, storageSubSystem, filter)
		if err != nil {
			errChan <- err
			return
		}
