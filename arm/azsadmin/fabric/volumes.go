...
func (client VolumesClient) Get(location string, storageSubSystem string, storagePool string, volume string) (result Volume, err error) {
	if err := validation.Validate([]validation.Validation{}); err != nil {
		return result, validation.NewErrorWithValidationError(err, "fabric.VolumesClient", "Get")
	}

	req, err := client.GetPreparer(location, storageSubSystem, storagePool, volume)
	if err != nil {
...
func (client VolumesClient) GetPreparer(location string, storageSubSystem string, storagePool string, volume string) (*http.Request, error) {
...
func (client VolumesClient) List(location string, storageSubSystem string, storagePool string, filter string) (result VolumeList, err error) {
	if err := validation.Validate([]validation.Validation{}); err != nil {
		return result, validation.NewErrorWithValidationError(err, "fabric.VolumesClient", "List")
	}

	req, err := client.ListPreparer(location, storageSubSystem, storagePool, filter)
	if err != nil {
...
func (client VolumesClient) ListPreparer(location string, storageSubSystem string, storagePool string, filter string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"location":         autorest.Encode("path", location),
		"storagePool":      autorest.Encode("path", storagePool),
		"storageSubSystem": autorest.Encode("path", storageSubSystem),
...
func (client VolumesClient) ListNextResults(lastResults VolumeList) (result VolumeList, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "fabric.VolumesClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "fabric.VolumesClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "fabric.VolumesClient", "List", resp, "Failure responding to next results request")
	}
...
func (client VolumesClient) ListComplete(location string, storageSubSystem string, storagePool string, filter string, cancel <-chan struct{}) (<-chan Volume, <-chan error) {
	resultChan := make(chan Volume)
	...
	go func() {
		...
		list, err := client.List(location, storageSubSystem, storagePool, filter)
		if err != nil {
			errChan <- err
			return
		}
