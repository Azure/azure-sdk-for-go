...
// OperationsClient is the consumption management client provides access to consumption resources for Azure Web-Direct
// subscriptions. Other subscription types which were not purchased directly through the Azure web portal are not
...
func (client OperationsClient) List() (result OperationListResult, err error) {
	if err := validation.Validate([]validation.Validation{}); err != nil {
		return result, validation.NewErrorWithValidationError(err, "consumption.OperationsClient", "List")
	}
...
func (client OperationsClient) ListNextResults(lastResults OperationListResult) (result OperationListResult, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "consumption.OperationsClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "consumption.OperationsClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "consumption.OperationsClient", "List", resp, "Failure responding to next results request")
	}
