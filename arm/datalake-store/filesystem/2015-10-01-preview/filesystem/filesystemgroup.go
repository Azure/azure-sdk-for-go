...
// appended to using either of these append options, it can only be appended to
// using that append option. ConcurrentAppend DOES NOT guarantee order and can
// result in duplicated data landing in the target file. In order to close a
// file after using ConcurrentAppend, call the Flush method.
//
// accountName is the Azure Data Lake Store account to execute filesystem
...
// Flush flushes the specified file to the store. This forces an update to the
...
//
// accountName is the Azure Data Lake Store account to execute filesystem
// operations on. flushFilePath is the Data Lake Store path (starting with '/')
// of the file to which to flush. op is the constant value for the operation.
// appendParameter is the constant value that avoids redirects. flush is the
// constant value initiating the flush.
