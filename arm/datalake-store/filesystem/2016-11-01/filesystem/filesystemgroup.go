...
// file metadata should get updated. leaseID is optional unique GUID per file
// to ensure single writer semantics, meaning that only clients that append to
// the file with the same leaseId will be allowed to do so. fileSessionID is
...
// do after completion of the create. DATA indicates that more data will be
...
// leaseID is optional unique GUID per file to ensure single writer semantics,
// meaning that only clients that append to the file with the same leaseId will
// be allowed to do so. permission is the octal representation of the unnamed
