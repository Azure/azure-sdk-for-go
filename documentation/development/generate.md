...
* [Generate a SDK](#generate-a-sdk)
    * [Using TypeSpec and tsp-client](#using-typespec-and-tsp-client)
        * [TypeSpec Customizations](#typespec-customizations)
        * [Post-Generation Code Customizations](#post-generation-code-customizations)
...
#### Post-Generation Code Customizations

> **Warning**: **Do not** use on new SDKs. Post-generation code customizations should only be used to maintain backward compatibility on older SDKs. Use TypeSpec customizations instead.

For rare cases where TypeSpec cannot express your customization and you need to maintain backward compatibility:
...
You can also invoke a post-generation action via [`go generate`](https://go.dev/blog/generate). This is specified in an SDK's `tspconfig.yaml` file ([example](https://github.com/Azure/azure-rest-api-specs/blob/fd8fd88708d4a1e2bc3d7f064d295b17fb4fa3f7/specification/keyvault/Security.KeyVault.Certificates/tspconfig.yaml#L56)). This requires a recent version of Go installed on your machine. See the [emitter docs](https://github.com/Azure/autorest.go/blob/main/packages/typespec-go/README.md#go-generate) for more details.
