...
1. **Create custom wrapper types** in separate files (not in `zz_*.go` generated files)
2. **Add convenience methods** by creating functions that call generated code
3. **Custom constructors** can be added in a separate `custom.go` file
...
// This is a convenience method not generated by TypeSpec.
...
**Remember**: If you find yourself needing many code customizations, consider whether the TypeSpec definition should be updated instead.
