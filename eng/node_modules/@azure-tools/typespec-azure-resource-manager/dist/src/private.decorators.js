import { $key, addVisibilityModifiers, clearVisibilityModifiersForClass, getKeyName, getLifecycleVisibilityEnum, getTypeName, isKey, sealVisibilityModifiers, } from "@typespec/compiler";
...
import { getArmResourceKind, getResourceBaseType, isArmVirtualResource, isCustomAzureResource, resolveResourceBaseType, } from "./resource.js";
...
function checkAllowedVirtualResource(program, target, type) {
    if (target.kind === "Model")
        return false;
    if (!isArmVirtualResource(program, type))
        return false;
...
function getPrimaryKeyProperty(program, resource) {
...
export function registerArmResource(context, resourceType) {
    const { program } = context;
