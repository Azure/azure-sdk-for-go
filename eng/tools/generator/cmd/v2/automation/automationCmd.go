...
func (ctx *automationContext) generate(input *pipeline.GenerateInput) (*pipeline.GenerateOutput, error) {
	...
	// typespe
	// tspconfig.yaml 存在 并且有 typespec-go option 时才通过tsp生成
	...
	if len(input.RelatedTypeSpecProjectFolder) == 0 {
		for _, tspProjectFolder := range input.RelatedTypeSpecProjectFolder {
			...
			tc, err := typespec.ParseTypeSpecConfig(tspconfigPath)
			if err != nil {
				return nil, fmt.Errorf("failed to parse tspconfig.yaml: %+v", err)
			}

			if _, ok := tc.TypeSpecProjectSchema.Options[string(typespec.TypeSpec_GO)]; ok {
				...
				generateCtx := common.GenerateContext{
					...
					// TypeSpecProjectFolder: tspProjectFolder,
					TypeSpecConfigPath: tspconfigPath,
				}

				namespaceResults, errors := generateCtx.GenerateForAutomationTypeSpec("readme", input.RepoHTTPSURL, ctx.goVersion)
				if len(errors) != 0 {
