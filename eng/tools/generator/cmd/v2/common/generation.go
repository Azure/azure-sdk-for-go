...
func (t *TypeSpecUpdateGenerator) PreGenerate(generateParam *GenerateParam) error {
	...
	// update build.go if package previously use autorest
	autorestMdPath := filepath.Join(t.PackagePath, "autorest.md")
	if _, err := os.Stat(autorestMdPath); os.IsNotExist(err) {
		// if autorest.md not exist, it means the package is generated by typespec, so we don't need to update build.go
		return nil
	}
	log.Printf("Update build.go for typespec package...")
	if err := GenerateBuildGoFile(t.SDKPath, t.PackagePath, generateParam.RPName, generateParam.NamespaceName); err != nil {
		return err
	}
	return nil
}
