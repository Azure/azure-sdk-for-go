...
type AudioOutputParameters struct {
	// REQUIRED; Specifies the output audio format.
	Format *OutputAudioFormat

	// REQUIRED; Specifies the voice type.
	Voice *SpeechVoice
}

// AudioResponseData - Object containing audio response data and its metadata.
type AudioResponseData struct {
	// REQUIRED; Base64 encoded audio bytes generated by the model, in the format specified in the request.
	Data *string

	// REQUIRED; The Unix timestamp (in seconds) for when this audio response will no longer be accessible on the server for use
	// in multi-turn conversations.
	ExpiresAt *time.Time

	// REQUIRED; Unique identifier for this audio response.
	ID *string

	// REQUIRED; Transcript of the audio generated by the model.
	Transcript *string
}
...
type ChatMessageAudioContentItem struct {
	// REQUIRED; The audio data.
...
// GetChatMessageContentItem implements the ChatMessageContentItemClassification interface for type ChatMessageAudioContentItem.
func (c *ChatMessageAudioContentItem) GetChatMessageContentItem() *ChatMessageContentItem {
...
// ChatRequestDeveloperMessage - Developer-provided instructions that the model should follow, regardless of messages sent
...
type ChatRequestDeveloperMessage struct {
	...
	// An optional name for the participant. Provides the model information to differentiate between participants of the same
	// role.
	Name *string
}

// GetChatRequestMessage implements the ChatRequestMessageClassification interface for type ChatRequestDeveloperMessage.
func (c *ChatRequestDeveloperMessage) GetChatRequestMessage() *ChatRequestMessage {
	return &ChatRequestMessage{
		role: c.role,
	}
}
...
type CompletionsUsageCompletionTokensDetails struct {
	...
	// When using Predicted Outputs, the number of tokens in the prediction that did not appear in the completion. However, like
...
type InputAudioContent struct {
	...
	// REQUIRED; The format of the audio data.
...
// UserSecurityContext - User security context contains several parameters that describe the AI application itself, and the
// end user that interacts with the AI application. These fields assist your security operations teams
// to investigate and mitigate security incidents by providing a comprehensive approach to protecting your AI applications.
// (Learn more at https://aka.ms/TP4AI/Documentation/EndUserContext) about
...
type UserSecurityContext struct {
	...
	// This identifier is the Microsoft Entra ID (formerly Azure Active Directory) user object ID used to authenticate end-users
	// within the generative AI application. Sensitive personal information should
