...
type ConnectionOptions struct {
	...
	// AuxiliaryTenants contains the additional tenant IDs to be used in a
...
func NewConnection(endpoint string, cred azcore.TokenCredential, options *ConnectionOptions) *Connection {
	...
	if len(options.AuxiliaryTenants) > 0 {
		policies = append(policies, NewMultiTenantAuthenticationPolicy(cred, ARMAuthenticationPolicyOptions{AuxiliaryTenants: options.AuxiliaryTenants, Options: azcore.TokenRequestOptions{Scopes: []string{endpointToScope(endpoint)}}}))
	}
