...
func TestRPRegistrationPolicySuccess(t *testing.T) {
	...
	pl := pipeline.NewPipeline(srv, NewRPRegistrationPolicy(srv.URL(), mockTokenCred{}, testRPRegistrationOptions(srv)))
...
func TestRPRegistrationPolicyCanCancel(t *testing.T) {
	...
	opts.Transport = srv
	pl := pipeline.NewPipeline(srv, NewRPRegistrationPolicy(srv.URL(), mockTokenCred{}, &opts))
	// log only RP registration
	log.SetClassifications(LogRPRegistration)
	defer func() {
		// reset logging
...
func TestRPRegistrationPolicyDisabled(t *testing.T) {
	...
	ops.MaxAttempts = -1
	pl := pipeline.NewPipeline(srv, NewRPRegistrationPolicy(srv.URL(), mockTokenCred{}, ops))
	req, err := runtime.NewRequest(context.Background(), http.MethodGet, runtime.JoinPaths(srv.URL(), requestEndpoint))
	if err != nil {
		t.Fatal(err)
	}
