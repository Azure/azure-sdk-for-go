...
func (r *Responder[T]) MarshalResponseAsJSON(req *http.Request) (*http.Response, error) {
	resp, err := marshalResponseAsJSON(req, r.resp)
	if err != nil {
...
// This method is typically called by the fake server internals.
func (e *ErrorResponder) Get(req *http.Request) error {
	if e.err == nil {
		return nil
	}
...
func (p *PagerResponder[T]) Next(req *http.Request) (*http.Response, error) {
	...
	pageT, ok := page.(T)
	if ok {
		return marshalResponseAsJSON(req, pageT)
	}
...
// marshalResponseAsJSON creates an *http.Response with v as the response body in JSON format.
// The provided *http.Request is associated with the response.
