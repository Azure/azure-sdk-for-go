...
func TestResponder(t *testing.T) {
	...
	req := &http.Request{}
	resp, err := respr.MarshalResponseAsJSON(req)
	require.NoError(t, err)
...
func TestErrorResponder(t *testing.T) {
	...
	require.NoError(t, errResp.Get(req))

	myErr := errors.New("failed")
	errResp.SetError(myErr)
	require.ErrorIs(t, errResp.Get(req), myErr)

	errResp.SetResponseError("ErrorInvalidWidget", http.StatusBadRequest)
	var respErr *azcore.ResponseError
	require.ErrorAs(t, errResp.Get(req), &respErr)
