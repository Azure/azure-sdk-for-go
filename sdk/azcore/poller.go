...
func (l *LROPoller) PollUntilDone(ctx context.Context, freq time.Duration, respType interface{}) (*http.Response, error) {
	logPollUntilDoneExit := func(v interface{}) {
		logger.Log().Writef(LogLongRunningOperation, "END PollUntilDone() for %T: %v", l.lro, v)
	}
	logger.Log().Writef(LogLongRunningOperation, "BEGIN PollUntilDone() for %T", l.lro)
	if l.resp != nil {
		...
		if retryAfter := RetryAfter(l.resp.Response); retryAfter > 0 {
			logger.Log().Writef(LogLongRunningOperation, "initial Retry-After delay for %s", retryAfter.String())
	...
	for {
		...
		if retryAfter := RetryAfter(resp); retryAfter > 0 {
			logger.Log().Writef(LogLongRunningOperation, "Retry-After delay for %s", retryAfter.String())
			d = retryAfter
		} else {
			logger.Log().Writef(LogLongRunningOperation, "delay for %s", d.String())
		}
