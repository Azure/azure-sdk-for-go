...
func TestPolicyTelemetryDefault(t *testing.T) {
	...
	defer close()
	srv.SetResponse()
	pl := NewPipeline(srv, NewTelemetryPolicy("test", "v1.2.3", nil))
...
func TestPolicyTelemetryWithAppID(t *testing.T) {
	...
	pl := NewPipeline(srv, NewTelemetryPolicy("test", "v1.2.3", &policy.TelemetryOptions{ApplicationID: appID}))
	req, err := NewRequest(context.Background(), http.MethodGet, srv.URL())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPolicyTelemetryDisabled(t *testing.T) {
	...
	srv.SetResponse()
	const appID = "my_application"
	pl := NewPipeline(srv, NewTelemetryPolicy("test", "v1.2.3", &policy.TelemetryOptions{ApplicationID: appID, Disabled: true}))
	req, err := NewRequest(context.Background(), http.MethodGet, srv.URL())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
