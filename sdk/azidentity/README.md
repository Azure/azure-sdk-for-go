...
This example demonstrates authenticating the `ResourcesClient` from the [armresources_library][armresources_library] library using `DefaultAzureCredential`. 
...
This example demonstrates authenticating the `ResourcesClient` from the [armresources][armresources_library] library using the `DefaultAzureCredential`, deployed to an Azure resource with a user assigned managed identity configured.
...
This examples demonstrates authenticating the `ResourcesClient` from the [armresources][armresources_library] library using `ManagedIdentityCredential` in a virtual machine, app service, function app, cloud shell, or AKS environment on Azure, with system assigned, or user assigned managed identity enabled.
