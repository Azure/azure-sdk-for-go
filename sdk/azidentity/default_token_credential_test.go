...
func TestDefaultTokenCredential_NilOptions(t *testing.T) {
	...
	// if the test is running in a MSI environment then the length of sources would be three since it will include environment credential, managed identity credential and cli credential
	if msiType, err := c.getMSIType(context.Background()); msiType == msiTypeIMDS || msiType == msiTypeCloudShell || msiType == msiTypeAppService {
		if len(cred.sources) != 3 {
			t.Fatalf("Length of ChainedTokenCredential sources for DefaultTokenCredential. Expected: 3, Received: %d", len(cred.sources))
		}
		//if a credential unavailable error is received or msiType is unknown then environment credential and cli credential will be added
