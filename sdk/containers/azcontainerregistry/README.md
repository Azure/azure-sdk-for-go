...
func main() {
	...
	calculatedDigest, err := azcontainerregistry.CalculateDigest(streaming.NopCloser(bytes.NewReader(blob)))
	if err != nil {
		log.Fatalf("failed to calculate blob digest: %v", err)
	}
	...
	downloadBlob, err := io.ReadAll(downloadRes.Body)
	if err != nil {
...
func main() {
	...
	uploadResp, err := blobClient.UploadChunk(ctx, *startRes.Location, streaming.NopCloser(bytes.NewReader(layer)), nil)
	if err != nil {
		log.Fatalf("failed to upload layer: %v", err)
	}
	calculatedDigest, err := azcontainerregistry.CalculateDigest(streaming.NopCloser(bytes.NewReader(layer)))
	if err != nil {
		log.Fatalf("failed to calculate layer digest: %v", err)
	}
	completeResp, err := blobClient.CompleteUpload(ctx, calculatedDigest, *uploadResp.Location, nil)
	if err != nil {
  ...
  rootfs: {
	...
	uploadResp, err = blobClient.UploadChunk(ctx, *startRes.Location, streaming.NopCloser(bytes.NewReader(config)), nil)
	if err != nil {
		log.Fatalf("failed to upload config: %v", err)
	}
	calculatedDigest, err = azcontainerregistry.CalculateDigest(streaming.NopCloser(bytes.NewReader(layer)))
	if err != nil {
		log.Fatalf("failed to calculate config digest: %v", err)
	}
	completeResp, err = blobClient.CompleteUpload(ctx, calculatedDigest, *uploadResp.Location, nil)
	if err != nil {
