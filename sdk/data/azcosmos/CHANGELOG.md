...
* Fixed `GetChangeFeed` to survive partition splits. The change feed now resolves customer-supplied `FeedRange`s by overlap-match against the current routing map (instead of exact-equality), retries on `410/Gone` with a PK-range substatus by refreshing the cache, expands a single `FeedRange` that straddles a split into one continuation queue entry per child (each inheriting the parent's ETag), and persists a multi-range composite continuation token so subsequent calls drain every child range. Also added a cross-container `ResourceID` guard that rejects continuation tokens issued for a different container. See [issue 26767](https://github.com/Azure/azure-sdk-for-go/issues/26767).
