...
func TestGlobalEndpointManagerGetWriteEndpoints(t *testing.T) {
	...
	gem, err := newGlobalEndpointManager(srv.URL(), pl, []string{"West US", "Central US"}, 5*time.Minute, true, true /*enableEndpointDiscovery*/)
...
func TestGlobalEndpointManagerGetEndpointLocation(t *testing.T) {
	...
	gem, err := newGlobalEndpointManager(srv.URL(), pl, []string{}, 5*time.Minute, true, true /*enableEndpointDiscovery*/)
	assert.NoError(t, err)
...
func TestGlobalEndpointManagerCanUseMultipleWriteLocations(t *testing.T) {
	...
	mockLc := newLocationCache(preferredRegions, *serverEndpoint, true, true /*enableEndpointDiscovery*/)
	...
	gem, err := newGlobalEndpointManager(srv.URL(), internalClient.Pipeline(), []string{}, 5*time.Minute, true, true /*enableEndpointDiscovery*/)
	assert.NoError(t, err)
...
func TestGlobalEndpointManagerConcurrentUpdate(t *testing.T) {
	...
	gem, err := newGlobalEndpointManager(srv.URL(), pl, []string{}, 5*time.Second, true, true /*enableEndpointDiscovery*/)
	assert.NoError(t, err)
