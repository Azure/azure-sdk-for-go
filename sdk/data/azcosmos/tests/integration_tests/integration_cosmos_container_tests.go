...
func TestContainerCRUD(t *testing.T) {
	for _, integrationTests.accountType := range accountTypes {

		integrationTests := newIntegrationTests(t, accountType.multipleWriteRegions)
		client := integrationTests.getClient(t, nil)

			database := integrationTests.createDatabase(t, context.TODO(), client, "containerCRUD")
		defer integrationTests.deleteDatabase(t, context.TODO(), database)
	...
	for _, integrationTests.accountType := range accountTypes {
		...
		properties := ContainerProperties{
			...
			IndexingPolicy: &IndexingPolicy{
				...
				ExcludedPaths: []ExcludedPath{
					{Path: "/\"_etag\"/?"}, // Fixed the path formatting
				},
				Automatic:    true,
				IndexingMode: IndexingModeConsistent,
			},
...
func TestContainerVectorSearch(t *testing.T) {
	for _, integrationTests.accountType := range accountTypes {
		...
		database := integrationTests.createDatabase(t, context.TODO(), client, "vectorSearch")
		defer integrationTests.deleteDatabase(t, context.TODO(), database)
	...
	func TestContainerFullTextSearch(t *testing.T) {
		...
		client := integrationTests.getClient(t, nil)
			...
			database := integrationTests.createDatabase(t, context.TODO(), client, "fullTextSearch")
			defer integrationTests.deleteDatabase(t, context.TODO(), database)
...
func TestIntegrationContainerPartitionKeyRangesAndFeedRanges(t *testing.T) {
	for _, integrationTests.accountType := range accountTypes {

		integrationTests := newIntegrationTests(t, accountType.multipleWriteRegions)
		client := integrationTests.getClient(t, nil)

		database := integrationTests.createDatabase(t, context.TODO(), client, "containerRangesTest")
		defer integrationTests.deleteDatabase(t, context.TODO(), database)
		properties := ContainerProperties{
			ID: "aContainer",
			PartitionKeyDefinition: PartitionKeyDefinition{
...
func TestIntegrationContainerChangeFeed(t *testing.T) {
	for _, integrationTests.accountType := range accountTypes {

		integrationTests := newIntegrationTests(t, accountType.multipleWriteRegions)
		client := integrationTests.getClient(t, nil)

		database := integrationTests.createDatabase(t, context.TODO(), client, "changeFeedTest")
		defer integrationTests.deleteDatabase(t, context.TODO(), database)
