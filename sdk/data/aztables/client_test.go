...
func TestContinuationTokensDifferentFilters(t *testing.T) {
	for _, service := range services {
		t.Run(fmt.Sprintf("%v_%v", t.Name(), service), func(t *testing.T) {
			client, delete := initClientTest(t, service, true)
			defer delete()
			...
			emptyPager := client.List(&ListEntitiesOptions{Top: to.Int32Ptr(1)})
			for emptyPager.NextPage(ctx) {
				for _, e := range emptyPager.PageResponse().Entities {
					var m map[string]interface{}
					err = json.Unmarshal(e, &m)
					require.NoError(t, err)
					fmt.Println(m)
					if emptyPager.NextPagePartitionKey() != nil && emptyPager.NextPageRowKey() != nil {
						fmt.Println(*emptyPager.NextPagePartitionKey(), *emptyPager.NextPageRowKey())
					}
				}
			}
			...
			newPager := client.List(&ListEntitiesOptions{
				PartitionKey: pkContToken,
				RowKey:       rkContToken,
				Filter:       to.StringPtr("Value ge 5"),
			})
			count := 0
			for newPager.NextPage(ctx) {
				count += len(newPager.PageResponse().Entities)
				for _, e := range newPager.PageResponse().Entities {
					var m map[string]interface{}
					err = json.Unmarshal(e, &m)
					require.NoError(t, err)
					fmt.Println(m)
