...
func ExampleClient_Encrypt() {
	...
	encryptResponse, err := client.Encrypt(context.TODO(), azcrypto.AlgorithmRSAOAEP, []byte("plaintext"), nil)
	if err != nil {
		panic(err)
	}
...
func ExampleClient_Decrypt() {
	...
	decryptResponse, err := client.Decrypt(context.TODO(), azcrypto.AlgorithmRSAOAEP, encryptResponse.Result, nil)
	if err != nil {
		panic(err)
	}
...
func ExampleClient_WrapKey() {
	...
	// Wrap
	wrapResp, err := client.WrapKey(context.TODO(), azcrypto.RSAOAEP, keyBytes, nil)
	if err != nil {
		panic(err)
	}
...
func ExampleClient_UnwrapKey() {
	...
	// Unwrap
	unwrapResp, err := client.UnwrapKey(context.TODO(), azcrypto.RSAOAEP, wrapResp.Result, nil)
	if err != nil {
		panic(err)
	}
...
func ExampleClient_Verify() {
	...
	verifyResponse, err := client.Verify(context.TODO(), azcrypto.RS256, digest, signResponse.Result, nil)
	if err != nil {
		panic(err)
	}
