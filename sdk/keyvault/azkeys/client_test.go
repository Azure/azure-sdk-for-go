...
func TestBackupKey(t *testing.T) {
	...
	backupResp, err := client.BackupKey(context.Background(), key, nil)
	require.NoError(t, err)
	require.Greater(t, len(backupResp.Value), 0)

	respPoller, err := client.BeginDeleteKey(context.Background(), key, nil)
	require.NoError(t, err)
	...
	_, err = client.GetKey(context.Background(), key, nil)
	...
	var i int
	for i = 0; i < 10; i++ {
		restoreResp, err = client.RestoreKeyBackup(context.Background(), backupResp.Value, nil)
		if err == nil {
			break
		}
		time.Sleep(delay())
...
func TestRotateKey(t *testing.T) {
	...
	key, err := createRandomName(t, "key")
	require.NoError(t, err)
	createResp, err := client.CreateRSAKey(context.Background(), key, nil)
	require.NoError(t, err)
	defer cleanUpKey(t, client, key)
