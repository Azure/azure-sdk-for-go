...
func TestClient_EncryptDecrypt(t *testing.T) {
	...
	encryptResponse, err := cryptoClient.Encrypt(ctx, EncryptionAlgorithms.RSAOAEP(), []byte("plaintext"), nil)
	require.NoError(t, err)
	require.NotNil(t, encryptResponse)

	decryptResponse, err := cryptoClient.Decrypt(ctx, EncryptionAlgorithms.RSAOAEP(), encryptResponse.Result, nil)
	require.NoError(t, err)
...
func TestClient_WrapUnwrap(t *testing.T) {
	...
	// Wrap
	wrapResp, err := cryptoClient.WrapKey(ctx, KeyWrapAlgorithms.RSAOAEP(), keyBytes, nil)
	require.NoError(t, err)

	// Unwrap
	unwrapResp, err := cryptoClient.UnwrapKey(ctx, KeyWrapAlgorithms.RSAOAEP(), wrapResp.Result, nil)
	require.NoError(t, err)
...
func TestClient_SignVerify(t *testing.T) {
	...
	signResponse, err := cryptoClient.Sign(ctx, SignatureAlgorithms.RS256(), digest, nil)
	require.NoError(t, err)

	verifyResponse, err := cryptoClient.Verify(ctx, SignatureAlgorithms.RS256(), digest, signResponse.Result, nil)
	require.NoError(t, err)
