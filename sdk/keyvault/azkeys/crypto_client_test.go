...
func TestClient_Decrypt(t *testing.T) {
	...
	encryptResponse, err := cryptoClient.Encrypt(ctx, azcrypto.AlgorithmRSAOAEP, []byte("plaintext"), nil)
	require.NoError(t, err)
	require.NotNil(t, encryptResponse)

	decryptResponse, err := cryptoClient.Decrypt(ctx, azcrypto.AlgorithmRSAOAEP, encryptResponse.Result, nil)
	require.NoError(t, err)
...
func TestClient_WrapUnwrap(t *testing.T) {
	...
	// Wrap
	wrapResp, err := cryptoClient.WrapKey(ctx, azcrypto.RSAOAEP, keyBytes, nil)
	require.NoError(t, err)

	// Unwrap
	unwrapResp, err := cryptoClient.UnwrapKey(ctx, azcrypto.RSAOAEP, wrapResp.Result, nil)
	require.NoError(t, err)
...
func TestClient_SignVerify(t *testing.T) {
	...
	signResponse, err := cryptoClient.Sign(ctx, azcrypto.RS256, digest, nil)
	require.NoError(t, err)

	verifyResponse, err := cryptoClient.Verify(ctx, azcrypto.RS256, digest, signResponse.Result, nil)
	require.NoError(t, err)
