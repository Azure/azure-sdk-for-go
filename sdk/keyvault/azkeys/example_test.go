...
func ExampleClient_CreateECKey() {
	...
	resp, err := client.CreateECKey(context.TODO(), "new-rsa-key", &azkeys.CreateECKeyOptions{CurveName: azkeys.JSONWebKeyCurveNameValues.P256().ToPtr()})
	if err != nil {
		panic(err)
	}
	fmt.Println(*resp.Key.ID)
...
func ExampleClient_UpdateKeyProperties() {
	...
	resp, err := client.UpdateKeyProperties(context.TODO(), "key-to-update", &azkeys.UpdateKeyPropertiesOptions{
		...
		KeyAttributes: &azkeys.KeyAttributes{
			RecoveryLevel: azkeys.DeletionRecoveryLevelValues.CustomizedRecoverablePurgeable().ToPtr(),
		},
	})
	if err != nil {
		panic(err)
