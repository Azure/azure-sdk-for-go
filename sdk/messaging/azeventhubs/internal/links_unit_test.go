...
func TestLinks_LinkStale(t *testing.T) {
	...
	require.NoError(t, err)
	require.Nil(t, links.links["0"], "closed link is removed from the cache")
	require.Equal(t, 1, receivers[0].CloseCalled, "RecoverIfNeeded closes the passed in staleLWID")
...
func TestLinks_ConnectionRecovery(t *testing.T) {
	...
	ns.EXPECT().NewAMQPSession(test.NotCancelled).Return(session, nil)

	receiver.EXPECT().LinkName().Return("link1").AnyTimes()
	receiver.EXPECT().ConnID().Return(uint64(1)).AnyTimes()
...
func TestLinks_LinkRecoveryButCloseIsCancelled(t *testing.T) {
	...
	require.Equal(t, []string{
		...
		"[azeh.Conn] (c:0,l:Link1,p:0): Succesfully created link for partition ID '0'",
		"[azeh.Conn] (c:0,l:Link1,p:0) Error when cleaning up old link for link recovery: context canceled"}, logs)
}

func TestLinks_Recovery_LWID_Is_Nil(t *testing.T) {
	ctrl := gomock.NewController(t)
	ns := mock.NewMockNamespaceForAMQPLinks(ctrl)
	receiver := mock.NewMockAMQPReceiverCloser(ctrl)
	...
	ns.EXPECT().Recover(test.NotCancelled, uint64(101)).Return(nil)

	amqpwrapErr := amqpwrap.NewError(&amqp.ConnError{}, 101, "backup-link-id")
	require.Equal(t, RecoveryKindConn, GetRecoveryKind(amqpwrapErr))

	// when we recover and we don't have an LWID (usually when we've failed to create _any_ links)
	// the the connection ID and needed info is carried into the 'error' itself.
	err := links.RecoverIfNeeded(context.Background(), "0", nil, amqpwrapErr)
	require.NoError(t, err)
}
...
func TestLinks_linkRecoveryOnly(t *testing.T) {
	...
	fakeNS.EXPECT().NewAMQPSession(test.NotCancelled).Return(session, nil)

	fakeReceiver.EXPECT().LinkName().Return("link1").AnyTimes()
	fakeReceiver.EXPECT().ConnID().Return(uint64(1))
...
func TestLinks_linkRecoveryFailsWithLinkFailure(t *testing.T) {
	...
	fakeReceiver.EXPECT().LinkName().Return("link1").AnyTimes()
	fakeReceiver.EXPECT().ConnID().Return(uint64(1)).AnyTimes()
