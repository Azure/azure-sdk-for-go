...
func NegotiateClaim(ctx context.Context, audience string, conn *amqp.Client, provider auth.TokenProvider) error {
	...
	defer func() {
		if err := link.Close(ctx); err != nil {
			azlog.Writef(exports.EventAuth, "Failed closing claim link: %s", err.Error())
		}
	}()
	...
	azlog.Writef(exports.EventAuth, "negotiating claim for audience %s with token type %s and expiry of %s", audience, token.TokenType, token.Expiry)
