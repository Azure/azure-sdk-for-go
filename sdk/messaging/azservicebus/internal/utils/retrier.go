...
func Retry(ctx context.Context, tracer tracing.Tracer, eventName log.Event, operation string, fn func(ctx context.Context, args *RetryFnArgs) error, isFatalFn func(err error) bool, o exported.RetryOptions, so *tracing.SpanOptions) error {
	if isFatalFn == nil {
	...
	var err error
	ctx, endSpan := tracing.StartSpan(ctx, tracer, so)
