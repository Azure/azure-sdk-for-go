...
directive:
 ...
 # rename log operations to generate into a separate logs client
  ...
  - from: models.go
    where: $
    transform: return $.replace(/(?:\/\/.*\s)+type (?:ErrorResponse|ErrorResponseAutoGenerated|ErrorInfo|ErrorDetail|ErrorContract|ErrorAdditionalInfo).+\{(?:\s.+\s)+\}\s/g, "");
  - from: models_serde.go
    where: $
    transform: return $.replace(/(?:\/\/.*\s)+func \(\w \*?(?:ErrorResponse|ErrorResponseAutoGenerated|ErrorInfo|ErrorDetail|ErrorContract|ErrorAdditionalInfo|ErrorResponseAutoGenerated2)\).*\{\s(?:.+\s)+\}\s/g, "");
