...
func (a *APIGatewayConfigConnectionServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.ApiManagement/gateways/(?P<gatewayName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/configConnections/(?P<configConnectionName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (a *APIGatewayConfigConnectionServerTransport) dispatchNewListByGatewayPager(req *http.Request) (*http.Response, error) {
	...
	if newListByGatewayPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.ApiManagement/gateways/(?P<gatewayName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/configConnections`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		var options *armapimanagement.APIGatewayConfigConnectionClientListByGatewayOptions
		if topParam != nil || skipTokenParam != nil {
			options = &armapimanagement.APIGatewayConfigConnectionClientListByGatewayOptions{
				Top:       topParam,
				SkipToken: skipTokenParam,
			}
		}
		resp := a.srv.NewListByGatewayPager(resourceGroupNameParam, gatewayNameParam, options)
