...
func (g *GatewaySKUsServerTransport) dispatchNewListAvailableSKUsPager(req *http.Request) (*http.Response, error) {
	...
	if newListAvailableSKUsPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.ApiManagement/gateways/(?P<gatewayName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/skus`
