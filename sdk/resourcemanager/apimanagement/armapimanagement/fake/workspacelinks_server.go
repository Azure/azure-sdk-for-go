...
func (w *WorkspaceLinksServerTransport) dispatchNewListByServicePager(req *http.Request) (*http.Response, error) {
	...
	if newListByServicePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.ApiManagement/service/(?P<serviceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/workspaceLinks`
		...
		var options *armapimanagement.WorkspaceLinksClientListByServiceOptions
		if topParam != nil || skipTokenParam != nil {
			options = &armapimanagement.WorkspaceLinksClientListByServiceOptions{
				Top:       topParam,
