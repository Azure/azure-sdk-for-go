...
type APIContractProperties struct {
	...
	// It is appended to the API endpoint base URL specified during the service instance creation to form a public URL for this
	// API.
	Path *string
...
type APIContractUpdateProperties struct {
	...
	// is appended to the API endpoint base URL specified during the service instance creation to form a public URL for this API.
	Path *string
...
type AssociationContractProperties struct {
	// FLAG; CONSTANT; Provisioning state.
...
// AuthorizationProviderFederatedIdentityCredentialsProperties - Federated identity credentials properties returned by the
// authorization provider.
type AuthorizationProviderFederatedIdentityCredentialsProperties struct {
	// READ-ONLY; The audience of the federated identity credentials.
	Audience *string

	// READ-ONLY; The issuer URL of the federated identity credentials.
	Issuer *string

	// READ-ONLY; The subject identifier of the federated identity credentials.
	Subject *string
}
...
// AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType - OAuth2 authorization code with federated identity credentials
// grant type parameters.
type AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType struct {
	...
	// READ-ONLY; The type of client assertion used for federated identity credentials.
...
type AuthorizationProviderOAuth2GrantTypes struct {
	...
	AuthorizationCodeWithFederatedIdentityCredentials *AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType
...
type AuthorizationProviderOAuth2Settings struct {
	...
	// READ-ONLY; Federated identity credentials properties.
...
type BackendPoolItem struct {
	...
	// to regions that have carbon emission less than or equal to the specified category. However, when all other backends are
	// not available it will route traffic to these regions anyway. This requires the backend to be attributed with 'azureRegion'
	// information.
...
type ClientApplicationContractProperties struct {
	...
	// it is requesting access tokens from Microsoft EntraID. This property is read-only and will be set by the system when the
...
type ConfigurationAPI struct {
	...
	// but must be 'Enabled' or 'Disabled'. If 'Disabled', legacy Configuration API (v1) will not be available for self-hosted
...
type ConnectivityStatusContract struct {
	...
	// inside the Virtual Network.The date conforms to the following format: `yyyy-MM-ddTHH:mm:ssZ` as specified by the ISO 8601
	// standard.
...
type GatewayHostnameBindingKeyVaultLastStatus struct {
	...
	// at least once and current state is failed. The date conforms to the following format: `yyyy-MM-ddTHH:mm:ssZ` as specified
	// by the ISO 8601 standard.
...
type IdentityProviderCreateContractProperties struct {
	...
	// it is App Secret for Facebook login, API Key for Google login, Public Key for Microsoft. This property will not be filled
...
// McpEndpoint - Endpoint definition for MCP API type.
type McpEndpoint struct {
	// REQUIRED; MCP endpoint name, e.g. 'sse' or 'messages'.
	Name *string
...
// McpProperties - Properties specific to MCP API type.
...
type OperationResultContractProperties struct {
	...
	// READ-ONLY; This property if only provided as part of the TenantConfiguration_Validate operation. It contains the log the
...
type ProductContractProperties struct {
	...
	// type. The subscription-key authentication type is used by default. If the property is omitted, the subscription-key authentication
	// type is used.
...
type QuotaCounterContract struct {
	...
	// REQUIRED; The date of the start of Counter Period. The date conforms to the following format: `yyyy-MM-ddTHH:mm:ssZ` as
...
type RequestReportRecordContract struct {
	...
	// value is hit, cached response was returned. If the value is miss, the cache was used but lookup resulted in a miss and
...
type SchemaContractProperties struct {
	...
	// document (e.g. application/json, application/xml). </br> - `Swagger` Schema use `application/vnd.ms-azure-apim.swagger.definitions+json`
...
type ServiceNameAvailabilityResult struct {
	...
	// naming requirements so that the user can select a valid name. If reason == AlreadyExists, explain that <resourceName> is
...
type ServiceProperties struct {
	...
	// be 'Enabled' or 'Disabled'. If 'Disabled', private endpoints are the exclusive access method. Default value is 'Enabled'
	...
	// with Zone redundant or fail the request if any underneath component cannot be zone redundant.
	ZoneRedundant *bool
	...
	// in an Internal Virtual Network. Available only for Basic, Standard, Premium and Isolated SKU.
	PrivateIPAddresses []*string
...
type SubscriptionContractProperties struct {
	...
	// is blocked, and the subscriber cannot call any APIs of the product, * submitted – the subscription request has been made
	// by the developer, but has not yet been approved or rejected, * rejected – the subscription request has been denied by an
	// administrator, * cancelled – the subscription has been cancelled by the developer or administrator, * expired – the subscription
