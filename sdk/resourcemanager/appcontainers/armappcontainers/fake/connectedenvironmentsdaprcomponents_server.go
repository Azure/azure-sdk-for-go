...
type ConnectedEnvironmentsDaprComponentsServer struct {
	...
	// NewListSecretsPager is the fake for method ConnectedEnvironmentsDaprComponentsClient.NewListSecretsPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListSecretsPager func(resourceGroupName string, connectedEnvironmentName string, componentName string, options *armappcontainers.ConnectedEnvironmentsDaprComponentsClientListSecretsOptions) (resp azfake.PagerResponder[armappcontainers.ConnectedEnvironmentsDaprComponentsClientListSecretsResponse])
}
...
func NewConnectedEnvironmentsDaprComponentsServerTransport(srv *ConnectedEnvironmentsDaprComponentsServer) *ConnectedEnvironmentsDaprComponentsServerTransport {
	return &ConnectedEnvironmentsDaprComponentsServerTransport{
		...
		newListSecretsPager: newTracker[azfake.PagerResponder[armappcontainers.ConnectedEnvironmentsDaprComponentsClientListSecretsResponse]](),
	}
}
...
type ConnectedEnvironmentsDaprComponentsServerTransport struct {
	...
	newListSecretsPager *tracker[azfake.PagerResponder[armappcontainers.ConnectedEnvironmentsDaprComponentsClientListSecretsResponse]]
}
...
func (c *ConnectedEnvironmentsDaprComponentsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "ConnectedEnvironmentsDaprComponentsClient.NewListSecretsPager":
				res.resp, res.err = c.dispatchNewListSecretsPager(req)
			default:
...
func (c *ConnectedEnvironmentsDaprComponentsServerTransport) dispatchNewListSecretsPager(req *http.Request) (*http.Response, error) {
	if c.srv.NewListSecretsPager == nil {
		return nil, &nonRetriableError{errors.New("fake for method NewListSecretsPager not implemented")}
	}
	newListSecretsPager := c.newListSecretsPager.get(req)
	if newListSecretsPager == nil {
		...
		resp := c.srv.NewListSecretsPager(resourceGroupNameParam, connectedEnvironmentNameParam, componentNameParam, nil)
		newListSecretsPager = &resp
		c.newListSecretsPager.add(req, newListSecretsPager)
	}
	...
	if !contains([]int{http.StatusOK}, resp.StatusCode) {
		c.newListSecretsPager.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusOK", resp.StatusCode)}
	}
