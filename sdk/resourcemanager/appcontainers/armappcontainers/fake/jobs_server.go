...
type JobsServer struct {
	...
	// JobExecution is the fake for method JobsClient.JobExecution
	// HTTP status codes to indicate success: http.StatusOK
	JobExecution func(ctx context.Context, resourceGroupName string, jobName string, jobExecutionName string, options *armappcontainers.JobsClientJobExecutionOptions) (resp azfake.Responder[armappcontainers.JobsClientJobExecutionResponse], errResp azfake.ErrorResponder)
	...
	// NewListSecretsPager is the fake for method JobsClient.NewListSecretsPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListSecretsPager func(resourceGroupName string, jobName string, options *armappcontainers.JobsClientListSecretsOptions) (resp azfake.PagerResponder[armappcontainers.JobsClientListSecretsResponse])
...
func NewJobsServerTransport(srv *JobsServer) *JobsServerTransport {
	return &JobsServerTransport{
		...
		newListSecretsPager:         newTracker[azfake.PagerResponder[armappcontainers.JobsClientListSecretsResponse]](),
...
type JobsServerTransport struct {
	...
	newListSecretsPager         *tracker[azfake.PagerResponder[armappcontainers.JobsClientListSecretsResponse]]
...
func (j *JobsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "JobsClient.JobExecution":
				res.resp, res.err = j.dispatchJobExecution(req)
			...
			case "JobsClient.NewListSecretsPager":
				res.resp, res.err = j.dispatchNewListSecretsPager(req)
...
func (j *JobsServerTransport) dispatchJobExecution(req *http.Request) (*http.Response, error) {
	if j.srv.JobExecution == nil {
	...
	respr, errRespr := j.srv.JobExecution(req.Context(), resourceGroupNameParam, jobNameParam, jobExecutionNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
func (j *JobsServerTransport) dispatchNewListSecretsPager(req *http.Request) (*http.Response, error) {
	if j.srv.NewListSecretsPager == nil {
		return nil, &nonRetriableError{errors.New("fake for method NewListSecretsPager not implemented")}
	}
	newListSecretsPager := j.newListSecretsPager.get(req)
	if newListSecretsPager == nil {
		...
		resp := j.srv.NewListSecretsPager(resourceGroupNameParam, jobNameParam, nil)
		newListSecretsPager = &resp
		j.newListSecretsPager.add(req, newListSecretsPager)
	}
	...
	if !contains([]int{http.StatusOK}, resp.StatusCode) {
		j.newListSecretsPager.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusOK", resp.StatusCode)}
	}
