...
func (a *AnalyticsItemsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if analyticsItemsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = analyticsItemsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (a *AnalyticsItemsServerTransport) dispatchList(req *http.Request) (*http.Response, error) {
	...
	scopeParam := getOptional(armapplicationinsights.ItemScope(qp.Get("scope")))
	typeParam := getOptional(armapplicationinsights.ItemTypeParameter(qp.Get("type")))
	includeContentParam, err := parseOptional(qp.Get("includeContent"), strconv.ParseBool)
	if err != nil {
		return nil, err
	}
...
func (a *AnalyticsItemsServerTransport) dispatchPut(req *http.Request) (*http.Response, error) {
	...
	overrideItemParam, err := parseOptional(qp.Get("overrideItem"), strconv.ParseBool)
	if err != nil {
		return nil, err
	}
...
// set this to conditionally intercept incoming requests to AnalyticsItemsServerTransport
var analyticsItemsServerTransportInterceptor interface {
