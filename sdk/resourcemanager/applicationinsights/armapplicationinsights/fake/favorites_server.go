...
func (f *FavoritesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if favoritesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = favoritesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "FavoritesClient.Add":
				res.resp, res.err = f.dispatchAdd(req)
...
func (f *FavoritesServerTransport) dispatchList(req *http.Request) (*http.Response, error) {
	...
	favoriteTypeParam := getOptional(armapplicationinsights.FavoriteType(qp.Get("favoriteType")))
	sourceTypeParam := getOptional(armapplicationinsights.FavoriteSourceType(qp.Get("sourceType")))
...
// set this to conditionally intercept incoming requests to FavoritesServerTransport
var favoritesServerTransportInterceptor interface {
