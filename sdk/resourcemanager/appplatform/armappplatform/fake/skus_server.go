...
// The returned SKUsServerTransport instance is connected to an instance of armappplatform.SKUsClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewSKUsServerTransport(srv *SKUsServer) *SKUsServerTransport {
	return &SKUsServerTransport{
		srv:          srv,
		newListPager: newTracker[azfake.PagerResponder[armappplatform.SKUsClientListResponse]](),
	}
}

// SKUsServerTransport connects instances of armappplatform.SKUsClient to instances of SKUsServer.
// Don't use this type directly, use NewSKUsServerTransport instead.
type SKUsServerTransport struct {
	srv          *SKUsServer
	newListPager *tracker[azfake.PagerResponder[armappplatform.SKUsClientListResponse]]
}
...
func (s *SKUsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.AppPlatform/skus`
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armappplatform.SKUsClientListResponse, createLink func() string) {
