...
func ExampleProviderClient_GetFunctionAppStacks() {
	...
	client := appservice.NewProviderClient("<subscription-id>", cred, nil)
	pager := client.GetFunctionAppStacks(&appservice.ProviderClientGetFunctionAppStacksOptions{StackOsType: nil})
	for {
...
func ExampleProviderClient_GetFunctionAppStacksForLocation() {
	...
	pager := client.GetFunctionAppStacksForLocation("<location>",
		&appservice.ProviderClientGetFunctionAppStacksForLocationOptions{StackOsType: nil})
	for {
		nextResult := pager.NextPage(ctx)
		if err := pager.Err(); err != nil {
			log.Fatalf("failed to advance page: %v", err)
...
func ExampleProviderClient_GetWebAppStacksForLocation() {
	...
	pager := client.GetWebAppStacksForLocation("<location>",
		&appservice.ProviderClientGetWebAppStacksForLocationOptions{StackOsType: nil})
	for {
		nextResult := pager.NextPage(ctx)
		if err := pager.Err(); err != nil {
			log.Fatalf("failed to advance page: %v", err)
...
func ExampleProviderClient_GetWebAppStacks() {
	...
	ctx := context.Background()
	client := appservice.NewProviderClient("<subscription-id>", cred, nil)
	pager := client.GetWebAppStacks(&appservice.ProviderClientGetWebAppStacksOptions{StackOsType: nil})
	for {
		nextResult := pager.NextPage(ctx)
		if err := pager.Err(); err != nil {
			log.Fatalf("failed to advance page: %v", err)
