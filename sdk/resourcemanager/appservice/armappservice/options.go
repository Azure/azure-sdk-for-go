...
type EnvironmentsClientBeginDeleteOptions struct {
	// Specify <code>true</code> to force the deletion even if the App Service Environment contains resources. The default is
	// <code>false</code>.
	ForceDelete *bool
...
type EnvironmentsClientListUsagesOptions struct {
	...
	// or name.value eq 'Metric2') and startTime eq 2014-01-01T00:00:00Z and endTime eq 2014-12-31T23:59:59Z and timeGrain eq
...
type PlansClientListOptions struct {
	// Specify <code>true</code> to return all App Service plan properties. The default is <code>false</code>, which returns a
...
type PlansClientListWebAppsOptions struct {
	...
	// web apps starting from (including) the skipToken. Otherwise, the resulting list contains web apps from the start of the
	// list
	SkipToken *string
...
type PlansClientRestartWebAppsOptions struct {
	// Specify <code>true</code> to perform a soft restart, applies the configuration settings and restarts the apps if necessary.
	// The default is <code>false</code>, which always restarts and reprovisions the apps
	SoftRestart *bool
}
...
type RecommendationsClientListHistoryForHostingEnvironmentOptions struct {
	// Specify <code>false</code> to return all recommendations. The default is <code>true</code>, which returns only expired
	// recommendations.
	ExpiredOnly *bool

	// Filter is specified by using OData syntax. Example: $filter=channel eq 'Api' or channel eq 'Notification' and startTime
	// eq 2014-01-01T00:00:00Z and endTime eq 2014-12-31T23:59:59Z and timeGrain eq duration'[PT1H|PT1M|P1D]
	Filter *string
}
...
type RecommendationsClientListOptions struct {
	// Specify <code>true</code> to return only the most critical recommendations. The default is <code>false</code>, which returns
	// all recommendations.
	Featured *bool
...
type WebAppsClientListUsagesOptions struct {
	...
	// name.value eq 'Metric2') and startTime eq 2014-01-01T00:00:00Z and endTime eq 2014-12-31T23:59:59Z and timeGrain eq duration'[Hour|Minute|Day]'.
	Filter *string
}
