...
func ExampleProvidersClient_Create() {
	...
	// 			PublicNetworkAccess: to.Ptr(armattestation.PublicNetworkAccessTypeEnabled),
	// 			Status: to.Ptr(armattestation.ServiceStatusReady),
	// 			TpmAttestationAuthentication: to.Ptr(armattestation.TpmAttestationAuthenticationTypeEnabled),
...
func ExampleProvidersClient_NewListPager() {
	...
	for pager.More() {
		...
		// page = armattestation.ProvidersClientListResponse{
...
func ExampleProvidersClient_NewListByResourceGroupPager() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
	...
	pager := clientFactory.NewProvidersClient().NewListByResourceGroupPager("testrg1", nil)
	for pager.More() {
		page, err := pager.NextPage(ctx)
		if err != nil {
			log.Fatalf("failed to advance page: %v", err)
		...
		// If the HTTP response code is 200 as defined in example definition, your page structure would look as follows. Please pay attention that all the values in the output are fake values for just demo purposes.
		// page = armattestation.ProvidersClientListByResourceGroupResponse{
		// 	ProviderListResult: armattestation.ProviderListResult{
		// 		Value: []*armattestation.Provider{
		// 			{
		// 				Name: to.Ptr("myattestationprovider"),
