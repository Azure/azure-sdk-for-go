...
- Type of `Key.KeyName` has been changed from `*AutomationKeyName` to `*KeyName`
- Type of `Key.Permissions` has been changed from `*AutomationKeyPermissions` to `*KeyPermissions`
...
- Operation `*SoftwareUpdateConfigurationsClient.List` has supported pagination, use `*SoftwareUpdateConfigurationsClient.NewListPager` instead.
- Operation `*NodeCountInformationClient.Get` has supported pagination, use `*NodeCountInformationClient.NewGetPager` instead.
...
- Field `DscNodeConfiguration` of struct `DscNodeConfigurationClientCreateOrUpdateResponse` has been removed
...
- New enum type `AccountState` with values `AccountStateOk`, `AccountStateSuspended`, `AccountStateUnavailable`
- New enum type `KeyName` with values `KeyNamePrimary`, `KeyNameSecondary`
- New enum type `KeyPermissions` with values `KeyPermissionsFull`, `KeyPermissionsRead`
...
- New function `NewAccountsClient(subscriptionID string, credential azcore.TokenCredential, options *arm.ClientOptions) (*AccountsClient, error)`
- New function `*AccountsClient.ConvertGraphRunbookContent(ctx context.Context, resourceGroupName string, automationAccountName string, parameters GraphicalRunbookContent, options *AccountsClientConvertGraphRunbookContentOptions) (AccountsClientConvertGraphRunbookContentResponse, error)`
...
- New field `Body`, `ContentType` in struct `DscConfigurationClientGetContentResponse`
...
- New field `Body`, `ContentType` in struct `RunbookClientGetContentResponse`
- New field `RuntimeEnvironment` in struct `RunbookCreateOrUpdateProperties`
- New field `Body`, `ContentType` in struct `RunbookDraftClientGetContentResponse`
