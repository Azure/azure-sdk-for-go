...
func (a *ActivityServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if activityServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = activityServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "ActivityClient.NewListByModulePager":
				res.resp, res.err = a.dispatchNewListByModulePager(req)
			default:
...
func (a *ActivityServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/modules/(?P<moduleName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/activities/(?P<activityName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (a *ActivityServerTransport) dispatchNewListByModulePager(req *http.Request) (*http.Response, error) {
	...
	if newListByModulePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/modules/(?P<moduleName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/activities`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to ActivityServerTransport
var activityServerTransportInterceptor interface {
