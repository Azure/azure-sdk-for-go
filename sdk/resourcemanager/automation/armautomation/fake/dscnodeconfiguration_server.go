...
func (d *DscNodeConfigurationServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if dscNodeConfigurationServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = dscNodeConfigurationServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (d *DscNodeConfigurationServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/nodeConfigurations/(?P<nodeConfigurationName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (d *DscNodeConfigurationServerTransport) dispatchNewListByAutomationAccountPager(req *http.Request) (*http.Response, error) {
	...
	if newListByAutomationAccountPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/nodeConfigurations`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to DscNodeConfigurationServerTransport
var dscNodeConfigurationServerTransportInterceptor interface {
