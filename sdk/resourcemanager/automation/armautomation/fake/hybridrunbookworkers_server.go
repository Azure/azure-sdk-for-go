...
type HybridRunbookWorkersServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Patch func(ctx context.Context, resourceGroupName string, automationAccountName string, hybridRunbookWorkerGroupName string, hybridRunbookWorkerID string, hybridRunbookWorkerCreationParameters armautomation.HybridRunbookWorkerCreateParameters, options *armautomation.HybridRunbookWorkersClientPatchOptions) (resp azfake.Responder[armautomation.HybridRunbookWorkersClientPatchResponse], errResp azfake.ErrorResponder)
}
...
func (h *HybridRunbookWorkersServerTransport) dispatchPatch(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := h.srv.Patch(req.Context(), resourceGroupNameParam, automationAccountNameParam, hybridRunbookWorkerGroupNameParam, hybridRunbookWorkerIDParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
