...
func (j *JobStreamServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if jobStreamServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = jobStreamServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (j *JobStreamServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/jobs/(?P<jobName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/streams/(?P<jobStreamId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (j *JobStreamServerTransport) dispatchNewListByJobPager(req *http.Request) (*http.Response, error) {
	...
	if newListByJobPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/jobs/(?P<jobName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/streams`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to JobStreamServerTransport
var jobStreamServerTransportInterceptor interface {
