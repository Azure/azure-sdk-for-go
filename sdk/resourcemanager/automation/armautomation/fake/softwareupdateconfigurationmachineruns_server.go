...
func (s *SoftwareUpdateConfigurationMachineRunsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if softwareUpdateConfigurationMachineRunsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = softwareUpdateConfigurationMachineRunsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (s *SoftwareUpdateConfigurationMachineRunsServerTransport) dispatchGetByID(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/softwareUpdateConfigurationMachineRuns/(?P<softwareUpdateConfigurationMachineRunId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (s *SoftwareUpdateConfigurationMachineRunsServerTransport) dispatchList(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Automation/automationAccounts/(?P<automationAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/softwareUpdateConfigurationMachineRuns`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 4 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to SoftwareUpdateConfigurationMachineRunsServerTransport
var softwareUpdateConfigurationMachineRunsServerTransportInterceptor interface {
