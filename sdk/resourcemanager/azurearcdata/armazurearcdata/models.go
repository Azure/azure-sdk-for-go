...
type AvailabilityGroupConfigure struct {
	...
	// secondary replica over the network. This method does not require you to backup and restore a copy of the primary database
	// on the replica. MANUAL specifies manual seeding (default). This method requires you to create a backup of the database
	// on the primary replica and manually restore that backup on the secondary replica.
...
type AvailabilityGroupCreateUpdateConfiguration struct {
	...
	// Wait time (in milliseconds) for the sp_server_diagnostics system stored procedure to return server-health information,
	// before the server instance is assumed to be slow or not responding.
...
type AvailabilityGroupState struct {
	...
	// (also known as replicas) and the availability mode of the replica (synchronous-commit or asynchronous-commit mode). The
...
type CronTrigger struct {
	...
	// with the following format: '{minutes} {hours} {days} {months} {days-of-week}'.
	Expression *string
...
type DataBaseMigrationAssessment struct {
	...
	// database. This value is set as part of a successful RunTargetRecommendation job and is independent of when the migration
...
type ImpactedObjectsInfo struct {
	...
	// OR https://learn.microsoft.com/en-us/data-migration/sql-server/managed-instance/assessment-rules?view=azuresql
	FeatureID *string
...
type MigrationAssessment struct {
	...
	// READ-ONLY; The UTC timestamp in ISO 8601 format indicating when the latest target recommendations were generated for this
	// SQL Server instance. This value is set as part of a successful RunTargetRecommendation job and is independent of when the
...
type MigrationAssessmentSettings struct {
	...
	// and likely increases in future usage - This buffer is applied on top of the performance metrics. Default is 100 (percent).
	ComfortFactor *int32
...
type PostgresInstanceSKU struct {
	...
	// FLAG; CONSTANT; This field is required to be implemented by the Resource Provider if the service has more than one tier.
...
type SQLAvailabilityGroupReplicaResourceProperties struct {
	...
	// of the availability group that hosts one replica in the DAG. In a non-distributed availability group this field is optional
...
type SQLServerInstanceTargetRecommendationReportsRequest struct {
	...
	// This value must be used together with the corresponding sectionOffset to resume pagination. The reportOffset advances only
	// after all sections of the current report have been retrieved. To retrieve subsequent pages, use the nextReportOffset and
	...
	// This value must be used together with the corresponding reportOffset to resume pagination. To retrieve subsequent pages,
	// use the nextReportOffset and nextSectionOffset values from the previous response.
	SectionOffset *int32

	// Filters the report to a specific section type. If not specified, all section types are returned. Section types are grouped
	// into two categories: requirements sections contain performance and resource metrics collected from the source SQL Server,
	// while recommendation sections contain suggested Azure target SKUs derived from those requirements.
...
type SQLServerInstanceTargetRecommendationReportsResponse struct {
	...
	// value remains the same until all sections of the current report have been retrieved. Present only when there is more data
	// to retrieve. When both nextReportOffset and nextSectionOffset are absent, all data has been returned.
	...
	// the current report. Present only when there is more data to retrieve. When both nextReportOffset and nextSectionOffset
	// are absent, all data has been returned.
