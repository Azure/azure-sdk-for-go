...
// when the jobStatus is Succeeded. A target recommendation job must be started using the RunTargetRecommendationJob API before
// calling this API. Only the most recent report is returned. If the report contains more sections than can be returned in
// a single response, use the nextReportOffset and nextSectionOffset values from the response to retrieve the remaining data.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2026-03-01-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
...
// Server instance. The response always includes the jobStatus field, which indicates the state of the most recent target
// recommendation job (NotStarted, InProgress, Succeeded, or Failed). The report data is included in the response only when
// the jobStatus is Succeeded. A target recommendation job must be started using the RunTargetRecommendationJob API before
// calling this API. Only the most recent report is returned. If the report contains more sections than can be returned in
// a single response, use the nextReportOffset and nextSectionOffset values from the response to retrieve the remaining data.
// If the operation fails it returns an *azcore.ResponseError type.
//
...
// NewListByResourceGroupPager - List sqlServerInstance resources in the resource group
//
...
// at a time. If the previous job is in a non-terminal state (NotStarted or InProgress), calling this operation again returns
// the existing job status without creating a new job. A new job is created only when the previous job has reached a terminal
// state (Succeeded or Failed). This operation does not return recommendations directly. Use the GetTargetRecommendationReports
// API to check the job status and retrieve the target recommendation report if the jobStatus is Succeeded.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2026-03-01-preview
//   - resourceGroupName - The name of the resource group. The name is case insensitive.
...
// time. If the previous job is in a non-terminal state (NotStarted or InProgress), calling this operation again returns the
// existing job status without creating a new job. A new job is created only when the previous job has reached a terminal
// state (Succeeded or Failed). This operation does not return recommendations directly. Use the GetTargetRecommendationReports
// API to check the job status and retrieve the target recommendation report if the jobStatus is Succeeded.
// If the operation fails it returns an *azcore.ResponseError type.
//
