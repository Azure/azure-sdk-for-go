...
// or if certificates have been specified on the pool which should not be accessible by normal tasks but should be accessible
// by start tasks.
...
// Windows compute nodes. For Linux compute nodes, the certificates are stored in a directory inside the task working directory
// and an environment variable AZ_BATCH_CERTIFICATES_DIR is supplied to the task to query for this location. For certificates
// with visibility of 'remoteUser', a 'certs' directory is created in the user's home directory (e.g., /home/{user-name}/certs)
...
const (
	...
	// ComputeNodeDeallocationOptionRetainedData - Deprecated, we encourage you to upload task data to Azure Storage in your task
	// and use `TaskCompletion` instead. Allow currently running tasks to complete, then wait for all task data retention periods
	// to expire. Schedule no new tasks while waiting. Remove nodes when all task retention periods have expired.
...
// DiffDiskPlacement - The location where the OS disk should be placed.
type DiffDiskPlacement string

const (
	// DiffDiskPlacementCacheDisk - The OS disk will be placed on the cache disk of the VM.
...
// IPRuleAction - The action when client IP address is matched.
type IPRuleAction string

const (
	// IPRuleActionAllow - Deny client access.
...
// ResourceType - The result of the request to list operations.
type ResourceType string

const (
	// ResourceTypeMicrosoftBatchBatchAccounts - The resource type for Microsoft.Batch/batchAccounts
	ResourceTypeMicrosoftBatchBatchAccounts ResourceType = "Microsoft.Batch/batchAccounts"
)

// PossibleResourceTypeValues returns the possible values for the ResourceType const type.
func PossibleResourceTypeValues() []ResourceType {
	return []ResourceType{
		ResourceTypeMicrosoftBatchBatchAccounts,
	}
}

// SecurityEncryptionTypes - Specifies the EncryptionType of the managed disk. It is set to VMGuestStateOnly for encryption
// of just the VMGuestState blob, and NonPersistedTPM for not persisting firmware state in the VMGuestState blob. **Note**:
// It can be set for only Confidential VMs and required when using Confidential VMs.
