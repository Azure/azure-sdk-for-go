...
type DiffDiskSettings struct {
	...
	//
	// FLAG; CONSTANT; This property can be used by user in the request to choose which location the operating system should be
	// in. e.g., cache disk space for Ephemeral OS disk provisioning. For more information on Ephemeral OS disk size requirements,
	// please refer to Ephemeral OS disk size requirements for Windows VMs at https://learn.microsoft.com/azure/virtual-machines/windows/ephemeral-os-disks#size-requirements
...
// DiskCustomerManagedKey - The Customer Managed Key reference to encrypt the Disk.
type DiskCustomerManagedKey struct {
	// The reference of one of the pool identities to encrypt Disk. This identity will be used to access the KeyVault.
	IdentityReference *ComputeNodeIdentityReference

	// Fully versioned Key Url pointing to a key in KeyVault. Version segment of the Url is required regardless of rotationToLatestKeyVersionEnabled
	// value.
	KeyURL *string
...
type DiskEncryptionConfiguration struct {
	...
	//
	// Customer Managed Key will encrypt OS Disk by EncryptionAtRest, and by default we will encrypt the data disk as well. It
	// can be used only when the pool is configured with an identity and OsDisk is set as one of the targets of DiskEncryption.
...
// DiskEncryptionSetParameters - The ARM resource id of the disk encryption set.
type DiskEncryptionSetParameters struct {
	// The resource id of the disk encryption set.
	//
	// REQUIRED; The ARM resource id of the disk encryption set. The resource should be in the same subscription as the Batch
	// account.
	ID *string
}
...
// IPTag - Specifies how tasks should be distributed across compute nodes.
//
...
type ManagedDisk struct {
	// Specifies the customer managed disk encryption set resource id for the managed disk. It can be set only in UserSubscription
	// mode.
...
type ProxyAgentSettings struct {
	// Specifies whether Metadata Security Protocol feature should be enabled on the virtual machine or virtual machine scale
...
type PublicIPAddressConfiguration struct {
	...
	//
	// IP families are used to determine single-stack or dual-stack pools. For single-stack, the expected value is IPv4. For dual-stack,
...
type VMDiskSecurityProfile struct {
	...
	// encrypted ConfidentialVM OS Disk and VMGuest blob. It can be set only in UserSubscription mode.
	DiskEncryptionSet *DiskEncryptionSetParameters
