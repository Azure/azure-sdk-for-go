...
func (a *AccountsServerTransport) dispatchBeginAddPaymentTerms(req *http.Request) (*http.Response, error) {
	...
	if beginAddPaymentTerms == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/addPaymentTerms`
...
func (a *AccountsServerTransport) dispatchBeginCancelPaymentTerms(req *http.Request) (*http.Response, error) {
	...
	if beginCancelPaymentTerms == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/cancelPaymentTerms`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (a *AccountsServerTransport) dispatchConfirmTransition(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/confirmTransition`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 2 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (a *AccountsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 2 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (a *AccountsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		...
		includeAllWithoutBillingProfilesParam, err := parseOptional(qp.Get("includeAllWithoutBillingProfiles"), strconv.ParseBool)
		if err != nil {
			return nil, err
		}
		...
		includePendingAgreementParam, err := parseOptional(qp.Get("includePendingAgreement"), strconv.ParseBool)
		if err != nil {
			return nil, err
		}
		includeReselleeParam, err := parseOptional(qp.Get("includeResellee"), strconv.ParseBool)
		if err != nil {
			return nil, err
		}
		legalOwnerTIDParam := getOptional(qp.Get("legalOwnerTID"))
		legalOwnerOIDParam := getOptional(qp.Get("legalOwnerOID"))
...
func (a *AccountsServerTransport) dispatchNewListInvoiceSectionsByCreateSubscriptionPermissionPager(req *http.Request) (*http.Response, error) {
	...
	if newListInvoiceSectionsByCreateSubscriptionPermissionPager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/listInvoiceSectionsWithCreateSubscriptionPermission`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (a *AccountsServerTransport) dispatchValidatePaymentTerms(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/validatePaymentTerms`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 2 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
