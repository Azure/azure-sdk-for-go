...
func (c *CustomersServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if customersServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = customersServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "CustomersClient.GetByBillingAccount":
				res.resp, res.err = c.dispatchGetByBillingAccount(req)
			case "CustomersClient.NewListByBillingAccountPager":
				res.resp, res.err = c.dispatchNewListByBillingAccountPager(req)
			case "CustomersClient.NewListByBillingProfilePager":
				res.resp, res.err = c.dispatchNewListByBillingProfilePager(req)
			default:
...
func (c *CustomersServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/billingProfiles/(?P<billingProfileName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customers/(?P<customerName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (c *CustomersServerTransport) dispatchGetByBillingAccount(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customers/(?P<customerName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 3 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (c *CustomersServerTransport) dispatchNewListByBillingAccountPager(req *http.Request) (*http.Response, error) {
	...
	if newListByBillingAccountPager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customers`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (c *CustomersServerTransport) dispatchNewListByBillingProfilePager(req *http.Request) (*http.Response, error) {
	...
	if newListByBillingProfilePager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/billingProfiles/(?P<billingProfileName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customers`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to CustomersServerTransport
var customersServerTransportInterceptor interface {
