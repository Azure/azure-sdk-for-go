...
func (i *InvoicesServerTransport) dispatchNewListByBillingAccountPager(req *http.Request) (*http.Response, error) {
	...
	if newListByBillingAccountPager == nil {
		...
		periodStartDateParam, err := parseOptional(periodStartDateUnescaped, func(v string) (time.Time, error) { return time.Parse(time.DateOnly, v) })
		if err != nil {
			return nil, err
		}
		...
		periodEndDateParam, err := parseOptional(periodEndDateUnescaped, func(v string) (time.Time, error) { return time.Parse(time.DateOnly, v) })
		if err != nil {
			return nil, err
		}
