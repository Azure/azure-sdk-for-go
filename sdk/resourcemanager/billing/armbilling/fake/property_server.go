...
func (p *PropertyServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Billing/billingProperty/default`
	...
	qp := req.URL.Query()
	includeBillingCountryParam, err := parseOptional(qp.Get("includeBillingCountry"), strconv.ParseBool)
	if err != nil {
		return nil, err
	}
	includeTransitionStatusParam, err := parseOptional(qp.Get("includeTransitionStatus"), strconv.ParseBool)
	if err != nil {
		return nil, err
	}
