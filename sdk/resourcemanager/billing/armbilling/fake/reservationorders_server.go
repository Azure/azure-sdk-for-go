...
func (r *ReservationOrdersServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if reservationOrdersServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = reservationOrdersServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (r *ReservationOrdersServerTransport) dispatchGetByBillingAccount(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/reservationOrders/(?P<reservationOrderId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (r *ReservationOrdersServerTransport) dispatchNewListByBillingAccountPager(req *http.Request) (*http.Response, error) {
	...
	if newListByBillingAccountPager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/reservationOrders`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to ReservationOrdersServerTransport
var reservationOrdersServerTransportInterceptor interface {
