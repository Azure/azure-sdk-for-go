...
func (s *SavingsPlanOrdersServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if savingsPlanOrdersServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = savingsPlanOrdersServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "SavingsPlanOrdersClient.GetByBillingAccount":
				res.resp, res.err = s.dispatchGetByBillingAccount(req)
...
func (s *SavingsPlanOrdersServerTransport) dispatchGetByBillingAccount(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/savingsPlanOrders/(?P<savingsPlanOrderId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (s *SavingsPlanOrdersServerTransport) dispatchNewListByBillingAccountPager(req *http.Request) (*http.Response, error) {
	...
	if newListByBillingAccountPager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/savingsPlanOrders`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to SavingsPlanOrdersServerTransport
var savingsPlanOrdersServerTransportInterceptor interface {
