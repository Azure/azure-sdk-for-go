...
// credit notes) as a zip file. The operation is supported for billing accounts with agreement type Microsoft Partner Agreement
...
// notes) as a zip file. The operation is supported for billing accounts with agreement type Microsoft Partner Agreement or
...
func (client *InvoicesClient) listByBillingAccountCreateRequest(ctx context.Context, billingAccountName string, options *InvoicesClientListByBillingAccountOptions) (*policy.Request, error) {
	...
	if options != nil && options.PeriodEndDate != nil {
		reqQP.Set("periodEndDate", options.PeriodEndDate.Format(time.DateOnly))
	}
	if options != nil && options.PeriodStartDate != nil {
		reqQP.Set("periodStartDate", options.PeriodStartDate.Format(time.DateOnly))
	}
