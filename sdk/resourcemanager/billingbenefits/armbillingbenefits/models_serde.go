...
// MarshalJSON implements the json.Marshaller interface for type PricingCurrencyTotal.
func (p PricingCurrencyTotal) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "amount", p.Amount)
	populate(objectMap, "currencyCode", p.CurrencyCode)
...
// UnmarshalJSON implements the json.Unmarshaller interface for type PricingCurrencyTotal.
func (p *PricingCurrencyTotal) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
		return fmt.Errorf("unmarshalling type %T: %v", p, err)
	}
