...
func (e *EmailServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if emailServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = emailServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "EmailClient.CreateSignInURL":
				res.resp, res.err = e.dispatchCreateSignInURL(req)
			default:
...
func (e *EmailServerTransport) dispatchCreateSignInURL(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.BotService/botServices/(?P<resourceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/createEmailSignInUrl`
...
// set this to conditionally intercept incoming requests to EmailServerTransport
var emailServerTransportInterceptor interface {
