...
func (a *AFDCustomDomainsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if afdCustomDomainsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = afdCustomDomainsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "AFDCustomDomainsClient.BeginRefreshValidationToken":
				res.resp, res.err = a.dispatchBeginRefreshValidationToken(req)
...
func (a *AFDCustomDomainsServerTransport) dispatchBeginCreate(req *http.Request) (*http.Response, error) {
	...
	if beginCreate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cdn/profiles/(?P<profileName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customDomains/(?P<customDomainName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (a *AFDCustomDomainsServerTransport) dispatchNewListByProfilePager(req *http.Request) (*http.Response, error) {
	...
	if newListByProfilePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cdn/profiles/(?P<profileName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customDomains`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (a *AFDCustomDomainsServerTransport) dispatchBeginRefreshValidationToken(req *http.Request) (*http.Response, error) {
	...
	if beginRefreshValidationToken == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cdn/profiles/(?P<profileName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customDomains/(?P<customDomainName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/refreshValidationToken`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to AFDCustomDomainsServerTransport
var afdCustomDomainsServerTransportInterceptor interface {
