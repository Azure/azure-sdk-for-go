	...
	poller, err := clientFactory.NewPoliciesClient().BeginCreateOrUpdate(ctx, "rg1", "MicrosoftCdnWafPolicy", armcdn.WafPolicy{
		...
		Properties: &armcdn.WafPolicyProperties{
			...
			PolicySettings: &armcdn.PolicySettings{
				...
				DefaultCustomBlockResponseStatusCode: to.Ptr(armcdn.PolicySettingsDefaultCustomBlockResponseStatusCodeTwoHundred),
...
func ExamplePoliciesClient_Delete() {
	...
	res, err := clientFactory.NewPoliciesClient().Delete(ctx, "rg1", "Policy1", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	// You could use response here. We use blank identifier for just demo purposes.
	_ = res
	// If the HTTP response code is 200 as defined in example definition, your response structure would look as follows. Please pay attention that all the values in the output are fake values for just demo purposes.
	// res = armcdn.PoliciesClientDeleteResponse{
	// }
}
...
func ExamplePoliciesClient_BeginUpdate() {
	...
	// 			PolicySettings: &armcdn.PolicySettings{
	// 				DefaultCustomBlockResponseBody: to.Ptr("PGh0bWw+CjxoZWFkZXI+PHRpdGxlPkhlbGxvPC90aXRsZT48L2hlYWRlcj4KPGJvZHk+CkhlbGxvIHdvcmxkCjwvYm9keT4KPC9odG1sPg=="),
	// 				DefaultCustomBlockResponseStatusCode: to.Ptr(			armcdn.PolicySettingsDefaultCustomBlockResponseStatusCodeFourHundredThree),
	// 				DefaultRedirectURL: to.Ptr("http://www.bing.com"),
	// 				EnabledState: to.Ptr(armcdn.PolicyEnabledStateEnabled),
	// 				Mode: to.Ptr(armcdn.PolicyModePrevention),
	// 			},
