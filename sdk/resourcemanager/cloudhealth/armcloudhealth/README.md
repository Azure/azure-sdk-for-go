...
clientFactory, err := armcloudhealth.NewClientFactory(<subscription ID>, cred, &options)
```
...
```go
client := clientFactory.NewAuthenticationSettingsClient()
```
...
[open an issue](https://github.com/Azure/azure-sdk-for-go/issues) and assign the `Cloudhealth` label.
