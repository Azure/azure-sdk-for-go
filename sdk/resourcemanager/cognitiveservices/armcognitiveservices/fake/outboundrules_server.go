...
// The returned OutboundRulesServerTransport instance is connected to an instance of armcognitiveservices.OutboundRulesClient via the
...
// OutboundRulesServerTransport connects instances of armcognitiveservices.OutboundRulesClient to instances of OutboundRulesServer.
...
func (o *OutboundRulesServerTransport) dispatchBeginPost(req *http.Request) (*http.Response, error) {
	...
	if beginPost == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.CognitiveServices/accounts/(?P<accountName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/managedNetworks/(?P<managedNetworkName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/batchOutboundRules`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armcognitiveservices.ManagedNetworkSettingsBasicResource](req)
		if err != nil {
			return nil, err
		}
		...
		respr, errRespr := o.srv.BeginPost(req.Context(), resourceGroupNameParam, accountNameParam, managedNetworkNameParam, body, nil)
