...
type AADAuthTypeConnectionProperties struct {
	...
	// READ-ONLY; A type definition that refers the id to an Azure Resource Manager resource.
...
// AzureOpenAI: Category:= AzureOpenAI AuthType:= ApiKey (as type discriminator) Credentials:= {ApiKey} as .ApiKey
// Target:= {ApiBase}
// CognitiveService: Category:= CognitiveService AuthType:= ApiKey (as type discriminator) Credentials:= {SubscriptionKey}
// as ApiKey Target:= ServiceRegion={serviceRegion}
// CognitiveSearch: Category:= CognitiveSearch AuthType:= ApiKey (as type discriminator) Credentials:= {Key} as ApiKey Target:=
// {Endpoint}
...
// ConnectionOAuth2 - ClientId and ClientSecret are required. Other properties are optional depending on each OAuth2 provider's
// implementation.
...
// CustomKeysConnectionProperties - Category:= CustomKeys AuthType:= CustomKeys (as type discriminator) Credentials:= {CustomKeys}
// as CustomKeys Target:= {any value} Use Metadata property bag for ApiVersion and other metadata fields
