...
func (testsuite *DedicatedHostTestSuite) SetupSuite() {
	...
	testsuite.hostGroupName = testutil.GenerateAlphaNumericID(testsuite.T(), "hostgroupn", 6)
...
func (testsuite *DedicatedHostTestSuite) TestDedicatedHosts() {
	hostName := testutil.GenerateAlphaNumericID(testsuite.T(), "hostname", 6)
	var err error
	...
	testsuite.Require().NoError(err)
	dedicatedHostsClientCreateOrUpdateResponsePoller, err := dedicatedHostsClient.BeginCreateOrUpdate(testsuite.ctx, testsuite.resourceGroupName, testsuite.hostGroupName, hostName, armcompute.DedicatedHost{
		Location: to.Ptr(testsuite.location),
		Tags: map[string]*string{
	...
	_, err = dedicatedHostsClient.Get(testsuite.ctx, testsuite.resourceGroupName, testsuite.hostGroupName, hostName, &armcompute.DedicatedHostsClientGetOptions{Expand: nil})
	testsuite.Require().NoError(err)

	// From step DedicatedHosts_Update
	fmt.Println("Call operation: DedicatedHosts_Update")
	dedicatedHostsClientUpdateResponsePoller, err := dedicatedHostsClient.BeginUpdate(testsuite.ctx, testsuite.resourceGroupName, testsuite.hostGroupName, hostName, armcompute.DedicatedHostUpdate{}, nil)
	testsuite.Require().NoError(err)
	...
	dedicatedHostsClientRestartResponsePoller, err := dedicatedHostsClient.BeginRestart(testsuite.ctx, testsuite.resourceGroupName, testsuite.hostGroupName, hostName, nil)
	testsuite.Require().NoError(err)
	...
	dedicatedHostsClientDeleteResponsePoller, err := dedicatedHostsClient.BeginDelete(testsuite.ctx, testsuite.resourceGroupName, testsuite.hostGroupName, hostName, nil)
	testsuite.Require().NoError(err)
