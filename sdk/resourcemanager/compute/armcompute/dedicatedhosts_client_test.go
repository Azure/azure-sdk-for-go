...
func TestDedicatedHostsClient_CreateOrUpdate(t *testing.T) {
	...
	rg,clean := createResourceGroup(t,cred,opt,subscriptionID,"dhcreate","eastus")
	rgName := *rg.Name
	defer clean()
	...
	dhClient := armcompute.NewDedicatedHostsClient(subscriptionID,cred,opt)
	...
	require.NoError(t, err)
	_, _ = dhPoller.PollUntilDone(context.Background(), 10*time.Second)
	//require.NoError(t, err)
