...
func TestVirtualMachinesClient_CreateOrUpdate(t *testing.T) {
	...
	// create subnet
	subClient := armnetwork.NewSubnetsClient(conn, subscriptionID)
	...
	ipClient := armnetwork.NewPublicIPAddressesClient(conn, subscriptionID)
	...
	nsgClient := armnetwork.NewNetworkSecurityGroupsClient(conn, subscriptionID)
	...
	vmClient := armcompute.NewVirtualMachinesClient(conn, subscriptionID)
