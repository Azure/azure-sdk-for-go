...
clientFactory, err := armcomputebulkactions.NewClientFactory(<subscription ID>, cred, &options)
```
...
```go
client := clientFactory.NewBulkActionsClient()
```
...
[open an issue](https://github.com/Azure/azure-sdk-for-go/issues) and assign the `Computebulkactions` label.
