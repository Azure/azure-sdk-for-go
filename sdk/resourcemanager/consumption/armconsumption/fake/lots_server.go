...
func (l *LotsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if lotsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = lotsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "LotsClient.NewListByBillingAccountPager":
				res.resp, res.err = l.dispatchNewListByBillingAccountPager(req)
			case "LotsClient.NewListByBillingProfilePager":
				res.resp, res.err = l.dispatchNewListByBillingProfilePager(req)
			case "LotsClient.NewListByCustomerPager":
				res.resp, res.err = l.dispatchNewListByCustomerPager(req)
			default:
...
func (l *LotsServerTransport) dispatchNewListByBillingAccountPager(req *http.Request) (*http.Response, error) {
	...
	if newListByBillingAccountPager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Consumption/lots`
...
func (l *LotsServerTransport) dispatchNewListByBillingProfilePager(req *http.Request) (*http.Response, error) {
	...
	if newListByBillingProfilePager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/billingProfiles/(?P<billingProfileId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Consumption/lots`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (l *LotsServerTransport) dispatchNewListByCustomerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByCustomerPager == nil {
		const regexStr = `/providers/Microsoft\.Billing/billingAccounts/(?P<billingAccountId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customers/(?P<customerId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Consumption/lots`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to LotsServerTransport
var lotsServerTransportInterceptor interface {
