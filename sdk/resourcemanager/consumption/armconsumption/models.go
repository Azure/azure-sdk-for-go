...
type BudgetProperties struct {
	...
	// date. Budget start date must be on or after June 1, 2017. Future start date should not be more than twelve months. Past
	// start date should be selected within the timegrain period. There are no restrictions on the end date.
...
type LegacyReservationTransactionProperties struct {
	...
	// a single purchase transaction. A reservation order contains reservations. The reservation order specifies the VM size and
	// region for the reservations.
...
type LegacyUsageDetailProperties struct {
	...
	// this field to get usage line item specific details such as the actual VM Size (ServiceType) or the ratio in which the reservation
	// discount is applied.
	...
	// for which the reservation was purchased. For the actual VM Size for which the reservation is purchased see productOrderName.
	MeterID *string
...
type ModernUsageDetailProperties struct {
	...
	// across the customers who have onboarded the Microsoft customer agreement and the customers in CSP who have made entitlement
	// purchases like SaaS, Marketplace, RI, etc.
	BillingProfileID *string
	...
	// it displays 12 months for yearly term of reserved instance. For one time purchases or recurring purchases, the terms displays
	// 1 month; This is not applicable for Azure consumption.
	Term *string
...
type Notification struct {
	...
	// email or contact group specified at the Subscription or Resource Group scopes. All other scopes must have at least one
...
type ReservationDetailProperties struct {
	...
	// for applying the benefit scope and also specifies the number of instances to which the reservation benefit can be applied
	// to.
	ReservationID *string
...
type ReservationRecommendationDetailsResourceProperties struct {
	...
	// and Azure Files, which use monthly rates. Only hardware rates are included; software rates are excluded. Note: there could
	// be new resources that use hourly rates in the future.
	OnDemandRate *float32

	// READ-ONLY; Azure product ex: Standard_E8s_v3 etc.
	Product *string
