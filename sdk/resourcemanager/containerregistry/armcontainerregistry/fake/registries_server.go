...
type RegistriesServer struct {
	...
	// NewListUsagesPager is the fake for method RegistriesClient.NewListUsagesPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListUsagesPager func(resourceGroupName string, registryName string, options *armcontainerregistry.RegistriesClientListUsagesOptions) (resp azfake.PagerResponder[armcontainerregistry.RegistriesClientListUsagesResponse])
...
func NewRegistriesServerTransport(srv *RegistriesServer) *RegistriesServerTransport {
	return &RegistriesServerTransport{
		...
		newListUsagesPager:               newTracker[azfake.PagerResponder[armcontainerregistry.RegistriesClientListUsagesResponse]](),
...
type RegistriesServerTransport struct {
	...
	newListUsagesPager               *tracker[azfake.PagerResponder[armcontainerregistry.RegistriesClientListUsagesResponse]]
...
func (r *RegistriesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "RegistriesClient.NewListUsagesPager":
				res.resp, res.err = r.dispatchNewListUsagesPager(req)
...
func (r *RegistriesServerTransport) dispatchGetPrivateLinkResource(req *http.Request) (*http.Response, error) {
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).MyPrivateLinkResource, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
...
func (r *RegistriesServerTransport) dispatchNewListUsagesPager(req *http.Request) (*http.Response, error) {
	if r.srv.NewListUsagesPager == nil {
		return nil, &nonRetriableError{errors.New("fake for method NewListUsagesPager not implemented")}
	}
	newListUsagesPager := r.newListUsagesPager.get(req)
	if newListUsagesPager == nil {
		...
		resp := r.srv.NewListUsagesPager(resourceGroupNameParam, registryNameParam, nil)
		newListUsagesPager = &resp
		r.newListUsagesPager.add(req, newListUsagesPager)
	}
	...
	if !contains([]int{http.StatusOK}, resp.StatusCode) {
		r.newListUsagesPager.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusOK", resp.StatusCode)}
	}
