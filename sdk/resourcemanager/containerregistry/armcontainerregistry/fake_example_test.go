...
func ExampleAgentPoolsServer() {
	...
	fakeAgentPoolsServer := fake.AgentPoolsServer{

		// next, provide implementations for the APIs you wish to fake.
		// this fake corresponds to the AgentPoolsClient.Get() API.
		Get: func(ctx context.Context, resourceGroupName, registryName, agentPoolName string, options *armcontainerregistry.AgentPoolsClientGetOptions) (resp azfake.Responder[armcontainerregistry.AgentPoolsClientGetResponse], errResp azfake.ErrorResponder) {
			...
			agentPoolsResp := armcontainerregistry.AgentPoolsClientGetResponse{}
			agentPoolsResp.ID = to.Ptr("/fake/resource/id")

			// use resp to set the desired response
			resp.SetResponse(http.StatusOK, agentPoolsResp, nil)
	...
	client, err := armcontainerregistry.NewAgentPoolsClient("subscriptionID", azfake.NewTokenCredential(), &arm.ClientOptions{
		ClientOptions: azcore.ClientOptions{
			Transport: fake.NewAgentPoolsServerTransport(&fakeAgentPoolsServer),
		},
	})
	if err != nil {
		log.Fatal(err)
	...
	resp, err := client.Get(context.TODO(), "fakeResourceGroup", "fakeRegistry", "fakeAgentPool", nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	_, err = client.GetQueueStatus(context.TODO(), "fakeResourceGroup", "fakeRegistry", "fakeAgentPool", nil)

	fmt.Println(err.Error())

	// Output:
	// /fake/resource/id
	// fake for method GetQueueStatus not implemented
}
