...
type MachineKubernetesProfile struct {
	...
	// These taints will not be reconciled by AKS and can be removed with a kubectl call. These taints allow for required configuration
	// to run before the node is ready to accept workloads, for example
	...
	// Both patch version and are supported. When is specified, the latest supported patch version is chosen automatically.
...
type MachineNetworkProperties struct {
	// Some scenarios may require the machine to receive their own dedicated public IP addresses. A common scenario is for gaming
...
type MachineStatus struct {
	...
	// READ-ONLY; Indicates whether a machine has deviated from its expected state due to changes in managed cluster properties,
	// requiring corrective action.
	DriftAction *DriftAction

	// READ-ONLY; Provides detailed information on why the machine has drifted. This field is omitted if the machine is up to
	// date.
	DriftReason *string
	...
	// READ-ONLY; Indicates the current state of the underlying virtual machine.
	VMState *VMState
}
