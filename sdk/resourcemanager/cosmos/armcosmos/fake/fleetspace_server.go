...
type FleetspaceServer struct {
	...
	BeginUpdate func(ctx context.Context, resourceGroupName string, fleetName string, fleetspaceName string, body armcosmos.FleetspaceUpdate, options *armcosmos.FleetspaceClientBeginUpdateOptions) (resp azfake.PollerResponder[armcosmos.FleetspaceClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (f *FleetspaceServerTransport) dispatchBeginCreate(req *http.Request) (*http.Response, error) {
	...
	if beginCreate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DocumentDB/fleets/(?P<fleetName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/fleetspaces/(?P<fleetspaceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (f *FleetspaceServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DocumentDB/fleets/(?P<fleetName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/fleetspaces`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
