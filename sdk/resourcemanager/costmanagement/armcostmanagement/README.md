...
Azure Costmanagement modules consist of one or more clients.  A client groups a set of related APIs, providing access to its functionality within the specified subscription.  Create one or more clients to access the APIs you require using your credential.

```go
client, err := armcostmanagement.NewAlertsClient(<subscription ID>, cred, nil)
```
...
client, err := armcostmanagement.NewAlertsClient(<subscription ID>, cred, &options)
```
