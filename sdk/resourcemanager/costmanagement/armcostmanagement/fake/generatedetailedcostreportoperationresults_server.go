...
func (g *GenerateDetailedCostReportOperationResultsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if generateDetailedCostReportOperationResultsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = generateDetailedCostReportOperationResultsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "GenerateDetailedCostReportOperationResultsClient.BeginGet":
				res.resp, res.err = g.dispatchBeginGet(req)
			default:
...
func (g *GenerateDetailedCostReportOperationResultsServerTransport) dispatchBeginGet(req *http.Request) (*http.Response, error) {
	...
	if beginGet == nil {
		const regexStr = `/(?P<scope>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.CostManagement/operationResults/(?P<operationId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
// set this to conditionally intercept incoming requests to GenerateDetailedCostReportOperationResultsServerTransport
