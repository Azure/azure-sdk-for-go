...
func (g *GenerateReservationDetailsReportServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if generateReservationDetailsReportServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = generateReservationDetailsReportServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "GenerateReservationDetailsReportClient.BeginByBillingAccountID":
				res.resp, res.err = g.dispatchBeginByBillingAccountID(req)
			case "GenerateReservationDetailsReportClient.BeginByBillingProfileID":
				res.resp, res.err = g.dispatchBeginByBillingProfileID(req)
			default:
...
func (g *GenerateReservationDetailsReportServerTransport) dispatchBeginByBillingAccountID(req *http.Request) (*http.Response, error) {
	...
	if beginByBillingAccountID == nil {
		const regexStr = `/providers/microsoft\.Billing/billingAccounts/(?P<billingAccountId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.CostManagement/generateReservationDetailsReport`
		...
		respr, errRespr := g.srv.BeginByBillingAccountID(req.Context(), billingAccountIDParam, qp.Get("startDate"), qp.Get("endDate"), nil)
...
func (g *GenerateReservationDetailsReportServerTransport) dispatchBeginByBillingProfileID(req *http.Request) (*http.Response, error) {
	...
	if beginByBillingProfileID == nil {
		const regexStr = `/providers/microsoft\.Billing/billingAccounts/(?P<billingAccountId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/billingProfiles/(?P<billingProfileId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.CostManagement/generateReservationDetailsReport`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		respr, errRespr := g.srv.BeginByBillingProfileID(req.Context(), billingAccountIDParam, billingProfileIDParam, qp.Get("startDate"), qp.Get("endDate"), nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
...
// set this to conditionally intercept incoming requests to GenerateReservationDetailsReportServerTransport
var generateReservationDetailsReportServerTransportInterceptor interface {
