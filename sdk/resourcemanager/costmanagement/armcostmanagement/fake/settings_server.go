...
func (s *SettingsServerTransport) dispatchCreateOrUpdateByScope(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/(?P<scope>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.CostManagement/settings/(?P<type>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (s *SettingsServerTransport) dispatchList(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/(?P<scope>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.CostManagement/settings`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 2 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
