...
func (d *DeviceCapacityCheckServerTransport) dispatchBeginCheckResourceCreationFeasibility(req *http.Request) (*http.Response, error) {
	...
	if beginCheckResourceCreationFeasibility == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DataBoxEdge/dataBoxEdgeDevices/(?P<deviceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/deviceCapacityCheck`
		...
		capacityNameParam := getOptional(qp.Get("capacityName"))
