...
func (d *DeviceCapacityInfoServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if deviceCapacityInfoServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = deviceCapacityInfoServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "DeviceCapacityInfoClient.GetDeviceCapacityInfo":
				res.resp, res.err = d.dispatchGetDeviceCapacityInfo(req)
			default:
...
func (d *DeviceCapacityInfoServerTransport) dispatchGetDeviceCapacityInfo(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DataBoxEdge/dataBoxEdgeDevices/(?P<deviceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/deviceCapacityInfo/default`
...
// set this to conditionally intercept incoming requests to DeviceCapacityInfoServerTransport
var deviceCapacityInfoServerTransportInterceptor interface {
