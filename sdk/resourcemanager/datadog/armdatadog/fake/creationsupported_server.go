...
func (c *CreationSupportedServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Datadog/subscriptionStatuses/default`
	...
	qp := req.URL.Query()
	respr, errRespr := c.srv.Get(req.Context(), qp.Get("datadogOrganizationId"), nil)
...
func (c *CreationSupportedServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Datadog/subscriptionStatuses`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		qp := req.URL.Query()
		resp := c.srv.NewListPager(qp.Get("datadogOrganizationId"), nil)
		newListPager = &resp
