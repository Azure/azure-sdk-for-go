...
- New struct `DataLakeAnalyticsUsqlActivity`
- New struct `DataLakeAnalyticsUsqlActivityTypeProperties`
- New struct `DatabricksJobActivity`
- New struct `DatabricksJobActivityTypeProperties`
- New struct `EnableInteractiveQueryRequest`
- New struct `ExecuteSsisPackageActivity`
- New struct `ExecuteSsisPackageActivityTypeProperties`
- New struct `InteractiveQueryProperties`
- New struct `JiraTableDatasetTypeProperties`
- New struct `SsisAccessCredential`
- New struct `SsisChildPackage`
- New struct `SsisExecutionCredential`
- New struct `SsisExecutionParameter`
- New struct `SsisLogLocation`
- New struct `SsisLogLocationTypeProperties`
- New struct `SsisPackageLocation`
- New struct `SsisPackageLocationTypeProperties`
- New struct `SsisPropertyOverride`
