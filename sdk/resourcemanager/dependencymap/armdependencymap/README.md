...
Install the Azure Dependencymap module:

```sh
go get github.com/Azure/azure-sdk-for-go/sdk/resourcemanager/dependencymap/armdependencymap
```
...
Azure Dependencymap module consists of one or more clients. We provide a client factory which could be used to create any client in this module.

```go
clientFactory, err := armdependencymap.NewClientFactory(<subscription ID>, cred, nil)
```
...
clientFactory, err := armdependencymap.NewClientFactory(<subscription ID>, cred, &options)
```
...
```go
client := clientFactory.NewDiscoverySourcesClient()
```
...
[open an issue](https://github.com/Azure/azure-sdk-for-go/issues) and assign the `Dependencymap` label.
