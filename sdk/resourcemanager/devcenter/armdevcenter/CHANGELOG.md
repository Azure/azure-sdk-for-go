...
- New enum type `AssignedGroupScope` with values `AssignedGroupScopeDevBox`
...
- New enum type `CancelOnConnectEnableStatus` with values `CancelOnConnectEnableStatusDisabled`, `CancelOnConnectEnableStatusEnabled`
- New enum type `CatalogAutoImageBuildEnableStatus` with values `CatalogAutoImageBuildEnableStatusDisabled`, `CatalogAutoImageBuildEnableStatusEnabled`
...
- New enum type `FeatureStateModifiable` with values `FeatureStateModifiableModifiable`, `FeatureStateModifiableNotModifiable`
- New enum type `FeatureStatus` with values `FeatureStatusAutoDeploy`, `FeatureStatusDisabled`, `FeatureStatusEnabled`
...
- New struct `InheritedProjectCatalogSettings`
...
- New field `AutoImageBuildEnableStatus` in struct `CatalogProperties`
- New field `AutoImageBuildEnableStatus` in struct `CatalogUpdateProperties`
- New field `FederatedClientID` in struct `CustomerManagedKeyEncryptionKeyIdentity`
- New field `Architecture` in struct `ImageProperties`
...
- New field `AssignedGroups`, `AzureAiServicesSettings`, `CustomizationSettings`, `DevBoxScheduleDeleteSettings`, `ServerlessGpuSessionsSettings`, `WorkspaceStorageSettings` in struct `ProjectProperties`
- New field `AssignedGroups`, `AzureAiServicesSettings`, `CustomizationSettings`, `DevBoxScheduleDeleteSettings`, `ServerlessGpuSessionsSettings`, `WorkspaceStorageSettings` in struct `ProjectUpdateProperties`
