...
func (p *ProjectAllowedEnvironmentTypesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if projectAllowedEnvironmentTypesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = projectAllowedEnvironmentTypesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (p *ProjectAllowedEnvironmentTypesServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DevCenter/projects/(?P<projectName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/allowedEnvironmentTypes/(?P<environmentTypeName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (p *ProjectAllowedEnvironmentTypesServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DevCenter/projects/(?P<projectName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/allowedEnvironmentTypes`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to ProjectAllowedEnvironmentTypesServerTransport
var projectAllowedEnvironmentTypesServerTransportInterceptor interface {
