...
func ExampleDpsCertificateClient_NewListPager() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
	...
	pager := clientFactory.NewDpsCertificateClient().NewListPager("myResourceGroup", "myFirstProvisioningService", nil)
	for pager.More() {
		...
		// page = armdeviceprovisioningservices.DpsCertificateClientListResponse{
