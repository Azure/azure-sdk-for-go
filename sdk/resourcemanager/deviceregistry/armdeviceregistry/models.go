...
// ActivateCertificateAuthorityRequest - Request payload for activating a Certificate Authority with a signed certificate.
type ActivateCertificateAuthorityRequest struct {
	// REQUIRED; Certificate chain in PEM format for activating an externally issued intermediate Certificate Authority, including
	// the signed certificate. The first certificate must be the signed certificate matching the CSR generated by the service,
	// followed by any intermediate CAs, and optionally the root CA. Certificates must be ordered from leaf to root and concatenated
	// in PEM format.
...
// CertificateAuthority - A Certificate Authority resource.
type CertificateAuthority struct {
	// REQUIRED; The geo-location where the resource lives
	Location *string

	// The resource-specific properties for this resource.
	Properties CertificateAuthorityPropertiesClassification
...
// CertificateAuthorityIssuer - The issuer for an intermediate Certificate Authority.
type CertificateAuthorityIssuer struct {
	// REQUIRED; The Certificate Authority issuer type.
	IssuerType *CertificateAuthorityIssuerType
}

// GetCertificateAuthorityIssuer implements the CertificateAuthorityIssuerClassification interface for type CertificateAuthorityIssuer.
func (c *CertificateAuthorityIssuer) GetCertificateAuthorityIssuer() *CertificateAuthorityIssuer {
	return c
}

// CertificateAuthorityListResult - The response of a CertificateAuthority list operation.
type CertificateAuthorityListResult struct {
	// REQUIRED; The CertificateAuthority items on this page
...
// CertificateAuthorityProperties - Details of the Certificate Authority resource.
type CertificateAuthorityProperties struct {
	// REQUIRED; The Certificate Authority type. Use Root for a self-signed, service-managed root CA, and ICA for an intermediate
	// CA.
	CertificateAuthorityType *CertificateAuthorityType

	// REQUIRED; Crypto type: ECC.
	...
	// READ-ONLY; Globally unique, immutable, non-reusable identifier of the Certificate Authority resource.
	UUID *string

	// READ-ONLY; Certificate is valid not after this date. Format ISO8601.
	ValidityNotAfter *time.Time

	// READ-ONLY; Certificate is valid not before this date. Format ISO8601.
	ValidityNotBefore *time.Time
}

// GetCertificateAuthorityProperties implements the CertificateAuthorityPropertiesClassification interface for type CertificateAuthorityProperties.
func (c *CertificateAuthorityProperties) GetCertificateAuthorityProperties() *CertificateAuthorityProperties {
	return c
}

// CertificateAuthorityUpdate - Update model for Certificate Authority.
type CertificateAuthorityUpdate struct {
	// Resource tags.
	Tags map[string]*string
}

// CertificatePolicy - A Certificate Policy resource.
...
// CertificatePolicyConfiguration - The Certificate Policy certificate configuration.
type CertificatePolicyConfiguration struct {
	// REQUIRED; The validity period in days.
	ValidityPeriodInDays *int32
}

// CertificatePolicyListResult - The response of a CertificatePolicy list operation.
type CertificatePolicyListResult struct {
	// REQUIRED; The CertificatePolicy items on this page
...
// CertificatePolicyProperties - Details of the Certificate Policy resource.
type CertificatePolicyProperties struct {
	...
	// READ-ONLY; Globally unique, immutable, non-reusable identifier of the Certificate Policy resource.
	UUID *string
}

// CertificatePolicyUpdate - The type used for update operations of the CertificatePolicy.
type CertificatePolicyUpdate struct {
	// The resource-specific properties for this resource.
...
// CertificatePolicyUpdateProperties - The updatable properties of the CertificatePolicy.
type CertificatePolicyUpdateProperties struct {
	// The certificate configuration.
...
type DeviceStatusEndpoint struct {
	...
	// READ-ONLY; Health state of the endpoint.
...
// ExternalCertificateAuthorityIssuer - An external issuer for an intermediate Certificate Authority.
type ExternalCertificateAuthorityIssuer struct {
	// CONSTANT; The external issuer type.
	// Field has constant value CertificateAuthorityIssuerTypeExternal, any specified value is ignored.
	IssuerType *CertificateAuthorityIssuerType

	// READ-ONLY; Certificate Signing Request (CSR) in PEM format, generated by the service.
	CertificateSigningRequest *string

	// READ-ONLY; The status of the external issuer certificate lifecycle.
	Status *CertificateAuthorityStatus

	// READ-ONLY; Thumbprint of the Certificate Authority certificate signed by the external issuer.
	Thumbprint *string
}

// GetCertificateAuthorityIssuer implements the CertificateAuthorityIssuerClassification interface for type ExternalCertificateAuthorityIssuer.
func (e *ExternalCertificateAuthorityIssuer) GetCertificateAuthorityIssuer() *CertificateAuthorityIssuer {
	return &CertificateAuthorityIssuer{
		IssuerType: e.IssuerType,
	}
}

// HealthState - Represents the health state of a resource.
type HealthState struct {
	// READ-ONLY; The timestamp (RFC3339) when the health status last changed.
	LastTransitionTime *string

	// READ-ONLY; The timestamp (RFC3339) when the health status was last updated, even if the status did not change.
	LastUpdateTime *string

	// READ-ONLY; A human-readable message describing the last transition.
	Message *string

	// READ-ONLY; Unique, CamelCase reason code describing the cause of the last health state transition.
	ReasonCode *string

	// READ-ONLY; The high-level health status of the resource.
	Status *HealthStatus
}
...
// InboundCallerIdentity - The identity a linked resource uses for inbound calls to the ADR namespace.
...
// IntermediateCertificateAuthorityProperties - Properties for an intermediate Certificate Authority signed by another same-namespace
// Certificate Authority.
type IntermediateCertificateAuthorityProperties struct {
	// CONSTANT; The intermediate Certificate Authority type.
	// Field has constant value CertificateAuthorityTypeICA, any specified value is ignored.
	CertificateAuthorityType *CertificateAuthorityType

	// REQUIRED; The issuer for the intermediate Certificate Authority.
	Issuer CertificateAuthorityIssuerClassification
...
// GetCertificateAuthorityProperties implements the CertificateAuthorityPropertiesClassification interface for type IntermediateCertificateAuthorityProperties.
func (i *IntermediateCertificateAuthorityProperties) GetCertificateAuthorityProperties() *CertificateAuthorityProperties {
	return &CertificateAuthorityProperties{
		CertificateAuthorityType: i.CertificateAuthorityType,
		KeyType:                  i.KeyType,
		ProvisioningState:        i.ProvisioningState,
		Subject:                  i.Subject,
		UUID:                     i.UUID,
		ValidityNotAfter:         i.ValidityNotAfter,
		ValidityNotBefore:        i.ValidityNotBefore,
	}
}
...
// Management - The namespace management endpoints model.
type Management struct {
	// Dictionary of management endpoints.
	Endpoints map[string]*ManagementEndpoint
}
...
// ManagementEndpoint - Namespace management endpoint model used by service to connect to device.
type ManagementEndpoint struct {
	// REQUIRED; The endpoint address to connect to.
	Address *string

	// REQUIRED; Type of connection used for management endpoint.
	EndpointType *string

	// REQUIRED; The messaging endpoint Azure resource Id.
	ResourceID *string

	// REQUIRED; The scope ID for the management endpoint.
	ScopeID *string
}
...
type MessagingEndpoint struct {
	...
	// READ-ONLY; The endpoint address used by devices.
	DeviceAddress *string

	// READ-ONLY; The error detail surfaced when this messaging endpoint is in the Failed linking state.
	LinkingError *NamespaceLinkingError

	// READ-ONLY; The linking state of this messaging endpoint.
	LinkingState *NamespaceLinkingStateValue
}

// MessagingEndpointProvisioning - The provisioning configuration for a messaging endpoint.
type MessagingEndpointProvisioning struct {
...
// MicrosoftCertificateAuthorityIssuer - A Microsoft issuer for an intermediate Certificate Authority.
type MicrosoftCertificateAuthorityIssuer struct {
	// REQUIRED; The resource ID of the root issuer Certificate Authority in the same namespace.
	CertificateAuthorityResourceID *string

	// CONSTANT; The Microsoft issuer type.
	// Field has constant value CertificateAuthorityIssuerTypeMicrosoft, any specified value is ignored.
	IssuerType *CertificateAuthorityIssuerType
}

// GetCertificateAuthorityIssuer implements the CertificateAuthorityIssuerClassification interface for type MicrosoftCertificateAuthorityIssuer.
func (m *MicrosoftCertificateAuthorityIssuer) GetCertificateAuthorityIssuer() *CertificateAuthorityIssuer {
	return &CertificateAuthorityIssuer{
...
// NamespaceAssetExecuteActionRequest - The request body for the executeAction operation on NamespaceAsset.
type NamespaceAssetExecuteActionRequest struct {
	// REQUIRED; Name of the management action to be executed.
	ManagementActionName *string

	// REQUIRED; Name of the management group under which the action is to be executed.
...
type NamespaceAssetStatus struct {
	...
	// READ-ONLY; The details about the runtime health state of the asset.
	HealthState *HealthState
...
// NamespaceLinkingError - Error detail for an endpoint whose linking state is Failed. Present only while the linking state
// is Failed and removed when the endpoint recovers.
type NamespaceLinkingError struct {
	// READ-ONLY; Error code that classifies the linking failure.
	Code *string

	// READ-ONLY; Human-readable message describing the linking failure.
	Message *string
}
...
type NamespaceProperties struct {
	...
	// The identity used for outbound calls from the ADR namespace. If not specified and the namespace has a system-assigned identity
	// enabled, the system-assigned identity is used by default.
...
// NamespaceProvisioning - The provisioning endpoints associated with this namespace.
type NamespaceProvisioning struct {
	// Dictionary of provisioning endpoints.
	Endpoints map[string]*ProvisioningEndpoint
}
...
// OptionalPropertiesCertificatePolicyConfiguration - The template for adding optional properties.
type OptionalPropertiesCertificatePolicyConfiguration struct {
...
// OutboundIdentity - The identity the ADR namespace uses for outbound calls. If this property is null and the namespace has
// a system-assigned identity enabled, the namespace defaults to using the system-assigned identity for outbound calls.
...
// ProvisioningEndpoint - A provisioning service endpoint linked to this namespace.
type ProvisioningEndpoint struct {
	// REQUIRED; Type of provisioning resource.
	EndpointType *ProvisioningEndpointType

	// REQUIRED; The identity the linked resource uses to call the ADR namespace.
	InboundCallerIdentity *InboundCallerIdentity

	// REQUIRED; The provisioning endpoint Azure resource Id.
	ResourceID *string

	// READ-ONLY; The error detail surfaced when this provisioning endpoint is in the Failed linking state.
	LinkingError *NamespaceLinkingError

	// READ-ONLY; The linking state of this provisioning endpoint.
	LinkingState *NamespaceLinkingStateValue
}

// RegistryDevice - A Registry Device resource is a tracked resource with a provisioning lifecycle. The different capabilities
// of the device are defined through child Device Capability resources.
...
// RegistryDeviceListResult - The response of a RegistryDevice list operation.
type RegistryDeviceListResult struct {
	// REQUIRED; The RegistryDevice items on this page
...
// RegistryDeviceProperties - The properties of a registry device.
type RegistryDeviceProperties struct {
	// REQUIRED; Whether the device is enabled or disabled. A disabled device cannot send messages, be updated, or reprovision.
...
// RegistryDeviceUpdate - The type used for update operations of the RegistryDevice.
type RegistryDeviceUpdate struct {
	// The resource-specific properties for this resource.
...
// RegistryDeviceUpdateProperties - The updatable properties of the RegistryDevice.
type RegistryDeviceUpdateProperties struct {
...
// RootCertificateAuthorityProperties - Properties for a self-signed, service-managed root Certificate Authority.
type RootCertificateAuthorityProperties struct {
	// CONSTANT; The self-signed, service-managed root Certificate Authority type.
	// Field has constant value CertificateAuthorityTypeRoot, any specified value is ignored.
	CertificateAuthorityType *CertificateAuthorityType
...
// GetCertificateAuthorityProperties implements the CertificateAuthorityPropertiesClassification interface for type RootCertificateAuthorityProperties.
func (r *RootCertificateAuthorityProperties) GetCertificateAuthorityProperties() *CertificateAuthorityProperties {
	return &CertificateAuthorityProperties{
		CertificateAuthorityType: r.CertificateAuthorityType,
		KeyType:                  r.KeyType,
		ProvisioningState:        r.ProvisioningState,
		Subject:                  r.Subject,
		UUID:                     r.UUID,
		ValidityNotAfter:         r.ValidityNotAfter,
		ValidityNotBefore:        r.ValidityNotBefore,
	}
}
...
type SchemaRegistryProperties struct {
	...
	// The identity used for outbound calls from the ADR schema registry. If not specified and the schema registry has a system-assigned
	// identity enabled, the system-assigned identity is used by default.
	OutboundIdentity *OutboundIdentity
