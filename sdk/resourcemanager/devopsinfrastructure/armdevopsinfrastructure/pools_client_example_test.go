...
func ExamplePoolsClient_CheckNameAvailability() {
	...
	res, err := clientFactory.NewPoolsClient().CheckNameAvailability(ctx, "2025-09-20", armdevopsinfrastructure.CheckNameAvailability{
...
func ExamplePoolsClient_BeginCreateOrUpdate() {
	...
	poller, err := clientFactory.NewPoolsClient().BeginCreateOrUpdate(ctx, "2025-09-20", "rg", "pool", armdevopsinfrastructure.Pool{
...
func ExamplePoolsClient_BeginDelete() {
	...
	poller, err := clientFactory.NewPoolsClient().BeginDelete(ctx, "2025-09-20", "rg", "pool", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
...
func ExamplePoolsClient_DeleteResources() {
	...
	res, err := clientFactory.NewPoolsClient().DeleteResources(ctx, "2025-09-20", "my-resource-group", "my-dev-ops-pool", armdevopsinfrastructure.DeleteResourcesDetails{
		ResourceIDs: []*string{
...
func ExamplePoolsClient_Get() {
	...
	res, err := clientFactory.NewPoolsClient().Get(ctx, "2025-09-20", "rg", "pool", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	// You could use response here. We use blank identifier for just demo purposes.
...
func ExamplePoolsClient_NewListByResourceGroupPager() {
	...
	pager := clientFactory.NewPoolsClient().NewListByResourceGroupPager("2025-09-20", "rg", nil)
	for pager.More() {
...
func ExamplePoolsClient_NewListBySubscriptionPager() {
	...
	pager := clientFactory.NewPoolsClient().NewListBySubscriptionPager("2025-09-20", nil)
	for pager.More() {
		page, err := pager.NextPage(ctx)
		if err != nil {
			log.Fatalf("failed to advance page: %v", err)
...
func ExamplePoolsClient_BeginUpdate() {
	...
	poller, err := clientFactory.NewPoolsClient().BeginUpdate(ctx, "2025-09-20", "rg", "pool", armdevopsinfrastructure.PoolUpdate{}, nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	res, err := poller.PollUntilDone(ctx, nil)
