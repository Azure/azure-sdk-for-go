...
func (d *DnssecConfigsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if dnssecConfigsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = dnssecConfigsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "DnssecConfigsClient.NewListByDNSZonePager":
				res.resp, res.err = d.dispatchNewListByDNSZonePager(req)
			default:
...
func (d *DnssecConfigsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/dnsZones/(?P<zoneName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/dnssecConfigs/default`
...
func (d *DnssecConfigsServerTransport) dispatchNewListByDNSZonePager(req *http.Request) (*http.Response, error) {
	...
	if newListByDNSZonePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/dnsZones/(?P<zoneName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/dnssecConfigs`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to DnssecConfigsServerTransport
var dnssecConfigsServerTransportInterceptor interface {
