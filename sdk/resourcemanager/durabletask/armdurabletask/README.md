...
clientFactory, err := armdurabletask.NewClientFactory(<subscription ID>, cred, &options)
```
...
```go
client := clientFactory.NewRetentionPoliciesClient()
```
...
[open an issue](https://github.com/Azure/azure-sdk-for-go/issues) and assign the `Durabletask` label.
