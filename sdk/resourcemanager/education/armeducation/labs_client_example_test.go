...
func ExampleLabsClient_CreateOrUpdate() {
	...
	res, err := clientFactory.NewLabsClient().CreateOrUpdate(ctx, "{billingAccountName}", "{billingProfileName}", "{invoiceSectionName}", armeducation.LabDetails{
		Properties: &armeducation.LabProperties{
			...
			ExpirationDate: to.Ptr(time.Date(2021, time.December, 9, 22, 11, 29, 422000000, time.UTC)),
		},
	}, nil)
	if err != nil {
	...
	// 			},
	// 			DisplayName: to.Ptr("example lab"),
	// 			EffectiveDate: to.Ptr(time.Date(2021, time.November, 9, 21, 25, 56, 838000000, time.UTC)),
	// 			ExpirationDate: to.Ptr(time.Date(2021, time.December, 9, 21, 25, 56, 838000000, time.UTC)),
