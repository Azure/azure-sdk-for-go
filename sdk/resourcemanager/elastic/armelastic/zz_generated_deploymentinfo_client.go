...
func NewDeploymentInfoClient(subscriptionID string, credential azcore.TokenCredential, options *arm.ClientOptions) *DeploymentInfoClient {
	...
	return &DeploymentInfoClient{subscriptionID: subscriptionID, ep: string(cp.Host), pl: armruntime.NewPipeline(module, version, credential, &cp)}
}
...
func (client *DeploymentInfoClient) List(ctx context.Context, resourceGroupName string, monitorName string, options *DeploymentInfoListOptions) (DeploymentInfoListResponse, error) {
	req, err := client.listCreateRequest(ctx, resourceGroupName, monitorName, options)
	if err != nil {
		return DeploymentInfoListResponse{}, err
	}
	...
	if !runtime.HasStatusCode(resp, http.StatusOK) {
		return DeploymentInfoListResponse{}, client.listHandleError(resp)
	}
	return client.listHandleResponse(resp)
}

// listCreateRequest creates the List request.
func (client *DeploymentInfoClient) listCreateRequest(ctx context.Context, resourceGroupName string, monitorName string, options *DeploymentInfoListOptions) (*policy.Request, error) {
...
func (client *DeploymentInfoClient) listHandleResponse(resp *http.Response) (DeploymentInfoListResponse, error) {
	result := DeploymentInfoListResponse{RawResponse: resp}
	if err := runtime.UnmarshalAsJSON(resp, &result.DeploymentInfoResponse); err != nil {
		return DeploymentInfoListResponse{}, runtime.NewResponseError(err, resp)
	}
	return result, nil
}

// listHandleError handles the List error response.
func (client *DeploymentInfoClient) listHandleError(resp *http.Response) error {
