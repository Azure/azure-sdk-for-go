...
// The returned SKUsServerTransport instance is connected to an instance of armelasticsan.SKUsClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewSKUsServerTransport(srv *SKUsServer) *SKUsServerTransport {
	return &SKUsServerTransport{
		srv:          srv,
		newListPager: newTracker[azfake.PagerResponder[armelasticsan.SKUsClientListResponse]](),
	}
}

// SKUsServerTransport connects instances of armelasticsan.SKUsClient to instances of SKUsServer.
// Don't use this type directly, use NewSKUsServerTransport instead.
type SKUsServerTransport struct {
	srv          *SKUsServer
	newListPager *tracker[azfake.PagerResponder[armelasticsan.SKUsClientListResponse]]
}
...
func (s *SKUsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.ElasticSan/skus`
		...
		var options *armelasticsan.SKUsClientListOptions
		if filterParam != nil {
			options = &armelasticsan.SKUsClientListOptions{
				Filter: filterParam,
			}
		}
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armelasticsan.SKUsClientListResponse, createLink func() string) {
