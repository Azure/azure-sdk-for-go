...
func ExampleDomainEventSubscriptionsClient_NewGetDeliveryAttributesPager() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
	...
	pager := clientFactory.NewDomainEventSubscriptionsClient().NewGetDeliveryAttributesPager("examplerg", "exampleDomain1", "examplesubscription1", nil)
	for pager.More() {
		...
		// page = armeventgrid.DomainEventSubscriptionsClientGetDeliveryAttributesResponse{
