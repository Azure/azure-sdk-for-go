...
func (c *ChannelsServerTransport) dispatchGetFullURL(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.EventGrid/partnerNamespaces/(?P<partnerNamespaceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/channels/(?P<channelName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/getFullUrl`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 5 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (c *ChannelsServerTransport) dispatchNewListByPartnerNamespacePager(req *http.Request) (*http.Response, error) {
	...
	if newListByPartnerNamespacePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.EventGrid/partnerNamespaces/(?P<partnerNamespaceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/channels`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
