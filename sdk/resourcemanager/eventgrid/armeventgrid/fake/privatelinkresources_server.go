...
// The returned PrivateLinkResourcesServerTransport instance is connected to an instance of armeventgrid.PrivateLinkResourcesClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewPrivateLinkResourcesServerTransport(srv *PrivateLinkResourcesServer) *PrivateLinkResourcesServerTransport {
	return &PrivateLinkResourcesServerTransport{
		srv:                    srv,
		newListByResourcePager: newTracker[azfake.PagerResponder[armeventgrid.PrivateLinkResourcesClientListByResourceResponse]](),
	}
}

// PrivateLinkResourcesServerTransport connects instances of armeventgrid.PrivateLinkResourcesClient to instances of PrivateLinkResourcesServer.
// Don't use this type directly, use NewPrivateLinkResourcesServerTransport instead.
type PrivateLinkResourcesServerTransport struct {
	srv                    *PrivateLinkResourcesServer
	newListByResourcePager *tracker[azfake.PagerResponder[armeventgrid.PrivateLinkResourcesClientListByResourceResponse]]
}
...
func (p *PrivateLinkResourcesServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.EventGrid/(?P<parentType>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/(?P<parentName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/privateLinkResources/(?P<privateLinkResourceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
	...
	parentTypeParam, err := url.PathUnescape(matches[regex.SubexpIndex("parentType")])
	if err != nil {
		return nil, err
	}
	parentNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("parentName")])
	if err != nil {
		return nil, err
	}
	...
	respr, errRespr := p.srv.Get(req.Context(), resourceGroupNameParam, parentTypeParam, parentNameParam, privateLinkResourceNameParam, nil)
...
func (p *PrivateLinkResourcesServerTransport) dispatchNewListByResourcePager(req *http.Request) (*http.Response, error) {
	...
	if newListByResourcePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.EventGrid/(?P<parentType>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/(?P<parentName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/privateLinkResources`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		var options *armeventgrid.PrivateLinkResourcesClientListByResourceOptions
		if filterParam != nil || topParam != nil {
			options = &armeventgrid.PrivateLinkResourcesClientListByResourceOptions{
				Filter: filterParam,
				Top:    topParam,
			}
		}
		resp := p.srv.NewListByResourcePager(resourceGroupNameParam, parentTypeParam, parentNameParam, options)
		newListByResourcePager = &resp
		p.newListByResourcePager.add(req, newListByResourcePager)
		server.PagerResponderInjectNextLinks(newListByResourcePager, req, func(page *armeventgrid.PrivateLinkResourcesClientListByResourceResponse, createLink func() string) {
