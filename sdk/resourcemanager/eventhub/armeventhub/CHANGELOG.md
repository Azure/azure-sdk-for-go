...
- New field `PlatformCapabilities` in struct `ClusterProperties`
- New field `PlatformCapabilities` in struct `EHNamespaceProperties`
- New field `SystemData` in struct `NetworkSecurityPerimeterConfiguration`
- New anonymous field `NetworkSecurityPerimeterConfiguration` in struct `NetworkSecurityPerimeterConfigurationsClientCreateOrUpdateResponse`
