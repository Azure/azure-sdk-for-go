...
type CustomLocationFindTargetResourceGroupResult struct {
	...
	// labels and had lowest priority. This is the rule responsible for mapping the target resource to the target resource group.
	...
	// matching resource sync rule against the selector property of the resource sync rule. The one with highest priority will
...
// CustomLocationOperationsList - Collection of CustomLocationOperation items
type CustomLocationOperationsList struct {
	// REQUIRED; The CustomLocationOperation items
...
// to find out matching resource sync rules using the selector property of the resource sync rule. If this resource sync rule
// has highest priority among all matching rules, then the unmapped custom resource will be projected to the target resource
// group associated with this resource sync rule.
type ResourceSyncRuleProperties struct {
	...
	// is one of the matching rules with highest priority, then the unmapped custom resource will be projected to the target resource
	// group associated with this resource sync rule. The user creating this resource sync rule should have write permissions
...
// part, matchLabels is a map of {key,value} pairs. A single {key,value} in the matchLabels map is equivalent to an element
// of matchExpressions, whose key field is 'key', the operator is 'In', and the values array contains only 'value'. The second
// part, matchExpressions is a list of resource selector requirements. Valid operators include In, NotIn, Exists, and DoesNotExist.
...
// All of the requirements, from both matchLabels and matchExpressions must all be satisfied in order to match.
