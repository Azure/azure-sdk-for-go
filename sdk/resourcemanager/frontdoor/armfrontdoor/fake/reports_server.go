...
func (r *ReportsServerTransport) dispatchGetLatencyScorecards(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/NetworkExperimentProfiles/(?P<profileName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/Experiments/(?P<experimentName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/latencyScorecard`
...
func (r *ReportsServerTransport) dispatchGetTimeseries(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/NetworkExperimentProfiles/(?P<profileName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/Experiments/(?P<experimentName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/timeseries`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 5 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
