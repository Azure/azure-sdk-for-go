...
func ExampleFrontendEndpointsClient_BeginDisableHTTPS() {
	...
	// res = armfrontdoor.FrontendEndpointsClientDisableHTTPSResponse{
	// }
}
...
func ExampleFrontendEndpointsClient_BeginEnableHTTPS() {
	...
	poller, err := clientFactory.NewFrontendEndpointsClient().BeginEnableHTTPS(ctx, "rg1", "frontDoor1", "frontendEndpoint1", armfrontdoor.CustomHTTPSConfiguration{
		CertificateSource: to.Ptr(armfrontdoor.CertificateSourceAzureKeyVault),
		...
		MinimumTLSVersion: to.Ptr(armfrontdoor.MinimumTLSVersion10),
		ProtocolType:      to.Ptr(armfrontdoor.TLSProtocolTypeServerNameIndication),
	}, nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	...
	_ = res
	// If the HTTP response code is 200 as defined in example definition, your response structure would look as follows. Please pay attention that all the values in the output are fake values for just demo purposes.
	// res = armfrontdoor.FrontendEndpointsClientEnableHTTPSResponse{
	// }
}
...
func ExampleFrontendEndpointsClient_NewListByFrontDoorPager() {
	...
	for pager.More() {
		...
		// page = armfrontdoor.FrontendEndpointsClientListByFrontDoorResponse{
		// 	FrontendEndpointsListResult: armfrontdoor.FrontendEndpointsListResult{
