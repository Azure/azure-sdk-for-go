...
type ConnectorsServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, connectorName string, properties armimpactreporting.ConnectorUpdate, options *armimpactreporting.ConnectorsClientUpdateOptions) (resp azfake.Responder[armimpactreporting.ConnectorsClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (c *ConnectorsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Impact/connectors/(?P<connectorName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (c *ConnectorsServerTransport) dispatchNewListBySubscriptionPager(req *http.Request) (*http.Response, error) {
	...
	if newListBySubscriptionPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Impact/connectors`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (c *ConnectorsServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armimpactreporting.ConnectorUpdate](req)
	if err != nil {
		return nil, err
	}
	connectorNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("connectorName")])
