...
func (client *SummariesClient) Get(ctx context.Context, resourceGroupName string, workspaceName string, firmwareID string, summaryType SummaryType, options *SummariesClientGetOptions) (SummariesClientGetResponse, error) {
	var err error
	const operationName = "SummariesClient.Get"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.getCreateRequest(ctx, resourceGroupName, workspaceName, firmwareID, summaryType, options)
	if err != nil {
...
func (client *SummariesClient) getCreateRequest(ctx context.Context, resourceGroupName string, workspaceName string, firmwareID string, summaryType SummaryType, _ *SummariesClientGetOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.IoTFirmwareDefense/workspaces/{workspaceName}/firmwares/{firmwareId}/summaries/{summaryType}"
	...
	if summaryType == "" {
		return nil, errors.New("parameter summaryType cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{summaryType}", url.PathEscape(string(summaryType)))
...
func (client *SummariesClient) getHandleResponse(resp *http.Response, successCodes ...int) (SummariesClientGetResponse, error) {
...
func (client *SummariesClient) listByFirmwareCreateRequest(ctx context.Context, resourceGroupName string, workspaceName string, firmwareID string, nextLink string, _ *SummariesClientListByFirmwareOptions) (*policy.Request, error) {
	firstPage := nextLink == ""
...
func (client *SummariesClient) listByFirmwareHandleResponse(resp *http.Response, successCodes ...int) (SummariesClientListByFirmwareResponse, error) {
	result := SummariesClientListByFirmwareResponse{}
	if !runtime.HasStatusCode(resp, successCodes...) {
		return result, runtime.NewResponseError(resp)
	}
	if err := runtime.UnmarshalAsJSON(resp, &result.SummaryResourceListResult); err != nil {
		return SummariesClientListByFirmwareResponse{}, err
	}
	return result, nil
}
