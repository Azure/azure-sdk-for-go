...
type ClustersServer struct {
	...
	// ListLanguageExtensions is the fake for method ClustersClient.ListLanguageExtensions
	// HTTP status codes to indicate success: http.StatusOK
	ListLanguageExtensions func(ctx context.Context, resourceGroupName string, clusterName string, options *armkusto.ClustersClientListLanguageExtensionsOptions) (resp azfake.Responder[armkusto.ClustersClientListLanguageExtensionsResponse], errResp azfake.ErrorResponder)
...
func (c *ClustersServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "ClustersClient.ListLanguageExtensions":
				res.resp, res.err = c.dispatchListLanguageExtensions(req)
...
func (c *ClustersServerTransport) dispatchListLanguageExtensions(req *http.Request) (*http.Response, error) {
	if c.srv.ListLanguageExtensions == nil {
		return nil, &nonRetriableError{errors.New("fake for method ListLanguageExtensions not implemented")}
	}
	...
	respr, errRespr := c.srv.ListLanguageExtensions(req.Context(), resourceGroupNameParam, clusterNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).LanguageExtensionsList, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
