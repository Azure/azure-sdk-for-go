...
// The returned UsagesServerTransport instance is connected to an instance of armlabservices.UsagesClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewUsagesServerTransport(srv *UsagesServer) *UsagesServerTransport {
	return &UsagesServerTransport{
		srv:                    srv,
		newListByLocationPager: newTracker[azfake.PagerResponder[armlabservices.UsagesClientListByLocationResponse]](),
	}
}

// UsagesServerTransport connects instances of armlabservices.UsagesClient to instances of UsagesServer.
// Don't use this type directly, use NewUsagesServerTransport instead.
type UsagesServerTransport struct {
	srv                    *UsagesServer
	newListByLocationPager *tracker[azfake.PagerResponder[armlabservices.UsagesClientListByLocationResponse]]
}
...
func (u *UsagesServerTransport) dispatchNewListByLocationPager(req *http.Request) (*http.Response, error) {
	...
	if newListByLocationPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.LabServices/locations/(?P<location>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/usages`
		...
		var options *armlabservices.UsagesClientListByLocationOptions
		if filterParam != nil {
			options = &armlabservices.UsagesClientListByLocationOptions{
				Filter: filterParam,
			}
		}
		resp := u.srv.NewListByLocationPager(locationParam, options)
		newListByLocationPager = &resp
		u.newListByLocationPager.add(req, newListByLocationPager)
		server.PagerResponderInjectNextLinks(newListByLocationPager, req, func(page *armlabservices.UsagesClientListByLocationResponse, createLink func() string) {
