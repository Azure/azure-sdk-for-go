...
func ExamplePrivateLinkResourcesForSCCPowershellClient_ListByService() {
	...
	res, err := clientFactory.NewPrivateLinkResourcesForSCCPowershellClient().ListByService(ctx, "rgname", "service1", nil)
	if err != nil {
...
func ExamplePrivateLinkResourcesForSCCPowershellClient_Get() {
	...
	res, err := clientFactory.NewPrivateLinkResourcesForSCCPowershellClient().Get(ctx, "rgname", "service1", "fhir", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	// You could use response here. We use blank identifier for just demo purposes.
