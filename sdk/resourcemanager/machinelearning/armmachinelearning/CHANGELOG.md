...
- Struct `IdleShutdownSetting` has been removed
...
- New field `EnableSSO`, `IdleTimeBeforeShutdown` in struct `ComputeInstanceProperties`
...
- New field `EnableServiceSideCMKEncryption`, `ProvisionNetworkNow`, `SystemDatastoresAuthMode` in struct `WorkspaceProperties`
- New field `SystemDatastoresAuthMode` in struct `WorkspacePropertiesUpdateParameters`
