...
type ManagedNetworkSettingsServer struct {
	...
	BeginPatch func(ctx context.Context, resourceGroupName string, workspaceName string, managedNetworkName string, body armmachinelearning.ManagedNetworkSettingsPropertiesBasicResource, options *armmachinelearning.ManagedNetworkSettingsClientBeginPatchOptions) (resp azfake.PollerResponder[armmachinelearning.ManagedNetworkSettingsClientPatchResponse], errResp azfake.ErrorResponder)
...
func (m *ManagedNetworkSettingsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.MachineLearningServices/workspaces/(?P<workspaceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/managedNetworks/(?P<managedNetworkName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (m *ManagedNetworkSettingsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.MachineLearningServices/workspaces/(?P<workspaceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/managedNetworks`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (m *ManagedNetworkSettingsServerTransport) dispatchBeginPatch(req *http.Request) (*http.Response, error) {
	...
	if beginPatch == nil {
		...
		respr, errRespr := m.srv.BeginPatch(req.Context(), resourceGroupNameParam, workspaceNameParam, managedNetworkNameParam, body, nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		beginPatch = &respr
