...
// QuotasServer is a fake server for instances of the armmachinelearningservices.QuotasClient type.
type QuotasServer struct {
	// NewListPager is the fake for method QuotasClient.NewListPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListPager	func(location string, options *armmachinelearningservices.QuotasClientListOptions) (resp azfake.PagerResponder[armmachinelearningservices.QuotasClientListResponse])

	// Update is the fake for method QuotasClient.Update
	// HTTP status codes to indicate success: http.StatusOK
	Update	func(ctx context.Context, location string, parameters armmachinelearningservices.QuotaUpdateParameters, options *armmachinelearningservices.QuotasClientUpdateOptions) (resp azfake.Responder[armmachinelearningservices.QuotasClientUpdateResponse], errResp azfake.ErrorResponder)
}

// NewQuotasServerTransport creates a new instance of QuotasServerTransport with the provided implementation.
// The returned QuotasServerTransport instance is connected to an instance of armmachinelearningservices.QuotasClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewQuotasServerTransport(srv *QuotasServer) *QuotasServerTransport {
	return &QuotasServerTransport{
		srv:		srv,
		newListPager:	newTracker[azfake.PagerResponder[armmachinelearningservices.QuotasClientListResponse]](),
	}
}

// QuotasServerTransport connects instances of armmachinelearningservices.QuotasClient to instances of QuotasServer.
// Don't use this type directly, use NewQuotasServerTransport instead.
type QuotasServerTransport struct {
	srv		*QuotasServer
	newListPager	*tracker[azfake.PagerResponder[armmachinelearningservices.QuotasClientListResponse]]
}
...
func (q *QuotasServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armmachinelearningservices.QuotasClientListResponse, createLink func() string) {
...
func (q *QuotasServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armmachinelearningservices.QuotaUpdateParameters](req)
	if err != nil {
		return nil, err
	}
	locationParam, err := url.PathUnescape(matches[regex.SubexpIndex("location")])
