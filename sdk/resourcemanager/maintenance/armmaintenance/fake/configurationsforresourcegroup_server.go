...
func (c *ConfigurationsForResourceGroupServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if configurationsForResourceGroupServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = configurationsForResourceGroupServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (c *ConfigurationsForResourceGroupServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Maintenance/maintenanceConfigurations`
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armmaintenance.ConfigurationsForResourceGroupClientListResponse, createLink func() string) {
...
// set this to conditionally intercept incoming requests to ConfigurationsForResourceGroupServerTransport
var configurationsForResourceGroupServerTransportInterceptor interface {
