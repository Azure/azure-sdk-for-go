...
func ExampleApplicationsClient_NewListAllowedUpgradePlansPager() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
	...
	pager := clientFactory.NewApplicationsClient().NewListAllowedUpgradePlansPager("rg", "myManagedApplication", nil)
	for pager.More() {
		...
		// page = armmanagedapplications.ApplicationsClientListAllowedUpgradePlansResponse{
...
func ExampleApplicationsClient_NewListTokensPager() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
	...
	pager := clientFactory.NewApplicationsClient().NewListTokensPager("rg", "myManagedApplication", armmanagedapplications.ListTokenRequest{
	...
	for pager.More() {
		...
		// If the HTTP response code is 200 as defined in example definition, your page structure would look as follows. Please pay attention that all the values in the output are fake values for just demo purposes.
		// page = armmanagedapplications.ApplicationsClientListTokensResponse{
