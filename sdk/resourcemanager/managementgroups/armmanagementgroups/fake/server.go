	...
	go func() {
		...
		if !intercepted {
			...
			case "Client.NewGetDescendantsPager":
				res.resp, res.err = s.dispatchNewGetDescendantsPager(req)
...
func (s *ServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/providers/Microsoft\.Management/managementGroups/(?P<groupId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (s *ServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	expandParam := getOptional(armmanagementgroups.ManagementGroupExpandType(qp.Get("$expand")))
	recurseParam, err := parseOptional(qp.Get("$recurse"), strconv.ParseBool)
	if err != nil {
		return nil, err
	}
...
func (s *ServerTransport) dispatchNewGetDescendantsPager(req *http.Request) (*http.Response, error) {
	...
	if newGetDescendantsPager == nil {
		const regexStr = `/providers/Microsoft\.Management/managementGroups/(?P<groupId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/descendants`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
