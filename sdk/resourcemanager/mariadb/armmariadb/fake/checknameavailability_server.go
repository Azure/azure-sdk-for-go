...
// The returned CheckNameAvailabilityServerTransport instance is connected to an instance of armmariadb.CheckNameAvailabilityClient via the
...
// CheckNameAvailabilityServerTransport connects instances of armmariadb.CheckNameAvailabilityClient to instances of CheckNameAvailabilityServer.
...
func (c *CheckNameAvailabilityServerTransport) dispatchExecute(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/checkNameAvailability`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 1 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armmariadb.NameAvailabilityRequest](req)
	if err != nil {
		return nil, err
	}
