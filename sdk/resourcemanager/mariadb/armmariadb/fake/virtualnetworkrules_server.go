...
// The returned VirtualNetworkRulesServerTransport instance is connected to an instance of armmariadb.VirtualNetworkRulesClient via the
...
func NewVirtualNetworkRulesServerTransport(srv *VirtualNetworkRulesServer) *VirtualNetworkRulesServerTransport {
	return &VirtualNetworkRulesServerTransport{
		srv:                  srv,
		beginCreateOrUpdate:  newTracker[azfake.PollerResponder[armmariadb.VirtualNetworkRulesClientCreateOrUpdateResponse]](),
		beginDelete:          newTracker[azfake.PollerResponder[armmariadb.VirtualNetworkRulesClientDeleteResponse]](),
		newListByServerPager: newTracker[azfake.PagerResponder[armmariadb.VirtualNetworkRulesClientListByServerResponse]](),
	}
}

// VirtualNetworkRulesServerTransport connects instances of armmariadb.VirtualNetworkRulesClient to instances of VirtualNetworkRulesServer.
...
type VirtualNetworkRulesServerTransport struct {
	...
	beginCreateOrUpdate  *tracker[azfake.PollerResponder[armmariadb.VirtualNetworkRulesClientCreateOrUpdateResponse]]
	beginDelete          *tracker[azfake.PollerResponder[armmariadb.VirtualNetworkRulesClientDeleteResponse]]
	newListByServerPager *tracker[azfake.PagerResponder[armmariadb.VirtualNetworkRulesClientListByServerResponse]]
}
...
func (v *VirtualNetworkRulesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/virtualNetworkRules/(?P<virtualNetworkRuleName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if matches == nil || len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmariadb.VirtualNetworkRule](req)
		if err != nil {
			return nil, err
		}
...
func (v *VirtualNetworkRulesServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/virtualNetworkRules`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armmariadb.VirtualNetworkRulesClientListByServerResponse, createLink func() string) {
