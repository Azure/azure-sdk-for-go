...
// The returned WaitStatisticsServerTransport instance is connected to an instance of armmariadb.WaitStatisticsClient via the
...
func NewWaitStatisticsServerTransport(srv *WaitStatisticsServer) *WaitStatisticsServerTransport {
	return &WaitStatisticsServerTransport{
		srv:                  srv,
		newListByServerPager: newTracker[azfake.PagerResponder[armmariadb.WaitStatisticsClientListByServerResponse]](),
	}
}

// WaitStatisticsServerTransport connects instances of armmariadb.WaitStatisticsClient to instances of WaitStatisticsServer.
...
type WaitStatisticsServerTransport struct {
	...
	newListByServerPager *tracker[azfake.PagerResponder[armmariadb.WaitStatisticsClientListByServerResponse]]
}
...
func (w *WaitStatisticsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/waitStatistics/(?P<waitStatisticsId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
...
func (w *WaitStatisticsServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMariaDB/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/waitStatistics`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if matches == nil || len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmariadb.WaitStatisticsInput](req)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armmariadb.WaitStatisticsClientListByServerResponse, createLink func() string) {
