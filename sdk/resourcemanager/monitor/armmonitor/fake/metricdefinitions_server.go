...
func (m *MetricDefinitionsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if metricDefinitionsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = metricDefinitionsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "MetricDefinitionsClient.NewListAtSubscriptionScopePager":
				res.resp, res.err = m.dispatchNewListAtSubscriptionScopePager(req)
			default:
...
func (m *MetricDefinitionsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armmonitor.MetricDefinitionsClientListResponse, createLink func() string) {
...
func (m *MetricDefinitionsServerTransport) dispatchNewListAtSubscriptionScopePager(req *http.Request) (*http.Response, error) {
	...
	if newListAtSubscriptionScopePager == nil {
		...
		resp := m.srv.NewListAtSubscriptionScopePager(qp.Get("region"), options)
		newListAtSubscriptionScopePager = &resp
		m.newListAtSubscriptionScopePager.add(req, newListAtSubscriptionScopePager)
		server.PagerResponderInjectNextLinks(newListAtSubscriptionScopePager, req, func(page *armmonitor.MetricDefinitionsClientListAtSubscriptionScopeResponse, createLink func() string) {
			page.NextLink = to.Ptr(createLink())
		})
	}
...
// set this to conditionally intercept incoming requests to MetricDefinitionsServerTransport
var metricDefinitionsServerTransportInterceptor interface {
