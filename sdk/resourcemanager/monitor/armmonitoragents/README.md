...
The `armmonitoragents` module provides operations for working with Azure Monitor.

[Source code](https://github.com/Azure/azure-sdk-for-go/tree/main/sdk/resourcemanager/monitor/armmonitoragents)
...
```sh
go get github.com/Azure/azure-sdk-for-go/sdk/resourcemanager/monitor/armmonitoragents
```
...
```go
clientFactory, err := armmonitoragents.NewClientFactory(<subscription ID>, cred, nil)
```
...
clientFactory, err := armmonitoragents.NewClientFactory(<subscription ID>, cred, &options)
```
...
```go
client := clientFactory.NewObservabilityAgentsClient()
```
