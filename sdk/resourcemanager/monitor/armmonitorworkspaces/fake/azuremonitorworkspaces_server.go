...
type AzureMonitorWorkspacesServer struct {
	...
	Get func(ctx context.Context, resourceGroupName string, azureMonitorWorkspaceName string, options *armmonitorworkspaces.AzureMonitorWorkspacesClientGetOptions) (resp azfake.Responder[armmonitorworkspaces.AzureMonitorWorkspacesClientGetResponse], errResp azfake.ErrorResponder)

	// NewListByResourceGroupPager is the fake for method AzureMonitorWorkspacesClient.NewListByResourceGroupPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListByResourceGroupPager func(resourceGroupName string, options *armmonitorworkspaces.AzureMonitorWorkspacesClientListByResourceGroupOptions) (resp azfake.PagerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientListByResourceGroupResponse])

	// NewListBySubscriptionPager is the fake for method AzureMonitorWorkspacesClient.NewListBySubscriptionPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListBySubscriptionPager func(options *armmonitorworkspaces.AzureMonitorWorkspacesClientListBySubscriptionOptions) (resp azfake.PagerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientListBySubscriptionResponse])

	// Update is the fake for method AzureMonitorWorkspacesClient.Update
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, resourceGroupName string, azureMonitorWorkspaceName string, properties armmonitorworkspaces.AzureMonitorWorkspaceResourceUpdate, options *armmonitorworkspaces.AzureMonitorWorkspacesClientUpdateOptions) (resp azfake.Responder[armmonitorworkspaces.AzureMonitorWorkspacesClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
// The returned AzureMonitorWorkspacesServerTransport instance is connected to an instance of armmonitorworkspaces.AzureMonitorWorkspacesClient via the
...
func NewAzureMonitorWorkspacesServerTransport(srv *AzureMonitorWorkspacesServer) *AzureMonitorWorkspacesServerTransport {
	return &AzureMonitorWorkspacesServerTransport{
		srv:                         srv,
		beginDelete:                 newTracker[azfake.PollerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientDeleteResponse]](),
		newListByResourceGroupPager: newTracker[azfake.PagerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientListByResourceGroupResponse]](),
		newListBySubscriptionPager:  newTracker[azfake.PagerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientListBySubscriptionResponse]](),
	}
}

// AzureMonitorWorkspacesServerTransport connects instances of armmonitorworkspaces.AzureMonitorWorkspacesClient to instances of AzureMonitorWorkspacesServer.
...
type AzureMonitorWorkspacesServerTransport struct {
	...
	beginDelete                 *tracker[azfake.PollerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientDeleteResponse]]
	newListByResourceGroupPager *tracker[azfake.PagerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientListByResourceGroupResponse]]
	newListBySubscriptionPager  *tracker[azfake.PagerResponder[armmonitorworkspaces.AzureMonitorWorkspacesClientListBySubscriptionResponse]]
}
...
func (a *AzureMonitorWorkspacesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if azureMonitorWorkspacesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = azureMonitorWorkspacesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (a *AzureMonitorWorkspacesServerTransport) dispatchNewListByResourceGroupPager(req *http.Request) (*http.Response, error) {
	...
	if newListByResourceGroupPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Monitor/accounts`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		server.PagerResponderInjectNextLinks(newListByResourceGroupPager, req, func(page *armmonitorworkspaces.AzureMonitorWorkspacesClientListByResourceGroupResponse, createLink func() string) {
...
func (a *AzureMonitorWorkspacesServerTransport) dispatchNewListBySubscriptionPager(req *http.Request) (*http.Response, error) {
	...
	if newListBySubscriptionPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Monitor/accounts`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		server.PagerResponderInjectNextLinks(newListBySubscriptionPager, req, func(page *armmonitorworkspaces.AzureMonitorWorkspacesClientListBySubscriptionResponse, createLink func() string) {
			page.NextLink = to.Ptr(createLink())
		})
	}
...
func (a *AzureMonitorWorkspacesServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armmonitorworkspaces.AzureMonitorWorkspaceResourceUpdate](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
	...
	respr, errRespr := a.srv.Update(req.Context(), resourceGroupNameParam, azureMonitorWorkspaceNameParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
// set this to conditionally intercept incoming requests to AzureMonitorWorkspacesServerTransport
var azureMonitorWorkspacesServerTransportInterceptor interface {
