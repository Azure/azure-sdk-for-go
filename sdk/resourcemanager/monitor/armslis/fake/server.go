...
// Server is a fake server for instances of the armslis.Client type.
type Server struct {
	// CreateOrUpdate is the fake for method Client.CreateOrUpdate
	// HTTP status codes to indicate success: http.StatusOK, http.StatusCreated
	CreateOrUpdate func(ctx context.Context, serviceGroupName string, sliName string, resource armslis.Sli, options *armslis.ClientCreateOrUpdateOptions) (resp azfake.Responder[armslis.ClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Delete is the fake for method Client.Delete
	// HTTP status codes to indicate success: http.StatusOK, http.StatusNoContent
	Delete func(ctx context.Context, serviceGroupName string, sliName string, options *armslis.ClientDeleteOptions) (resp azfake.Responder[armslis.ClientDeleteResponse], errResp azfake.ErrorResponder)
	...
	// HTTP status codes to indicate success: http.StatusOK
	NewListByParentPager func(serviceGroupName string, options *armslis.ClientListByParentOptions) (resp azfake.PagerResponder[armslis.ClientListByParentResponse])
}

// NewServerTransport creates a new instance of ServerTransport with the provided implementation.
// The returned ServerTransport instance is connected to an instance of armslis.Client via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewServerTransport(srv *Server) *ServerTransport {
	return &ServerTransport{
		srv:                  srv,
		newListByParentPager: newTracker[azfake.PagerResponder[armslis.ClientListByParentResponse]](),
	}
}

// ServerTransport connects instances of armslis.Client to instances of Server.
// Don't use this type directly, use NewServerTransport instead.
type ServerTransport struct {
	srv                  *Server
	newListByParentPager *tracker[azfake.PagerResponder[armslis.ClientListByParentResponse]]
}
...
func (s *ServerTransport) dispatchCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/providers/Microsoft\.Management/serviceGroups/(?P<serviceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Monitor/slis/(?P<sliName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 3 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	body, err := server.UnmarshalRequestAsJSON[armslis.Sli](req)
	if err != nil {
		return nil, err
	}
	...
	sliNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("sliName")])
	if err != nil {
		return nil, err
	}
	respr, errRespr := s.srv.CreateOrUpdate(req.Context(), serviceGroupNameParam, sliNameParam, body, nil)
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).Sli, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
...
func (s *ServerTransport) dispatchDelete(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := s.srv.Delete(req.Context(), serviceGroupNameParam, sliNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
func (s *ServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := s.srv.Get(req.Context(), serviceGroupNameParam, sliNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
func (s *ServerTransport) dispatchNewListByParentPager(req *http.Request) (*http.Response, error) {
	...
	if newListByParentPager == nil {
		const regexStr = `/providers/Microsoft\.Management/serviceGroups/(?P<serviceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Monitor/slis`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		resp := s.srv.NewListByParentPager(serviceGroupNameParam, nil)
		newListByParentPager = &resp
		s.newListByParentPager.add(req, newListByParentPager)
		server.PagerResponderInjectNextLinks(newListByParentPager, req, func(page *armslis.ClientListByParentResponse, createLink func() string) {
