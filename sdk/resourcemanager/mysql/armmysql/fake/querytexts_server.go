...
type QueryTextsServer struct {
	...
	Get func(ctx context.Context, resourceGroupName string, serverName string, queryID string, options *armmysql.QueryTextsClientGetOptions) (resp azfake.Responder[armmysql.QueryTextsClientGetResponse], errResp azfake.ErrorResponder)

	// NewListByServerPager is the fake for method QueryTextsClient.NewListByServerPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListByServerPager func(resourceGroupName string, serverName string, queryIDs []string, options *armmysql.QueryTextsClientListByServerOptions) (resp azfake.PagerResponder[armmysql.QueryTextsClientListByServerResponse])
}

// NewQueryTextsServerTransport creates a new instance of QueryTextsServerTransport with the provided implementation.
// The returned QueryTextsServerTransport instance is connected to an instance of armmysql.QueryTextsClient via the
...
func NewQueryTextsServerTransport(srv *QueryTextsServer) *QueryTextsServerTransport {
	return &QueryTextsServerTransport{
		srv:                  srv,
		newListByServerPager: newTracker[azfake.PagerResponder[armmysql.QueryTextsClientListByServerResponse]](),
	}
}

// QueryTextsServerTransport connects instances of armmysql.QueryTextsClient to instances of QueryTextsServer.
...
type QueryTextsServerTransport struct {
	...
	newListByServerPager *tracker[azfake.PagerResponder[armmysql.QueryTextsClientListByServerResponse]]
}
...
func (q *QueryTextsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMySQL/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/queryTexts/(?P<queryId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
...
func (q *QueryTextsServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DBforMySQL/servers/(?P<serverName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/queryTexts`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armmysql.QueryTextsClientListByServerResponse, createLink func() string) {
