...
// The returned FirewallRulesServerTransport instance is connected to an instance of armmysqlflexibleservers.FirewallRulesClient via the
...
func NewFirewallRulesServerTransport(srv *FirewallRulesServer) *FirewallRulesServerTransport {
	return &FirewallRulesServerTransport{
		srv:                  srv,
		beginCreateOrUpdate:  newTracker[azfake.PollerResponder[armmysqlflexibleservers.FirewallRulesClientCreateOrUpdateResponse]](),
		beginDelete:          newTracker[azfake.PollerResponder[armmysqlflexibleservers.FirewallRulesClientDeleteResponse]](),
		newListByServerPager: newTracker[azfake.PagerResponder[armmysqlflexibleservers.FirewallRulesClientListByServerResponse]](),
	}
}

// FirewallRulesServerTransport connects instances of armmysqlflexibleservers.FirewallRulesClient to instances of FirewallRulesServer.
...
type FirewallRulesServerTransport struct {
	...
	beginCreateOrUpdate  *tracker[azfake.PollerResponder[armmysqlflexibleservers.FirewallRulesClientCreateOrUpdateResponse]]
	beginDelete          *tracker[azfake.PollerResponder[armmysqlflexibleservers.FirewallRulesClientDeleteResponse]]
	newListByServerPager *tracker[azfake.PagerResponder[armmysqlflexibleservers.FirewallRulesClientListByServerResponse]]
}
...
func (f *FirewallRulesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DBforMySQL/flexibleServers/(?P<serverName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/firewallRules/(?P<firewallRuleName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmysqlflexibleservers.FirewallRule](req)
		if err != nil {
			return nil, err
		}
...
func (f *FirewallRulesServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DBforMySQL/flexibleServers/(?P<serverName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/firewallRules`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armmysqlflexibleservers.FirewallRulesClientListByServerResponse, createLink func() string) {
