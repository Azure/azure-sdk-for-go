...
func (client *ServersClient) BeginFailover(ctx context.Context, resourceGroupName string, serverName string, options *ServersClientBeginFailoverOptions) (ServersClientFailoverPollerResponse, error) {
	resp, err := client.failover(ctx, resourceGroupName, serverName, options)
	if err != nil {
		return ServersClientFailoverPollerResponse{}, err
	}
	result := ServersClientFailoverPollerResponse{
		RawResponse: resp,
	}
	pt, err := armruntime.NewPoller("ServersClient.Failover", "", resp, client.pl)
	if err != nil {
		return ServersClientFailoverPollerResponse{}, err
	}
	result.Poller = &ServersClientFailoverPoller{
		pt: pt,
	}
	return result, nil
}
...
func (client *ServersClient) BeginRestart(ctx context.Context, resourceGroupName string, serverName string, parameters ServerRestartParameter, options *ServersClientBeginRestartOptions) (ServersClientRestartPollerResponse, error) {
