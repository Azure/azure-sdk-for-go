...
func ExampleAdminRulesClient_NewListPager() {
	...
	pager := client.NewListPager("rg1",
		...
		"testRuleCollection",
		&armnetwork.AdminRulesClientListOptions{Top: nil,
			SkipToken: nil,
		})
	for pager.More() {
...
func ExampleAdminRulesClient_Get() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
...
func ExampleAdminRulesClient_CreateOrUpdate() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
...
func ExampleAdminRulesClient_BeginDelete() {
	...
	poller, err := client.BeginDelete(ctx,
		...
		"testRuleCollection",
		"SampleAdminRule",
		&armnetwork.AdminRulesClientBeginDeleteOptions{Force: to.Ptr(false)})
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
