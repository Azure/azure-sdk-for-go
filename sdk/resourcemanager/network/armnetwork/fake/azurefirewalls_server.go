...
func (a *AzureFirewallsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		...
		createAfcControlPlaneParam, err := parseOptional(qp.Get("createAfcControlPlane"), strconv.ParseBool)
		if err != nil {
			return nil, err
		}
		var options *armnetwork.AzureFirewallsClientBeginCreateOrUpdateOptions
		if createAfcControlPlaneParam != nil {
			options = &armnetwork.AzureFirewallsClientBeginCreateOrUpdateOptions{
				CreateAfcControlPlane: createAfcControlPlaneParam,
			}
		}
		respr, errRespr := a.srv.BeginCreateOrUpdate(req.Context(), resourceGroupNameParam, azureFirewallNameParam, body, options)
...
func (a *AzureFirewallsServerTransport) dispatchBeginPacketCaptureOperation(req *http.Request) (*http.Response, error) {
	...
	if beginPacketCaptureOperation == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/azureFirewalls/(?P<azureFirewallName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/packetCaptureOperation`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
