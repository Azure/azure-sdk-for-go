...
type IpamPoolsServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, resourceGroupName string, networkManagerName string, poolName string, body armnetwork.IpamPoolUpdate, options *armnetwork.IpamPoolsClientUpdateOptions) (resp azfake.Responder[armnetwork.IpamPoolsClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (i *IpamPoolsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if ipamPoolsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = ipamPoolsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (i *IpamPoolsServerTransport) dispatchBeginCreate(req *http.Request) (*http.Response, error) {
	...
	if beginCreate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/networkManagers/(?P<networkManagerName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/ipamPools/(?P<poolName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
		...
		var options *armnetwork.IpamPoolsClientBeginCreateOptions
		if ifMatchParam != nil {
			options = &armnetwork.IpamPoolsClientBeginCreateOptions{
				IfMatch: ifMatchParam,
			}
		}
		respr, errRespr := i.srv.BeginCreate(req.Context(), resourceGroupNameParam, networkManagerNameParam, poolNameParam, body, options)
...
func (i *IpamPoolsServerTransport) dispatchBeginDelete(req *http.Request) (*http.Response, error) {
	...
	if beginDelete == nil {
		...
		ifMatchParam := getOptional(getHeaderValue(req.Header, "If-Match"))
		var options *armnetwork.IpamPoolsClientBeginDeleteOptions
		if ifMatchParam != nil {
			options = &armnetwork.IpamPoolsClientBeginDeleteOptions{
				IfMatch: ifMatchParam,
			}
		}
		respr, errRespr := i.srv.BeginDelete(req.Context(), resourceGroupNameParam, networkManagerNameParam, poolNameParam, options)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		beginDelete = &respr
...
func (i *IpamPoolsServerTransport) dispatchGetPoolUsage(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/networkManagers/(?P<networkManagerName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/ipamPools/(?P<poolName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/getPoolUsage`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 5 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (i *IpamPoolsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/networkManagers/(?P<networkManagerName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/ipamPools`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (i *IpamPoolsServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := i.srv.Update(req.Context(), resourceGroupNameParam, networkManagerNameParam, poolNameParam, body, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
// set this to conditionally intercept incoming requests to IpamPoolsServerTransport
var ipamPoolsServerTransportInterceptor interface {
