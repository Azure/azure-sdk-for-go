...
func (s *StaticCidrsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if staticCidrsServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = staticCidrsServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (s *StaticCidrsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/networkManagers/(?P<networkManagerName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/ipamPools/(?P<poolName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/staticCidrs`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to StaticCidrsServerTransport
var staticCidrsServerTransportInterceptor interface {
