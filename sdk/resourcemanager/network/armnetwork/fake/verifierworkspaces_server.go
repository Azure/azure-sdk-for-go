...
type VerifierWorkspacesServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, resourceGroupName string, networkManagerName string, workspaceName string, body armnetwork.VerifierWorkspaceUpdate, options *armnetwork.VerifierWorkspacesClientUpdateOptions) (resp azfake.Responder[armnetwork.VerifierWorkspacesClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (v *VerifierWorkspacesServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Network/networkManagers/(?P<networkManagerName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/verifierWorkspaces`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		sortKeyParam := getOptional(qp.Get("sortKey"))
		sortValueParam := getOptional(qp.Get("sortValue"))
...
func (v *VerifierWorkspacesServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := v.srv.Update(req.Context(), resourceGroupNameParam, networkManagerNameParam, workspaceNameParam, body, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
