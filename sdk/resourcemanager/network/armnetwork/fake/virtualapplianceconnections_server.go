...
func (v *VirtualApplianceConnectionsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if v.beginCreateOrUpdate == nil {
		const regexStr = "/subscriptions/(?P<subscriptionId>[a-zA-Z0-9-_]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9-_]+)/providers/Microsoft.Network/networkVirtualAppliances/(?P<networkVirtualApplianceName>[a-zA-Z0-9-_]+)/networkVirtualApplianceConnections/(?P<connectionName>[a-zA-Z0-9-_]+)"
		...
		respr, errRespr := v.srv.BeginCreateOrUpdate(req.Context(), matches[regex.SubexpIndex("resourceGroupName")], matches[regex.SubexpIndex("networkVirtualApplianceName")], matches[regex.SubexpIndex("connectionName")], body, nil)
...
func (v *VirtualApplianceConnectionsServerTransport) dispatchBeginDelete(req *http.Request) (*http.Response, error) {
	...
	if v.beginDelete == nil {
		...
		respr, errRespr := v.srv.BeginDelete(req.Context(), matches[regex.SubexpIndex("resourceGroupName")], matches[regex.SubexpIndex("networkVirtualApplianceName")], matches[regex.SubexpIndex("connectionName")], nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		v.beginDelete = &respr
...
func (v *VirtualApplianceConnectionsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := v.srv.Get(req.Context(), matches[regex.SubexpIndex("resourceGroupName")], matches[regex.SubexpIndex("networkVirtualApplianceName")], matches[regex.SubexpIndex("connectionName")], nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
...
func (v *VirtualApplianceConnectionsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if v.newListPager == nil {
		const regexStr = "/subscriptions/(?P<subscriptionId>[a-zA-Z0-9-_]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9-_]+)/providers/Microsoft.Network/networkVirtualAppliances/(?P<networkVirtualApplianceName>[a-zA-Z0-9-_]+)/networkVirtualApplianceConnections"
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.Path)
		if matches == nil || len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		resp := v.srv.NewListPager(matches[regex.SubexpIndex("resourceGroupName")], matches[regex.SubexpIndex("networkVirtualApplianceName")], nil)
		v.newListPager = &resp
