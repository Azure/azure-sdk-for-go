...
//   - poolName - The name of the IPAM pool.
//   - staticCidrName - Name for the static CIDR.
...
func (client *StaticCidrsClient) Create(ctx context.Context, resourceGroupName string, networkManagerName string, poolName string, staticCidrName string, body StaticCidr, options *StaticCidrsClientCreateOptions) (StaticCidrsClientCreateResponse, error) {
	var err error
	const operationName = "StaticCidrsClient.Create"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.createCreateRequest(ctx, resourceGroupName, networkManagerName, poolName, staticCidrName, body, options)
	if err != nil {
...
func (client *StaticCidrsClient) createCreateRequest(ctx context.Context, resourceGroupName string, networkManagerName string, poolName string, staticCidrName string, body StaticCidr, _ *StaticCidrsClientCreateOptions) (*policy.Request, error) {
