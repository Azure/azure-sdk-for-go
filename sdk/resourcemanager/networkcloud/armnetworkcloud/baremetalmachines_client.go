...
func (client *BareMetalMachinesClient) BeginValidateHardware(ctx context.Context, resourceGroupName string, bareMetalMachineName string, bareMetalMachineValidateHardwareParameters BareMetalMachineValidateHardwareParameters, options *BareMetalMachinesClientBeginValidateHardwareOptions) (*runtime.Poller[BareMetalMachinesClientValidateHardwareResponse], error) {
	if options == nil || options.ResumeToken == "" {
		...
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[BareMetalMachinesClientValidateHardwareResponse]{
			FinalStateVia: runtime.FinalStateViaLocation,
		})
		return poller, err
	} else {
...
func (client *BareMetalMachinesClient) validateHardwareCreateRequest(ctx context.Context, resourceGroupName string, bareMetalMachineName string, bareMetalMachineValidateHardwareParameters BareMetalMachineValidateHardwareParameters, options *BareMetalMachinesClientBeginValidateHardwareOptions) (*policy.Request, error) {
	...
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	if err := runtime.MarshalAsJSON(req, bareMetalMachineValidateHardwareParameters); err != nil {
		return nil, err
	}
	return req, nil
}
