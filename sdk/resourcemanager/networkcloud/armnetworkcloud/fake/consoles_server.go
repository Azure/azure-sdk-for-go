...
func (c *ConsolesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.NetworkCloud/virtualMachines/(?P<virtualMachineName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/consoles/(?P<consoleName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (c *ConsolesServerTransport) dispatchNewListByVirtualMachinePager(req *http.Request) (*http.Response, error) {
	...
	if newListByVirtualMachinePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.NetworkCloud/virtualMachines/(?P<virtualMachineName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/consoles`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		var options *armnetworkcloud.ConsolesClientListByVirtualMachineOptions
		if topParam != nil || skipTokenParam != nil {
			options = &armnetworkcloud.ConsolesClientListByVirtualMachineOptions{
				Top:       topParam,
				SkipToken: skipTokenParam,
			}
		}
		resp := c.srv.NewListByVirtualMachinePager(resourceGroupNameParam, virtualMachineNameParam, options)
