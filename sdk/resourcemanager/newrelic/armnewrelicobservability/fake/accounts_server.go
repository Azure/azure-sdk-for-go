...
func (a *AccountsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/NewRelic\.Observability/accounts`
		...
		qp := req.URL.Query()
		resp := a.srv.NewListPager(qp.Get("userEmail"), qp.Get("location"), nil)
		newListPager = &resp
