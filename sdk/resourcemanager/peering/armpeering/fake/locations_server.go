...
type LocationsServer struct {
	...
	NewListPager func(kind armpeering.LocationsKind, options *armpeering.LocationsClientListOptions) (resp azfake.PagerResponder[armpeering.LocationsClientListResponse])
}
...
func (l *LocationsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		...
		qp := req.URL.Query()
		kindParam, err := parseWithCast(qp.Get("kind"), func(v string) (armpeering.LocationsKind, error) {
			p, unescapeErr := url.QueryUnescape(v)
			if unescapeErr != nil {
				return "", unescapeErr
			}
			return armpeering.LocationsKind(p), nil
		})
		if err != nil {
			return nil, err
		}
		...
		directPeeringTypeParam := getOptional(armpeering.LocationsDirectPeeringType(directPeeringTypeUnescaped))
