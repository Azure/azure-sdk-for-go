...
func (p *PrefixesServerTransport) dispatchCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Peering/peeringServices/(?P<peeringServiceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/prefixes/(?P<prefixName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (p *PrefixesServerTransport) dispatchNewListByPeeringServicePager(req *http.Request) (*http.Response, error) {
	...
	if newListByPeeringServicePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Peering/peeringServices/(?P<peeringServiceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/prefixes`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
