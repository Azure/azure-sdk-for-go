...
func NewPeeringServiceProvidersClient(subscriptionID string, credential azcore.TokenCredential, options *arm.ClientOptions) *PeeringServiceProvidersClient {
	...
	return &PeeringServiceProvidersClient{subscriptionID: subscriptionID, ep: string(cp.Host), pl: armruntime.NewPipeline(module, version, credential, &cp)}
}
...
func (client *PeeringServiceProvidersClient) listHandleResponse(resp *http.Response) (PeeringServiceProvidersListResponse, error) {
	...
	if err := runtime.UnmarshalAsJSON(resp, &result.PeeringServiceProviderListResult); err != nil {
		return PeeringServiceProvidersListResponse{}, runtime.NewResponseError(err, resp)
	}
	return result, nil
}
