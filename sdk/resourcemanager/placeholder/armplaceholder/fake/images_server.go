...
// ImagesServer is a fake server for instances of the armplaceholder.ImagesClient type.
type ImagesServer struct {
	// Get is the fake for method ImagesClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, name string, imageName string, options *armplaceholder.ImagesClientGetOptions) (resp azfake.Responder[armplaceholder.ImagesClientGetResponse], errResp azfake.ErrorResponder)

	// NewListByDisconnectedOperationPager is the fake for method ImagesClient.NewListByDisconnectedOperationPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListByDisconnectedOperationPager func(resourceGroupName string, name string, options *armplaceholder.ImagesClientListByDisconnectedOperationOptions) (resp azfake.PagerResponder[armplaceholder.ImagesClientListByDisconnectedOperationResponse])

	// ListDownloadURI is the fake for method ImagesClient.ListDownloadURI
	// HTTP status codes to indicate success: http.StatusOK
	ListDownloadURI func(ctx context.Context, resourceGroupName string, name string, imageName string, options *armplaceholder.ImagesClientListDownloadURIOptions) (resp azfake.Responder[armplaceholder.ImagesClientListDownloadURIResponse], errResp azfake.ErrorResponder)
}

// NewImagesServerTransport creates a new instance of ImagesServerTransport with the provided implementation.
// The returned ImagesServerTransport instance is connected to an instance of armplaceholder.ImagesClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewImagesServerTransport(srv *ImagesServer) *ImagesServerTransport {
	return &ImagesServerTransport{
		srv:                                 srv,
		newListByDisconnectedOperationPager: newTracker[azfake.PagerResponder[armplaceholder.ImagesClientListByDisconnectedOperationResponse]](),
	}
}

// ImagesServerTransport connects instances of armplaceholder.ImagesClient to instances of ImagesServer.
// Don't use this type directly, use NewImagesServerTransport instead.
type ImagesServerTransport struct {
	srv                                 *ImagesServer
	newListByDisconnectedOperationPager *tracker[azfake.PagerResponder[armplaceholder.ImagesClientListByDisconnectedOperationResponse]]
}
...
func (i *ImagesServerTransport) dispatchNewListByDisconnectedOperationPager(req *http.Request) (*http.Response, error) {
	...
	if newListByDisconnectedOperationPager == nil {
		...
		var options *armplaceholder.ImagesClientListByDisconnectedOperationOptions
		if filterParam != nil || topParam != nil || skipParam != nil {
			options = &armplaceholder.ImagesClientListByDisconnectedOperationOptions{
				Filter: filterParam,
				Top:    topParam,
				Skip:   skipParam,
			}
		...
		server.PagerResponderInjectNextLinks(newListByDisconnectedOperationPager, req, func(page *armplaceholder.ImagesClientListByDisconnectedOperationResponse, createLink func() string) {
