...
clientFactory, err := armplatformvalidation.NewClientFactory(<subscription ID>, cred, &options)
```
...
```go
client := clientFactory.NewCloudValidationsClient()
```
...
[open an issue](https://github.com/Azure/azure-sdk-for-go/issues) and assign the `Platformvalidation` label.
