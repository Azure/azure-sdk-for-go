...
type TuningOptionsServer struct {
	...
	Get func(ctx context.Context, resourceGroupName string, serverName string, tuningOption armpostgresql.TuningOption, options *armpostgresql.TuningOptionsClientGetOptions) (resp azfake.Responder[armpostgresql.TuningOptionsClientGetResponse], errResp azfake.ErrorResponder)
	...
	// HTTP status codes to indicate success: http.StatusOK
	NewListRecommendationsPager func(resourceGroupName string, serverName string, tuningOption armpostgresql.TuningOption, options *armpostgresql.TuningOptionsClientListRecommendationsOptions) (resp azfake.PagerResponder[armpostgresql.TuningOptionsClientListRecommendationsResponse])
}
...
func (t *TuningOptionsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	tuningOptionParam, err := parseWithCast(matches[regex.SubexpIndex("tuningOption")], func(v string) (armpostgresql.TuningOption, error) {
		p, unescapeErr := url.PathUnescape(v)
		if unescapeErr != nil {
			return "", unescapeErr
		}
		return armpostgresql.TuningOption(p), nil
	})
	if err != nil {
		return nil, err
	}
...
func (t *TuningOptionsServerTransport) dispatchNewListRecommendationsPager(req *http.Request) (*http.Response, error) {
	...
	if newListRecommendationsPager == nil {
		...
		recommendationTypeParam := getOptional(armpostgresql.RecommendationType(recommendationTypeUnescaped))
