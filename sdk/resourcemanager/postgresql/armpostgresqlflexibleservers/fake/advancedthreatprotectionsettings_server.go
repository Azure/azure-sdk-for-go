...
// AdvancedThreatProtectionSettingsServer is a fake server for instances of the armpostgresqlflexibleservers.AdvancedThreatProtectionSettingsClient type.
...
// The returned AdvancedThreatProtectionSettingsServerTransport instance is connected to an instance of armpostgresqlflexibleservers.AdvancedThreatProtectionSettingsClient via the
...
func NewAdvancedThreatProtectionSettingsServerTransport(srv *AdvancedThreatProtectionSettingsServer) *AdvancedThreatProtectionSettingsServerTransport {
	return &AdvancedThreatProtectionSettingsServerTransport{
		srv:                  srv,
		newListByServerPager: newTracker[azfake.PagerResponder[armpostgresqlflexibleservers.AdvancedThreatProtectionSettingsClientListByServerResponse]](),
	}
}

// AdvancedThreatProtectionSettingsServerTransport connects instances of armpostgresqlflexibleservers.AdvancedThreatProtectionSettingsClient to instances of AdvancedThreatProtectionSettingsServer.
...
type AdvancedThreatProtectionSettingsServerTransport struct {
	...
	newListByServerPager *tracker[azfake.PagerResponder[armpostgresqlflexibleservers.AdvancedThreatProtectionSettingsClientListByServerResponse]]
}
...
func (a *AdvancedThreatProtectionSettingsServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DBforPostgreSQL/flexibleServers/(?P<serverName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/advancedThreatProtectionSettings`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armpostgresqlflexibleservers.AdvancedThreatProtectionSettingsClientListByServerResponse, createLink func() string) {
