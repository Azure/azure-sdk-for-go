...
// PrivateLinkResourcesServer is a fake server for instances of the armpostgresqlhsc.PrivateLinkResourcesClient type.
type PrivateLinkResourcesServer struct {
	// Get is the fake for method PrivateLinkResourcesClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, clusterName string, privateLinkResourceName string, options *armpostgresqlhsc.PrivateLinkResourcesClientGetOptions) (resp azfake.Responder[armpostgresqlhsc.PrivateLinkResourcesClientGetResponse], errResp azfake.ErrorResponder)

	// NewListByClusterPager is the fake for method PrivateLinkResourcesClient.NewListByClusterPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListByClusterPager func(resourceGroupName string, clusterName string, options *armpostgresqlhsc.PrivateLinkResourcesClientListByClusterOptions) (resp azfake.PagerResponder[armpostgresqlhsc.PrivateLinkResourcesClientListByClusterResponse])
}
...
// The returned PrivateLinkResourcesServerTransport instance is connected to an instance of armpostgresqlhsc.PrivateLinkResourcesClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewPrivateLinkResourcesServerTransport(srv *PrivateLinkResourcesServer) *PrivateLinkResourcesServerTransport {
	return &PrivateLinkResourcesServerTransport{
		srv:                   srv,
		newListByClusterPager: newTracker[azfake.PagerResponder[armpostgresqlhsc.PrivateLinkResourcesClientListByClusterResponse]](),
	}
}

// PrivateLinkResourcesServerTransport connects instances of armpostgresqlhsc.PrivateLinkResourcesClient to instances of PrivateLinkResourcesServer.
// Don't use this type directly, use NewPrivateLinkResourcesServerTransport instead.
type PrivateLinkResourcesServerTransport struct {
	srv                   *PrivateLinkResourcesServer
	newListByClusterPager *tracker[azfake.PagerResponder[armpostgresqlhsc.PrivateLinkResourcesClientListByClusterResponse]]
}
...
func (p *PrivateLinkResourcesServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DBforPostgreSQL/serverGroupsv2/(?P<clusterName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/privateLinkResources/(?P<privateLinkResourceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (p *PrivateLinkResourcesServerTransport) dispatchNewListByClusterPager(req *http.Request) (*http.Response, error) {
	...
	if newListByClusterPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.DBforPostgreSQL/serverGroupsv2/(?P<clusterName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/privateLinkResources`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		server.PagerResponderInjectNextLinks(newListByClusterPager, req, func(page *armpostgresqlhsc.PrivateLinkResourcesClientListByClusterResponse, createLink func() string) {
