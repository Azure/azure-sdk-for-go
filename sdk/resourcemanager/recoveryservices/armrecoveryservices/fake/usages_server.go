...
func (u *UsagesServerTransport) dispatchNewListByVaultsPager(req *http.Request) (*http.Response, error) {
	...
	if newListByVaultsPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.RecoveryServices/vaults/(?P<vaultName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/usages`
		...
		server.PagerResponderInjectNextLinks(newListByVaultsPager, req, func(page *armrecoveryservices.UsagesClientListByVaultsResponse, createLink func() string) {
