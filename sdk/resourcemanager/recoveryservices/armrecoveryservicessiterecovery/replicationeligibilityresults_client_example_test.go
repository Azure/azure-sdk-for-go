...
func ExampleReplicationEligibilityResultsClient_NewListPager() {
	cred, err := azidentity.NewDefaultAzureCredential(nil)
	if err != nil {
		log.Fatalf("failed to obtain a credential: %v", err)
	}
	...
	pager := clientFactory.NewReplicationEligibilityResultsClient().NewListPager("testRg1", "testVm2", nil)
	for pager.More() {
		...
		// page = armrecoveryservicessiterecovery.ReplicationEligibilityResultsClientListResponse{
