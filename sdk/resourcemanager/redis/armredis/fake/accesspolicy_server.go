...
func (a *AccessPolicyServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if accessPolicyServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = accessPolicyServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (a *AccessPolicyServerTransport) dispatchBeginCreateUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cache/redis/(?P<cacheName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/accessPolicies/(?P<accessPolicyName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (a *AccessPolicyServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cache/redis/(?P<cacheName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/accessPolicies`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to AccessPolicyServerTransport
var accessPolicyServerTransportInterceptor interface {
