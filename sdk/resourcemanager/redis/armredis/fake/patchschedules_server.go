...
func (p *PatchSchedulesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if patchSchedulesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = patchSchedulesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			...
			case "PatchSchedulesClient.NewListByRedisResourcePager":
				res.resp, res.err = p.dispatchNewListByRedisResourcePager(req)
			default:
...
func (p *PatchSchedulesServerTransport) dispatchCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cache/redis/(?P<name>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/patchSchedules/(?P<default>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (p *PatchSchedulesServerTransport) dispatchNewListByRedisResourcePager(req *http.Request) (*http.Response, error) {
	...
	if newListByRedisResourcePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cache/redis/(?P<cacheName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/patchSchedules`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
// set this to conditionally intercept incoming requests to PatchSchedulesServerTransport
var patchSchedulesServerTransportInterceptor interface {
