...
func (p *PrivateLinkResourcesServerTransport) dispatchNewListByRedisCachePager(req *http.Request) (*http.Response, error) {
	...
	if newListByRedisCachePager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Cache/redis/(?P<cacheName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/privateLinkResources`
		...
		server.PagerResponderInjectNextLinks(newListByRedisCachePager, req, func(page *armredis.PrivateLinkResourcesClientListByRedisCacheResponse, createLink func() string) {
