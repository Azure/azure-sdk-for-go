...
func (s SKUDetails) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "customerFacingSizeInGB", s.CustomerFacingSizeInGB)
	populate(objectMap, "name", s.Name)
	return json.Marshal(objectMap)
}
...
func (s *SKUDetails) UnmarshalJSON(data []byte) error {
	...
	for key, val := range rawMsg {
		var err error
		switch key {
		case "customerFacingSizeInGB":
			err = unpopulate(val, "CustomerFacingSizeInGB", &s.CustomerFacingSizeInGB)
			delete(rawMsg, key)
		case "name":
			err = unpopulate(val, "Name", &s.Name)
			delete(rawMsg, key)
