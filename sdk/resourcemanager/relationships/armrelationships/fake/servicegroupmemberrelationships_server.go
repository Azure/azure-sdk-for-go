...
func (s *ServiceGroupMemberRelationshipsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/(?P<resourceUri>[a-zA-Z0-9._~%!$&'()*+,;=:@/-]+)/providers/Microsoft\.Relationships/serviceGroupMember/(?P<name>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
...
func (s *ServiceGroupMemberRelationshipsServerTransport) dispatchNewListByParentPager(req *http.Request) (*http.Response, error) {
	...
	if newListByParentPager == nil {
		const regexStr = `/(?P<resourceUri>[a-zA-Z0-9._~%!$&'()*+,;=:@/-]+)/providers/Microsoft\.Relationships/serviceGroupMember`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
