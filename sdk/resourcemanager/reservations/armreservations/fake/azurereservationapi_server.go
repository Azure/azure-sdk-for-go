...
func (a *AzureReservationAPIServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	resultChan := make(chan result, 1)
	go func() {
		var intercepted bool
		var res result
		if azureReservationApiServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = azureReservationApiServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
			case "AzureReservationAPIClient.GetAppliedReservationList":
				res.resp, res.err = a.dispatchGetAppliedReservationList(req)
			case "AzureReservationAPIClient.NewGetCatalogPager":
				res.resp, res.err = a.dispatchNewGetCatalogPager(req)
			default:
...
func (a *AzureReservationAPIServerTransport) dispatchNewGetCatalogPager(req *http.Request) (*http.Response, error) {
	...
	if newGetCatalogPager == nil {
		...
		reservedResourceTypeParam := getOptional(qp.Get("reservedResourceType"))
		locationParam := getOptional(qp.Get("location"))
		...
		skipParam, err := parseOptional(qp.Get("$skip"), func(v string) (float32, error) {
...
// set this to conditionally intercept incoming requests to AzureReservationAPIServerTransport
var azureReservationApiServerTransportInterceptor interface {
