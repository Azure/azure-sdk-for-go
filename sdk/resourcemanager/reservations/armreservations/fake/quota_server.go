...
type QuotaServer struct {
	...
	BeginCreateOrUpdate func(ctx context.Context, providerID string, location string, resourceName string, createQuotaRequest armreservations.CurrentQuotaLimitBase, options *armreservations.QuotaClientBeginCreateOrUpdateOptions) (resp azfake.PollerResponder[armreservations.QuotaClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method QuotaClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, providerID string, location string, resourceName string, options *armreservations.QuotaClientGetOptions) (resp azfake.Responder[armreservations.QuotaClientGetResponse], errResp azfake.ErrorResponder)

	// NewListPager is the fake for method QuotaClient.NewListPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListPager func(providerID string, location string, options *armreservations.QuotaClientListOptions) (resp azfake.PagerResponder[armreservations.QuotaClientListResponse])
...
func (q *QuotaServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		...
		respr, errRespr := q.srv.BeginCreateOrUpdate(req.Context(), providerIDParam, locationParam, resourceNameParam, body, nil)
...
func (q *QuotaServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := q.srv.Get(req.Context(), providerIDParam, locationParam, resourceNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
...
func (q *QuotaServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		...
		resp := q.srv.NewListPager(providerIDParam, locationParam, nil)
		newListPager = &resp
...
func (q *QuotaServerTransport) dispatchBeginUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginUpdate == nil {
		...
		respr, errRespr := q.srv.BeginUpdate(req.Context(), providerIDParam, locationParam, resourceNameParam, body, nil)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
		beginUpdate = &respr
