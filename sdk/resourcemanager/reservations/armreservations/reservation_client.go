...
// NewListPager - Get `Reservation`s in a given reservation Order
//
...
// NewListRevisionsPager - Get `Reservation` revisions.
//
...
// BeginMerge - Merges two `Reservation`s.
//
...
// Merge - Merges two `Reservation`s.
//
// Merge the specified `Reservation`s into a new `Reservation`. The two `Reservation`s being merged must have same properties.
// If the operation fails it returns an *azcore.ResponseError type.
//
...
// BeginSplit - Split the `Reservation`.
//
...
// Unarchive - Unarchive a `Reservation`.
//
...
// BeginUpdate - Updates a `Reservation`.
//
