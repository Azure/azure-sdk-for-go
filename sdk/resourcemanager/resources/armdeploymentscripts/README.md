...
The `armdeploymentscripts` module provides operations for working with Azure Resources.
...
```go
client := armdeploymentscripts.{{NewClientName}}(<subscription ID>, cred, nil)
```
...
client := armdeploymentscripts.{{NewClientName}}(<subscription ID>, cred, &options)
```
