...
// ExemptionsClient contains the methods for the Exemptions group.
...
// CreateOrUpdate - Creates or updates a policy exemption.
//
...
//   - policyExemptionName - The name of the policy exemption to get.
...
// Delete - Deletes a policy exemption.
//
...
// Get - Retrieves a policy exemption.
//
...
// NewListPager - Retrieves all policy exemptions that apply to a subscription.
//
...
// given $filter. Valid values for $filter are: 'atScope()', 'atExactScope()', 'excludeExpired()' or 'policyAssignmentId eq
// '{value}”. If $filter is not provided, the unfiltered list includes all policy exemptions associated with the subscription,
...
// NewListForManagementGroupPager - Retrieves all policy exemptions that apply to a management group.
//
...
// Valid values for $filter are: 'atScope()', 'atExactScope()', 'excludeExpired()' or 'policyAssignmentId eq '{value}”. If
// $filter=atScope() is provided, the returned list includes all policy exemptions that are assigned to the management group
// or the management group's ancestors.
...
// NewListForResourcePager - Retrieves all policy exemptions that apply to a resource.
//
...
// exemptions associated with the resource, including those that apply directly or from all containing scopes, as well as
// any applied to resources contained within the resource. Three parameters plus the resource name are used to identify a
// specific resource. If the resource is not part of a parent resource (the more common case), the parent resource path should
// not be provided (or provided as ”). For example a web app could be specified as ({resourceProviderNamespace} == 'Microsoft.Web',
// {parentResourcePath} == ”, {resourceType} == 'sites', {resourceName} == 'MyWebApp'). If the resource is part of a parent
// resource, then all parameters should be provided. For example a virtual machine DNS name could be specified as ({resourceProviderNamespace}
// == 'Microsoft.Compute', {parentResourcePath} == 'virtualMachines/MyVirtualMachine', {resourceType} == 'domainNames', {resourceName}
// == 'MyComputerName'). A convenient alternative to providing the namespace and type name separately is to provide both in
// the {resourceType} parameter, format: ({resourceProviderNamespace} == ”, {parentResourcePath} == ”, {resourceType} ==
...
// NewListForResourceGroupPager - Retrieves all policy exemptions that apply to a resource group.
//
...
// Update - Updates a policy exemption.
//
