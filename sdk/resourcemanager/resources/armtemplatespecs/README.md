...
The `armtemplatespecs` module provides operations for working with Azure Resources.
...
```go
client := armtemplatespecs.{{NewClientName}}(<subscription ID>, cred, nil)
```
...
client := armtemplatespecs.{{NewClientName}}(<subscription ID>, cred, &options)
```
