...
- Struct `OfferingsListResult` has been removed

### Features Added

- New enum type `AutoGeneratedDomainNameLabelScope` with values `AutoGeneratedDomainNameLabelScopeNoReuse`, `AutoGeneratedDomainNameLabelScopeSubscriptionReuse`, `AutoGeneratedDomainNameLabelScopeTenantReuse`
- New function `*OfferingsClient.Fetch(ctx context.Context, options *OfferingsClientFetchOptions) (OfferingsClientFetchResponse, error)`
- New struct `OfferingsResult`
- New field `AutoGeneratedDomainNameLabelScope` in struct `ServiceProperties`
