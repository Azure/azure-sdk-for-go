...
func (client *PricingsClient) deleteCreateRequest(ctx context.Context, scopeID string, pricingName string, _ *PricingsClientDeleteOptions) (*policy.Request, error) {
	...
	if scopeID == "" {
		return nil, errors.New("parameter scopeID cannot be empty")
	}
...
// Valid scopes are: subscription id or a specific resource id (Supported resources are: 'VirtualMachines, VMSS and ARC Machines').
// Valid $filter is: 'name in ({planName1},{planName2},...)'. If $filter is not provided, the unfiltered list will be returned.
