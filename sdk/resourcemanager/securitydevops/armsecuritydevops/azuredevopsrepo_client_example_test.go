...
func ExampleAzureDevOpsRepoClient_NewListPager() {
	...
	pager := client.NewListPager("westusrg", "testconnector", "myOrg", "myProject", nil)
	for pager.More() {
		nextResult, err := pager.NextPage(ctx)
		if err != nil {
			log.Fatalf("failed to advance page: %v", err)
...
func ExampleAzureDevOpsRepoClient_BeginCreateOrUpdate() {
	...
	poller, err := client.BeginCreateOrUpdate(ctx, "westusrg", "testconnector", "myOrg", "myProject", "myRepo", armsecuritydevops.AzureDevOpsRepo{
