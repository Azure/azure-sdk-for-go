...
type IncidentsServer struct {
	...
	// NewListAlertsPager is the fake for method IncidentsClient.NewListAlertsPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListAlertsPager func(resourceGroupName string, workspaceName string, incidentID string, options *armsecurityinsights.IncidentsClientListAlertsOptions) (resp azfake.PagerResponder[armsecurityinsights.IncidentsClientListAlertsResponse])

	// NewListBookmarksPager is the fake for method IncidentsClient.NewListBookmarksPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListBookmarksPager func(resourceGroupName string, workspaceName string, incidentID string, options *armsecurityinsights.IncidentsClientListBookmarksOptions) (resp azfake.PagerResponder[armsecurityinsights.IncidentsClientListBookmarksResponse])
...
func NewIncidentsServerTransport(srv *IncidentsServer) *IncidentsServerTransport {
	return &IncidentsServerTransport{
		...
		newListAlertsPager:    newTracker[azfake.PagerResponder[armsecurityinsights.IncidentsClientListAlertsResponse]](),
		newListBookmarksPager: newTracker[azfake.PagerResponder[armsecurityinsights.IncidentsClientListBookmarksResponse]](),
	}
}
...
type IncidentsServerTransport struct {
	...
	newListAlertsPager    *tracker[azfake.PagerResponder[armsecurityinsights.IncidentsClientListAlertsResponse]]
	newListBookmarksPager *tracker[azfake.PagerResponder[armsecurityinsights.IncidentsClientListBookmarksResponse]]
}
...
func (i *IncidentsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "IncidentsClient.NewListAlertsPager":
				res.resp, res.err = i.dispatchNewListAlertsPager(req)
			case "IncidentsClient.NewListBookmarksPager":
				res.resp, res.err = i.dispatchNewListBookmarksPager(req)
...
func (i *IncidentsServerTransport) dispatchNewListAlertsPager(req *http.Request) (*http.Response, error) {
	...
	if newListAlertsPager == nil {
		...
		resp := i.srv.NewListAlertsPager(resourceGroupNameParam, workspaceNameParam, incidentIDParam, nil)
...
func (i *IncidentsServerTransport) dispatchNewListBookmarksPager(req *http.Request) (*http.Response, error) {
	if i.srv.NewListBookmarksPager == nil {
		return nil, &nonRetriableError{errors.New("fake for method NewListBookmarksPager not implemented")}
	}
	newListBookmarksPager := i.newListBookmarksPager.get(req)
	if newListBookmarksPager == nil {
		...
		resp := i.srv.NewListBookmarksPager(resourceGroupNameParam, workspaceNameParam, incidentIDParam, nil)
		newListBookmarksPager = &resp
		i.newListBookmarksPager.add(req, newListBookmarksPager)
	}
	resp, err := server.PagerResponderNext(newListBookmarksPager, req)
	if err != nil {
		return nil, err
	}
	if !contains([]int{http.StatusOK}, resp.StatusCode) {
		i.newListBookmarksPager.remove(req)
		return nil, &nonRetriableError{fmt.Errorf("unexpected status code %d. acceptable values are http.StatusOK", resp.StatusCode)}
	}
	if !server.PagerResponderMore(newListBookmarksPager) {
		i.newListBookmarksPager.remove(req)
	}
	return resp, nil
}
...
func (i *IncidentsServerTransport) dispatchRunPlaybook(req *http.Request) (*http.Response, error) {
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).RunPlaybookResponse, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
