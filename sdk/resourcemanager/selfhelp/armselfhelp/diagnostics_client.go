...
func (client *DiagnosticsClient) BeginCreate(ctx context.Context, scope string, diagnosticsResourceName string, options *DiagnosticsClientBeginCreateOptions) (*runtime.Poller[DiagnosticsClientCreateResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.create(ctx, scope, diagnosticsResourceName, options)
		if err != nil {
			return nil, err
		}
...
//
// Generated from API version 2023-06-01
func (client *DiagnosticsClient) create(ctx context.Context, scope string, diagnosticsResourceName string, options *DiagnosticsClientBeginCreateOptions) (*http.Response, error) {
	var err error
	req, err := client.createCreateRequest(ctx, scope, diagnosticsResourceName, options)
	if err != nil {
		return nil, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
...
func (client *DiagnosticsClient) createCreateRequest(ctx context.Context, scope string, diagnosticsResourceName string, options *DiagnosticsClientBeginCreateOptions) (*policy.Request, error) {
	...
	req.Raw().URL.RawQuery = reqQP.Encode()
	req.Raw().Header["Accept"] = []string{"application/json"}
	if options != nil && options.DiagnosticResourceRequest != nil {
		if err := runtime.MarshalAsJSON(req, *options.DiagnosticResourceRequest); err != nil {
	return nil, err
}
		return req, nil
	}
