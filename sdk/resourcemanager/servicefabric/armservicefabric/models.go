...
type ApplicationMetricDescription struct {
	...
	// This is the maximum Load for an instance of this application on a single node. Even if the capacity of node is greater
	// than this value, Service Fabric will limit the total load of services within the application on each node to this value.
	// If set to zero, capacity for this metric is unlimited on each node.
	// When creating a new application with application capacity defined, the product of MaximumNodes and this value must always
	// be smaller than or equal to TotalApplicationCapacity.
	// When updating existing application with application capacity, the product of MaximumNodes and this value must always be
	// smaller than or equal to TotalApplicationCapacity.
	...
	// When setting application capacity or when updating application capacity; this value must be smaller than or equal to MaximumCapacity
	// for each metric.
	ReservationCapacity *int64

	// The total metric capacity for Service Fabric application.
	// This is the total metric capacity for this application in the cluster. Service Fabric will try to limit the sum of loads
	// of services within the application to this value.
	// When creating a new application with application capacity defined, the product of MaximumNodes and MaximumCapacity must
...
type ApplicationResourceProperties struct {
	...
	// that the services of this application will be placed on all of those nodes. By default, the value of this property is zero
	// and it means that the services can be placed on any node.
	MaximumNodes *int64
	...
	// that the services of this application will be placed on all of those nodes. If this property is set to zero, no capacity
	// will be reserved. The value of this property cannot be more than the value of the MaximumNodes property.
	MinimumNodes *int64
...
// ApplicationUserAssignedIdentity - Describes a user assigned identity for the application.
...
type ArmRollingUpgradeMonitoringPolicy struct {
	...
	// The amount of time to wait after completing an upgrade domain before applying health policies. It is interpreted as a string
...
// ClusterCodeVersionsListResult - The response of a ClusterCodeVersions list operation.
type ClusterCodeVersionsListResult struct {
	// REQUIRED; The ClusterCodeVersions items on this page
...
type ClusterProperties struct {
	...
	// the cluster, SSL certificate for cluster management endpoint and default admin client.
	...
	// The Service Fabric runtime version of the cluster. This property can only by set the user when **upgradeMode** is set to
	// 'Manual'. To get list of available Service Fabric versions for new clusters use [ClusterVersion API](https://learn.microsoft.com/rest/api/servicefabric/cluster-versions/list).
	...
	// 1.3 and above. If token-based authentication is used, HttpGatewayTokenAuthEndpointPort must be defined.
	...
	//   - Deploying - Indicates that the Service Fabric runtime is being installed on the VMs. Cluster resource will be in this state
	...
	//   - UpdatingInfrastructure - Indicates that the cluster is being upgraded with the latest Service Fabric runtime version. This
...
type ClusterUpgradeDeltaHealthPolicy struct {
	// REQUIRED; The maximum allowed percentage of applications health degradation allowed during cluster upgrades.
	...
	// REQUIRED; The maximum allowed percentage of upgrade domain nodes health degradation allowed during cluster upgrades.
...
type ClusterUpgradePolicy struct {
	...
	// are unexpected issues. When this timeout expires, processing of the upgrade domain will proceed regardless of availability
	// loss issues. The timeout is reset at the start of each upgrade domain. The timeout can be in either hh:mm:ss or in d.hh:mm:ss.ms
	// format.
	UpgradeReplicaSetCheckTimeout *string
...
type NodeTypeDescription struct {
	...
	// VMInstanceCount = 0 can be done only in these scenarios: NodeType is a secondary nodeType. Durability = Bronze or Durability
	// >= Bronze and InfrastructureServiceManager = true. If VMInstanceCount = 0, implies the VMs for this nodeType will not be
	...
	//   - Gold - The infrastructure jobs can be paused for a duration of 2 hours per UD. Gold durability can be enabled only on full
	//     node VM skus like D15_V2, G5 etc.
...
type ServiceTypeDeltaHealthPolicy struct {
	...
	// The delta is measured between the state of the services at the beginning of upgrade and the state of the services at the
	// time of the health evaluation.
	// The check is performed after every upgrade domain upgrade completion to make sure the global state of the cluster is within
	// tolerated limits.
...
type StatelessServiceProperties struct {
	...
	// the delay starts prior to closing the instance. This delay enables existing requests to drain gracefully before the instance
	// actually goes down (https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-application-upgrade-advanced#avoid-connection-drops-during-stateless-service-planned-downtime-preview).
...
type UniformInt64RangePartitionSchemeDescription struct {
	...
	// REQUIRED; String indicating the upper bound of the partition key range that
	// should be split between the partition ‘count’
	HighKey *string

	// REQUIRED; String indicating the lower bound of the partition key range that
	// should be split between the partition ‘count’
	LowKey *string
...
// UpgradableVersionsDescription - The upgrade path description with target version.
type UpgradableVersionsDescription struct {
	// REQUIRED; The target code version.
	TargetVersion *string
}

// UserAssignedIdentity - Describes a user assigned identity.
...
type VMSizesResult struct {
	// REQUIRED; The VMSizeResource items on this page
