...
// CustomCertificatesClientListPager provides operations for iterating over paged responses.
type CustomCertificatesClientListPager struct {
	client    *CustomCertificatesClient
	current   CustomCertificatesClientListResponse
	err       error
	requester func(context.Context) (*policy.Request, error)
	advancer  func(context.Context, CustomCertificatesClientListResponse) (*policy.Request, error)
}

// Err returns the last error encountered while paging.
func (p *CustomCertificatesClientListPager) Err() error {
	return p.err
}

// NextPage returns true if the pager advanced to the next page.
// Returns false if there are no more pages or an error occurred.
func (p *CustomCertificatesClientListPager) NextPage(ctx context.Context) bool {
	var req *policy.Request
	var err error
	if !reflect.ValueOf(p.current).IsZero() {
		if p.current.CustomCertificateList.NextLink == nil || len(*p.current.CustomCertificateList.NextLink) == 0 {
			return false
		}
		req, err = p.advancer(ctx, p.current)
	} else {
...
// PageResponse returns the current CustomCertificatesClientListResponse page.
func (p *CustomCertificatesClientListPager) PageResponse() CustomCertificatesClientListResponse {
	return p.current
}
...
func (p *CustomDomainsClientListPager) NextPage(ctx context.Context) bool {
	...
	var err error
	if !reflect.ValueOf(p.current).IsZero() {
		if p.current.CustomDomainList.NextLink == nil || len(*p.current.CustomDomainList.NextLink) == 0 {
			return false
		}
		req, err = p.advancer(ctx, p.current)
	} else {
