...
- New field `SystemData` in struct `DataMaskingRule`
...
- New field `SystemData` in struct `DatabaseExtensions`
...
- New field `SystemData` in struct `DatabaseOperation`
...
- New field `SystemData` in struct `DatabaseUsage`
...
- New field `SystemData` in struct `DatabaseVulnerabilityAssessmentScansExport`
...
- New field `SystemData` in struct `ElasticPoolOperation`
...
- New field `SystemData` in struct `ImportExportOperationResult`
...
- New field `SystemData` in struct `LongTermRetentionBackupOperationResult`
...
- New field `SystemData` in struct `QueryStatistics`
- New field `SystemData` in struct `RecommendedAction`
- New field `SystemData` in struct `RecommendedSensitivityLabelUpdate`
- New field `SystemData` in struct `RecoverableDatabase`
- New field `SystemData` in struct `RecoverableManagedDatabase`
- New field `SystemData` in struct `RefreshExternalGovernanceStatusOperationResult`
- New field `SystemData` in struct `RefreshExternalGovernanceStatusOperationResultMI`
- New field `SystemData` in struct `ReplicationLink`
- New field `SystemData` in struct `ReplicationLinkUpdate`
...
- New field `SystemData` in struct `ServerOperation`
- New field `SystemData` in struct `ServerTrustCertificate`
- New field `SystemData` in struct `ServerTrustGroup`
- New field `ID`, `Properties`, `SystemData`, `Type` in struct `ServerUsage`
...
- New field `SystemData` in struct `SyncMember`
- New field `SystemData` in struct `TdeCertificate`
- New field `SystemData` in struct `TimeZone`
- New field `ScanState` in struct `TransparentDataEncryptionProperties`
- New field `SystemData` in struct `VirtualCluster`
