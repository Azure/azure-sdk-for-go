...
// The returned VirtualNetworkRulesServerTransport instance is connected to an instance of armsql.VirtualNetworkRulesClient via the
...
func NewVirtualNetworkRulesServerTransport(srv *VirtualNetworkRulesServer) *VirtualNetworkRulesServerTransport {
	return &VirtualNetworkRulesServerTransport{
		srv:                  srv,
		beginCreateOrUpdate:  newTracker[azfake.PollerResponder[armsql.VirtualNetworkRulesClientCreateOrUpdateResponse]](),
		beginDelete:          newTracker[azfake.PollerResponder[armsql.VirtualNetworkRulesClientDeleteResponse]](),
		newListByServerPager: newTracker[azfake.PagerResponder[armsql.VirtualNetworkRulesClientListByServerResponse]](),
	}
}

// VirtualNetworkRulesServerTransport connects instances of armsql.VirtualNetworkRulesClient to instances of VirtualNetworkRulesServer.
...
type VirtualNetworkRulesServerTransport struct {
	...
	beginCreateOrUpdate  *tracker[azfake.PollerResponder[armsql.VirtualNetworkRulesClientCreateOrUpdateResponse]]
	beginDelete          *tracker[azfake.PollerResponder[armsql.VirtualNetworkRulesClientDeleteResponse]]
	newListByServerPager *tracker[azfake.PagerResponder[armsql.VirtualNetworkRulesClientListByServerResponse]]
}
...
func (v *VirtualNetworkRulesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Sql/servers/(?P<serverName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/virtualNetworkRules/(?P<virtualNetworkRuleName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armsql.VirtualNetworkRule](req)
		if err != nil {
			return nil, err
		}
...
func (v *VirtualNetworkRulesServerTransport) dispatchNewListByServerPager(req *http.Request) (*http.Response, error) {
	...
	if newListByServerPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.Sql/servers/(?P<serverName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/virtualNetworkRules`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		server.PagerResponderInjectNextLinks(newListByServerPager, req, func(page *armsql.VirtualNetworkRulesClientListByServerResponse, createLink func() string) {
