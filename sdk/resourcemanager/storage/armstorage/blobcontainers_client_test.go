...
func TestBlobContainersClient_Lease(t *testing.T) {
	...
	// create resource group
	rg, clean := createResourceGroup(t, cred, opt, subscriptionID, "rg", "westus")
	defer clean()
	rgName := *rg.Name
	...
	require.NoError(t, err)
	require.Equal(t, 200, leaseResp.RawResponse.StatusCode)
	...
	require.NoError(t, err)
	require.Equal(t, "0", *breakResp.LeaseTimeSeconds)
}
...
/* clean resources need long time
