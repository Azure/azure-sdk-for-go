...
type AccountCreateParameters struct {
	...
	// Regions (e.g. West US, East US, Southeast Asia, etc.). The geo region of a resource cannot be changed once it is created,
	...
	// resource (across resource groups). A maximum of 15 tags can be provided for a resource. Each tag must have a key with a
...
type AccountProperties struct {
	...
	AllowSharedKeyAccessForServices *AccountSharedKeyAccessProperties
	...
	// timestamp is retained. This element is not returned if there has never been a failover instance. Only available if the
	...
	// READ-ONLY; Gets the URLs that are used to perform a retrieval of a public blob, queue, or table object. Note that Standard_ZRS
...
type AccountPropertiesUpdateParameters struct {
	...
	// per storage account at this time. To clear the existing custom domain, use an empty string for the custom domain name property.
	CustomDomain *CustomDomain
...
type AccountSasParameters struct {
	...
	// Container (c): Access to container-level APIs; Object (o): Access to object-level APIs for blobs, queue messages, table
	// entities, and files.
...
// AccountSharedKeyAccessProperties - Defines shared key access properties for a storage account.
type AccountSharedKeyAccessProperties struct {
...
type AccountUsage struct {
	// Usage of provisioned storage, IOPS, bandwidth and number of file shares across all live shares or soft-deleted shares in
	// the account.
...
type ActiveDirectoryProperties struct {
	...
	// directoryServiceOptions is set to AADDS (Entra DS authentication), this property can be omitted. If directoryServiceOptions
	...
	// this property is optional. If provided, accountType should also be provided. For directoryServiceOptions AADDS (Entra DS
...
type AzureFilesIdentityBasedAuthentication struct {
	...
	// for directoryServiceOptions AADDS (Entra DS authentication) and AADKERB (Entra authentication).
...
type BlobInventoryPolicyDefinition struct {
	...
	// inventory. The Schema field value 'Name' is always required. The valid values for this field for the 'Blob' definition.objectType
	// include 'Name, Creation-Time, Last-Modified, Content-Length, Content-MD5, BlobType, AccessTier, AccessTierChangeTime, AccessTierInferred,
	// Tags, Expiry-Time, hdi_isfolder, Owner, Group, Permissions, Acl, Snapshot, VersionId, IsCurrentVersion, Metadata, LastAccessTime,
	// Tags, Etag, ContentType, ContentEncoding, ContentLanguage, ContentCRC64, CacheControl, ContentDisposition, LeaseStatus,
	// LeaseState, LeaseDuration, ServerEncrypted, Deleted, DeletionId, DeletedTime, RemainingRetentionDays, ImmutabilityPolicyUntilDate,
	// ImmutabilityPolicyMode, LegalHold, CopyId, CopyStatus, CopySource, CopyProgress, CopyCompletionTime, CopyStatusDescription,
	// CustomerProvidedKeySha256, RehydratePriority, ArchiveStatus, XmsBlobSequenceNumber, EncryptionScope, IncrementalCopy, TagCount'.
	// For Blob object type schema field value 'DeletedTime' is applicable only for Hns enabled accounts. The valid values for
	// 'Container' definition.objectType include 'Name, Last-Modified, Metadata, LeaseStatus, LeaseState, LeaseDuration, PublicAccess,
	// HasImmutabilityPolicy, HasLegalHold, Etag, DefaultEncryptionScope, DenyEncryptionScopeOverride, ImmutableStorageWithVersioningEnabled,
	// Deleted, Version, DeletedTime, RemainingRetentionDays'. Schema field values 'Expiry-Time, hdi_isfolder, Owner, Group, Permissions,
	// Acl, DeletionId' are valid only for Hns enabled accounts.Schema field values 'Tags, TagCount' are only valid for Non-Hns
	// accounts.
	SchemaFields []*string
...
// all filter properties are applicable, 'blobTypes' is required and others are optional. For 'Container' definition.objectType
...
type Connector struct {
	...
	// REQUIRED; The properties of the Storage Connector.
...
// ConnectorAuthProperties - The authentication properties of the backing data source
type ConnectorAuthProperties struct {
...
// GetConnectorConnection implements the ConnectorConnectionClassification interface for type ConnectorConnection.
func (c *ConnectorConnection) GetConnectorConnection() *ConnectorConnection { return c }
...
// ConnectorProperties - The storage connector properties
type ConnectorProperties struct {
	// REQUIRED; The type of backing data source for this Storage Connector.
	DataSourceType *StorageConnectorDataSourceType

	// REQUIRED; Information about how to communicate with and authenticate to the backing data store.
	Source ConnectorSourceClassification
	...
	// READ-ONLY; System-generated creation time of the Storage Connector in ISO 8601 date-time format (YYYY-MM-DDTHH:mm:ssZ).
	// Not a valid input parameter during creating.
	CreationTime *string

	// READ-ONLY; Represents the provisioning state of the storage connector.
	ProvisioningState *NativeDataSharingProvisioningState

	// READ-ONLY; System-generated GUID identifier for the Storage Connector. Not a valid input parameter when creating.
	UniqueID *string
}
...
type ContainerProperties struct {
	...
	// container. The hasImmutabilityPolicy public property is set to false by SRP if ImmutabilityPolicy has not been created
	// for this container.
	...
	// public property is set to false by SRP if all existing legal hold tags are cleared out. There can be a maximum of 1000
...
type DataShare struct {
	...
	// REQUIRED; The properties of the Storage DataShare.
	Properties *DataShareProperties
...
// DataShareAccessPolicy - Policy that specify the permission allowed to a managed identity
type DataShareAccessPolicy struct {
	// REQUIRED; Allowed permissions. Currently, only supported value is Read.
	Permission *StorageDataShareAccessPolicyPermission

	// REQUIRED; The AAD principal ID of the Managed Identity.
	PrincipalID *string

	// REQUIRED; The AAD tenant ID of the Managed Identity.
	TenantID *string
}

// DataShareAsset - Properties of a shared resource.
type DataShareAsset struct {
	// REQUIRED; Source Path to be shared. It can be a folder or a blob.
	// The asset path should contain container name followed by path within the container, e.g. /container1/logs/external.
	AssetPath *string

	// REQUIRED; Consumer visible name of the original path.
	DisplayName *string
}

// DataShareConnection - The connection details for Data Share source
...
// GetConnectorConnection implements the ConnectorConnectionClassification interface for type DataShareConnection.
func (d *DataShareConnection) GetConnectorConnection() *ConnectorConnection {
...
type DataShareListResult struct {
	// REQUIRED; The DataShare items on this page
	Value []*DataShare
...
// DataShareProperties - The storage datashare properties
type DataShareProperties struct {
	// REQUIRED; List of access policies that specify the permission allowed to a managed identity.
	...
	// REQUIRED; List of assets that specify the properties of the shared resources.
	...
	// READ-ONLY; System-generated GUID identifier for the Storage DataShare. Not a valid input parameter when creating.
	DataShareIdentifier *string

	// READ-ONLY; The DataShare URI to be shared with the consumer.
	// URI Format - 'azds://<location>:<dataShareName>:<dataShareIdentifier>'.
	DataShareURI *string

	// READ-ONLY; Represents the provisioning state of the storage datashare.
	ProvisioningState *NativeDataSharingProvisioningState
}
...
type DataShareSource struct {
	...
	AuthProperties ConnectorAuthPropertiesClassification
...
// GetConnectorSource implements the ConnectorSourceClassification interface for type DataShareSource.
func (d *DataShareSource) GetConnectorSource() *ConnectorSource {
...
type DateAfterCreation struct {
	...
	// and requires daysAfterCreationGreaterThan to be set for snapshots and blob version based actions. The blob will be archived
...
// daysAfterLastAccessTimeGreaterThan and daysAfterCreationGreaterThan are mutually exclusive. The daysAfterLastTierChangeGreaterThan
// property is only applicable for tierToArchive actions which requires daysAfterModificationGreaterThan to be set, also it
// cannot be used in conjunction with daysAfterLastAccessTimeGreaterThan or daysAfterCreationGreaterThan.
type DateAfterModification struct {
	...
	// Value indicating the age in days after last blob tier change time. This property is only applicable for tierToArchive actions
	// and requires daysAfterModificationGreaterThan to be set for baseBlobs based actions. The blob will be archived if both
...
type FileServiceUsages struct {
	// READ-ONLY; The FileServiceUsage items on this page
...
type FileShareItems struct {
	// READ-ONLY; The FileShareItem items on this page
	Value []*FileShareItem
...
type FileShareProperties struct {
	...
	// Provisioned v2 account type. Please refer to the GetFileServiceUsage API response for the minimum and maximum allowed value
...
type GeoReplicationStats struct {
	...
	// writes following this point in time may or may not be available for reads. Element may be default value if value of LastSyncTime
	...
	// active and operational. - Bootstrap: Indicates initial synchronization from the primary location to the secondary location
	// is in progress.This typically occurs when replication is first enabled. - Unavailable: Indicates that the secondary location
...
type IPRule struct {
	...
	// FLAG; CONSTANT; The action of IP ACL rule.
...
type ImmutabilityPolicyProperty struct {
	...
	// an append blob while maintaining immutability protection and compliance. Only new blocks can be added and any existing
	// blocks cannot be modified or deleted. This property cannot be changed with ExtendImmutabilityPolicy API.
	AllowProtectedAppendWrites *bool

	// This property can only be changed for unlocked time-based retention policies. When enabled, new blocks can be written to
	// both 'Append and Bock Blobs' while maintaining immutability protection and compliance. Only new blocks can be added and
	// any existing blocks cannot be modified or deleted. This property cannot be changed with ExtendImmutabilityPolicy API. The
...
type ImmutableStorageAccount struct {
	...
	// explicit immutability policy at the object level. The object-level immutability policy has higher precedence than the container-level
	// immutability policy, which has a higher precedence than the account-level immutability policy.
...
type ListContainerItems struct {
	// READ-ONLY; The ListContainerItem items on this page
...
type ListQueueResource struct {
	// READ-ONLY; The ListQueue items on this page
	Value []*ListQueue
...
type ListTableResource struct {
	// READ-ONLY; The Table items on this page
	Value []*Table
...
// is performed on values in the filter. If multiple filters are defined, a logical AND is performed on all filters.
...
type StaticWebsite struct {
	// REQUIRED; Indicates whether static website support is enabled for the specified account.
	Enabled *bool
...
type TaskAssignmentsList struct {
	// READ-ONLY; The StorageTaskAssignment items on this page
...
type TaskReportProperties struct {
	...
	// eq 0 and other comparison operators can be used as described for Numerical properties in https://learn.microsoft.com/rest/api/storageservices/querying-tables-and-entities#supported-comparison-operators
	...
	// objectsOperatedOnCount ge 100 and other comparison operators can be used as described for Numerical properties in https://learn.microsoft.com/rest/api/storageservices/querying-tables-and-entities#supported-comparison-operators
	...
	// comparison operators can be used as described for DateTime properties in https://learn.microsoft.com/rest/api/storageservices/querying-tables-and-entities#supported-comparison-operators
	StartTime *string
...
type TaskReportSummary struct {
	// READ-ONLY; The StorageTaskReportInstance items on this page
...
type TestConnectionResponse struct {
	// REQUIRED; Indicates the method used to validate the connection to the backing data store.
	...
	// REQUIRED; The request Id associated with the request sent to the backing data store for validation.
...
type TestExistingConnectionRequest struct {
	// REQUIRED; The uniqueId of the storage connector as returned by the server.
	UniqueID *string
}
...
type VirtualNetworkRule struct {
	...
	// FLAG; CONSTANT; The action of virtual network rule.
	// Field has constant value "Allow", any specified value is ignored.
	Action *string
