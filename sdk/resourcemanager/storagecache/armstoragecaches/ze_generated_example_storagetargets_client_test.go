...
func ExampleStorageTargetsClient_BeginDNSRefresh() {
	...
	client := armstoragecaches.NewStorageTargetsClient("<subscription-id>", cred, nil)
...
func ExampleStorageTargetsClient_BeginDelete() {
	...
	poller, err := client.BeginDelete(ctx,
		...
		"<cache-name>",
		"<storage-target-name>",
		&armstoragecaches.StorageTargetsBeginDeleteOptions{Force: nil})
	if err != nil {
		log.Fatal(err)
	}
	_, err = poller.PollUntilDone(ctx, 30*time.Second)
...
func ExampleStorageTargetsClient_BeginCreateOrUpdate() {
	...
	poller, err := client.BeginCreateOrUpdate(ctx,
		...
		"<cache-name>",
		"<storage-target-name>",
		&armstoragecaches.StorageTargetsBeginCreateOrUpdateOptions{Storagetarget: &armstoragecaches.StorageTarget{
			Properties: &armstoragecaches.StorageTargetProperties{
				Junctions: []*armstoragecaches.NamespaceJunction{
					{
				...
				Nfs3: &armstoragecaches.Nfs3Target{
					Target:     to.StringPtr("<target>"),
					UsageModel: to.StringPtr("<usage-model>"),
				},
				TargetType: armstoragecaches.StorageTargetTypeNfs3.ToPtr(),
			},
		},
		})
	if err != nil {
