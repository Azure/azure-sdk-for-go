...
// The returned WorkspaceManagedIdentitySQLControlSettingsServerTransport instance is connected to an instance of armsynapse.WorkspaceManagedIdentitySQLControlSettingsClient via the
...
func NewWorkspaceManagedIdentitySQLControlSettingsServerTransport(srv *WorkspaceManagedIdentitySQLControlSettingsServer) *WorkspaceManagedIdentitySQLControlSettingsServerTransport {
	return &WorkspaceManagedIdentitySQLControlSettingsServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[armsynapse.WorkspaceManagedIdentitySQLControlSettingsClientCreateOrUpdateResponse]](),
	}
}

// WorkspaceManagedIdentitySQLControlSettingsServerTransport connects instances of armsynapse.WorkspaceManagedIdentitySQLControlSettingsClient to instances of WorkspaceManagedIdentitySQLControlSettingsServer.
// Don't use this type directly, use NewWorkspaceManagedIdentitySQLControlSettingsServerTransport instead.
type WorkspaceManagedIdentitySQLControlSettingsServerTransport struct {
	srv                 *WorkspaceManagedIdentitySQLControlSettingsServer
	beginCreateOrUpdate *tracker[azfake.PollerResponder[armsynapse.WorkspaceManagedIdentitySQLControlSettingsClientCreateOrUpdateResponse]]
}

// Do implements the policy.Transporter interface for WorkspaceManagedIdentitySQLControlSettingsServerTransport.
func (w *WorkspaceManagedIdentitySQLControlSettingsServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	switch method {
	case "WorkspaceManagedIdentitySQLControlSettingsClient.BeginCreateOrUpdate":
		resp, err = w.dispatchBeginCreateOrUpdate(req)
	case "WorkspaceManagedIdentitySQLControlSettingsClient.Get":
...
func (w *WorkspaceManagedIdentitySQLControlSettingsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	if w.srv.BeginCreateOrUpdate == nil {
		return nil, &nonRetriableError{errors.New("fake for method BeginCreateOrUpdate not implemented")}
	}
	beginCreateOrUpdate := w.beginCreateOrUpdate.get(req)
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Synapse/workspaces/(?P<workspaceName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/managedIdentitySqlControlSettings/default`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if matches == nil || len(matches) < 3 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armsynapse.ManagedIdentitySQLControlSettingsModel](req)
		if err != nil {
			return nil, err
		}
...
func (w *WorkspaceManagedIdentitySQLControlSettingsServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	if w.srv.Get == nil {
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).ManagedIdentitySQLControlSettingsModel, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
