...
func ExampleWorkspaceAADAdminsClient_Get() {
	...
	res, err := clientFactory.NewWorkspaceAADAdminsClient().Get(ctx, "resourceGroup1", "workspace1", nil)
	if err != nil {
...
func ExampleWorkspaceAADAdminsClient_BeginCreateOrUpdate() {
	...
	poller, err := clientFactory.NewWorkspaceAADAdminsClient().BeginCreateOrUpdate(ctx, "resourceGroup1", "workspace1", armsynapse.WorkspaceAADAdminInfo{
		Properties: &armsynapse.AADAdminProperties{
			AdministratorType: to.Ptr("ActiveDirectory"),
			Login:             to.Ptr("bob@contoso.com"),
			Sid:               to.Ptr("c6b82b90-a647-49cb-8a62-0d2d3cb7ac7c"),
...
func ExampleWorkspaceAADAdminsClient_BeginDelete() {
	...
	poller, err := clientFactory.NewWorkspaceAADAdminsClient().BeginDelete(ctx, "resourceGroup1", "workspace1", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
