...
// The returned SKUsServerTransport instance is connected to an instance of armtestbase.SKUsClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewSKUsServerTransport(srv *SKUsServer) *SKUsServerTransport {
	return &SKUsServerTransport{
		srv:          srv,
		newListPager: newTracker[azfake.PagerResponder[armtestbase.SKUsClientListResponse]](),
	}
}

// SKUsServerTransport connects instances of armtestbase.SKUsClient to instances of SKUsServer.
// Don't use this type directly, use NewSKUsServerTransport instead.
type SKUsServerTransport struct {
	srv          *SKUsServer
	newListPager *tracker[azfake.PagerResponder[armtestbase.SKUsClientListResponse]]
}
...
func (s *SKUsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.TestBase/skus`
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armtestbase.SKUsClientListResponse, createLink func() string) {
