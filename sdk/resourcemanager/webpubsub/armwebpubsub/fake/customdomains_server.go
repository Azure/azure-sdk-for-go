...
// The returned CustomDomainsServerTransport instance is connected to an instance of armwebpubsub.CustomDomainsClient via the
...
func NewCustomDomainsServerTransport(srv *CustomDomainsServer) *CustomDomainsServerTransport {
	return &CustomDomainsServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[armwebpubsub.CustomDomainsClientCreateOrUpdateResponse]](),
		beginDelete:         newTracker[azfake.PollerResponder[armwebpubsub.CustomDomainsClientDeleteResponse]](),
		newListPager:        newTracker[azfake.PagerResponder[armwebpubsub.CustomDomainsClientListResponse]](),
	}
}

// CustomDomainsServerTransport connects instances of armwebpubsub.CustomDomainsClient to instances of CustomDomainsServer.
...
type CustomDomainsServerTransport struct {
	...
	beginCreateOrUpdate *tracker[azfake.PollerResponder[armwebpubsub.CustomDomainsClientCreateOrUpdateResponse]]
	beginDelete         *tracker[azfake.PollerResponder[armwebpubsub.CustomDomainsClientDeleteResponse]]
	newListPager        *tracker[azfake.PagerResponder[armwebpubsub.CustomDomainsClientListResponse]]
}
...
func (c *CustomDomainsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.SignalRService/webPubSub/(?P<resourceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customDomains/(?P<name>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 5 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armwebpubsub.CustomDomain](req)
		if err != nil {
			return nil, err
		}
...
func (c *CustomDomainsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/resourceGroups/(?P<resourceGroupName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/providers/Microsoft\.SignalRService/webPubSub/(?P<resourceName>[a-zA-Z0-9._~%!$&'()*+,;=:@-]+)/customDomains`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		server.PagerResponderInjectNextLinks(newListPager, req, func(page *armwebpubsub.CustomDomainsClientListResponse, createLink func() string) {
