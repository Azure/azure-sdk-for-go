...
func ExampleHubsClient_BeginCreateOrUpdate() {
	...
	poller, err := clientFactory.NewHubsClient().BeginCreateOrUpdate(ctx, "myResourceGroup", "myWebPubSubService", "exampleHub", armwebpubsub.Hub{
...
func ExampleHubsClient_BeginDelete() {
	...
	poller, err := clientFactory.NewHubsClient().BeginDelete(ctx, "myResourceGroup", "myWebPubSubService", "exampleHub", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	res, err := poller.PollUntilDone(ctx, nil)
	...
	_ = res
	// If the HTTP response code is 200 as defined in example definition, your response structure would look as follows. Please pay attention that all the values in the output are fake values for just demo purposes.
	// res = armwebpubsub.HubsClientDeleteResponse{
	// }
}
...
func ExampleHubsClient_Get() {
	...
	res, err := clientFactory.NewHubsClient().Get(ctx, "myResourceGroup", "myWebPubSubService", "exampleHub", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	// You could use response here. We use blank identifier for just demo purposes.
...
func ExampleHubsClient_NewListPager() {
	...
	for pager.More() {
		...
		// page = armwebpubsub.HubsClientListResponse{
