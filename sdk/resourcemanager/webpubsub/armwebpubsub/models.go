...
type CustomCertificateList struct {
	// REQUIRED; The CustomCertificate items on this page
...
type EventHandler struct {
	...
	// For example, if the template is `http://example.com/api/{event}`, when `connect` event is triggered, a POST request will
	// be sent to the URL `http://example.com/chat/api/connect`.
	// Note: Parameters are not allowed in the hostname of the URL, and curly brackets `{}` are reserved for parameter syntax
	// only. If your URL path contains literal curly brackets, please URL-encode them to ensure proper handling.
	URLTemplate *string
...
// EventHubEndpoint - An Event Hub endpoint.
...
type EventHubEndpoint struct {
	...
	// CONSTANT; Field has constant value EventListenerEndpointDiscriminatorEventHub, any specified value is ignored.
...
type EventNameFilter struct {
	// CONSTANT; Field has constant value EventListenerFilterDiscriminatorEventName, any specified value is ignored.
	Type *EventListenerFilterDiscriminator
...
type HubList struct {
	// REQUIRED; The WebPubSubHub items on this page
	Value []*Hub
...
type ManagedIdentity struct {
	...
	// READ-ONLY; Get the principal id for the system assigned identity.
	// Only be used in response.
	PrincipalID *string

	// READ-ONLY; Get the tenant id for the system assigned identity.
	// Only be used in response
	TenantID *string
}
...
// ReplicaList - Paged collection of Replica items
...
type ResourceInfoList struct {
	// REQUIRED; The WebPubSubResource items on this page
	Value []*ResourceInfo
...
type SKUList struct {
	// READ-ONLY; The URL the client should use to fetch the next page (per server side paging).
...
type SignalRServiceUsageList struct {
	// REQUIRED; The SignalRServiceUsage items on this page
...
type ThrottleByJwtCustomClaimRule struct {
	...
	// CONSTANT; Field has constant value ClientConnectionCountRuleDiscriminatorThrottleByJwtCustomClaimRule, any specified value
	// is ignored.
	Type *ClientConnectionCountRuleDiscriminator
...
type ThrottleByJwtSignatureRule struct {
	// CONSTANT; Field has constant value ClientConnectionCountRuleDiscriminatorThrottleByJwtSignatureRule, any specified value
	// is ignored.
	Type *ClientConnectionCountRuleDiscriminator
...
type ThrottleByUserIDRule struct {
	// CONSTANT; Field has constant value ClientConnectionCountRuleDiscriminatorThrottleByUserIDRule, any specified value is ignored.
	Type *ClientConnectionCountRuleDiscriminator
...
type TrafficThrottleByJwtCustomClaimRule struct {
	...
	// CONSTANT; Field has constant value ClientTrafficControlRuleDiscriminatorTrafficThrottleByJwtCustomClaimRule, any specified
	// value is ignored.
	Type *ClientTrafficControlRuleDiscriminator
	...
	// JWT claim will get disconnected if the message bytes exceeds this value. Default value is 1GB.
...
type TrafficThrottleByJwtSignatureRule struct {
	// CONSTANT; Field has constant value ClientTrafficControlRuleDiscriminatorTrafficThrottleByJwtSignatureRule, any specified
	// value is ignored.
	Type *ClientTrafficControlRuleDiscriminator
	...
	// Maximum accumulated inbound message bytes allowed for the same JWT signature within a time window. Clients with the same
	// JWT signature will get disconnected if the message bytes exceeds this value. Default value is 1GB.
	MaxInboundMessageBytes *int64
}
...
type TrafficThrottleByUserIDRule struct {
	// CONSTANT; Field has constant value ClientTrafficControlRuleDiscriminatorTrafficThrottleByUserIDRule, any specified value
	// is ignored.
	Type *ClientTrafficControlRuleDiscriminator
	...
	// ID will get disconnected if the message bytes exceeds this value. Default value is 1GB.
	MaxInboundMessageBytes *int64
}
