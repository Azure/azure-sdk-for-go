...
type DynamicSchemasServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, resourceGroupName string, schemaName string, dynamicSchemaName string, properties armworkloadorchestration.DynamicSchemaUpdate, options *armworkloadorchestration.DynamicSchemasClientUpdateOptions) (resp azfake.Responder[armworkloadorchestration.DynamicSchemasClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (d *DynamicSchemasServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.DynamicSchemaUpdate](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
