...
type DynamicSchemaVersionsServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, resourceGroupName string, schemaName string, dynamicSchemaName string, schemaVersionName string, properties armworkloadorchestration.DynamicSchemaVersionUpdate, options *armworkloadorchestration.DynamicSchemaVersionsClientUpdateOptions) (resp azfake.Responder[armworkloadorchestration.DynamicSchemaVersionsClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (d *DynamicSchemaVersionsServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.DynamicSchemaVersionUpdate](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
