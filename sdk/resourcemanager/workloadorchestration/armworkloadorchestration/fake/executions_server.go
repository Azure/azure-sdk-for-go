...
type ExecutionsServer struct {
	...
	BeginUpdate func(ctx context.Context, resourceGroupName string, contextName string, workflowName string, versionName string, executionName string, properties armworkloadorchestration.ExecutionUpdate, options *armworkloadorchestration.ExecutionsClientBeginUpdateOptions) (resp azfake.PollerResponder[armworkloadorchestration.ExecutionsClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (e *ExecutionsServerTransport) dispatchBeginUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginUpdate == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.ExecutionUpdate](req)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
