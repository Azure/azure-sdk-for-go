...
type InstancesServer struct {
	...
	BeginUpdate func(ctx context.Context, resourceGroupName string, targetName string, solutionName string, instanceName string, properties armworkloadorchestration.InstanceUpdate, options *armworkloadorchestration.InstancesClientBeginUpdateOptions) (resp azfake.PollerResponder[armworkloadorchestration.InstancesClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (i *InstancesServerTransport) dispatchBeginUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginUpdate == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.InstanceUpdate](req)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
