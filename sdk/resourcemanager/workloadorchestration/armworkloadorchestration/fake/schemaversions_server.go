...
type SchemaVersionsServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	Update func(ctx context.Context, resourceGroupName string, schemaName string, schemaVersionName string, properties armworkloadorchestration.SchemaVersionUpdate, options *armworkloadorchestration.SchemaVersionsClientUpdateOptions) (resp azfake.Responder[armworkloadorchestration.SchemaVersionsClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (s *SchemaVersionsServerTransport) dispatchUpdate(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.SchemaVersionUpdate](req)
	if err != nil {
		return nil, err
	}
	resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
