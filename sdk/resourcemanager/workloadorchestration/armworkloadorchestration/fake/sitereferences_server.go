...
type SiteReferencesServer struct {
	...
	BeginUpdate func(ctx context.Context, resourceGroupName string, contextName string, siteReferenceName string, properties armworkloadorchestration.SiteReferenceUpdate, options *armworkloadorchestration.SiteReferencesClientBeginUpdateOptions) (resp azfake.PollerResponder[armworkloadorchestration.SiteReferencesClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (s *SiteReferencesServerTransport) dispatchBeginUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginUpdate == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.SiteReferenceUpdate](req)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
