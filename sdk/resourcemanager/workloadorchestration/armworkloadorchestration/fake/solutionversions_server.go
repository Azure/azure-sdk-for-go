...
type SolutionVersionsServer struct {
	...
	BeginUpdate func(ctx context.Context, resourceGroupName string, targetName string, solutionName string, solutionVersionName string, properties armworkloadorchestration.SolutionVersionUpdate, options *armworkloadorchestration.SolutionVersionsClientBeginUpdateOptions) (resp azfake.PollerResponder[armworkloadorchestration.SolutionVersionsClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (s *SolutionVersionsServerTransport) dispatchBeginUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginUpdate == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.SolutionVersionUpdate](req)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
