...
type WorkflowsServer struct {
	...
	BeginUpdate func(ctx context.Context, resourceGroupName string, contextName string, workflowName string, properties armworkloadorchestration.WorkflowUpdate, options *armworkloadorchestration.WorkflowsClientBeginUpdateOptions) (resp azfake.PollerResponder[armworkloadorchestration.WorkflowsClientUpdateResponse], errResp azfake.ErrorResponder)
}
...
func (w *WorkflowsServerTransport) dispatchBeginUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginUpdate == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[armworkloadorchestration.WorkflowUpdate](req)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
