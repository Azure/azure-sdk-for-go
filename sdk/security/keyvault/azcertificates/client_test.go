...
func TestBackupRestore(t *testing.T) {
	...
	internal.TestSerde(t, &backup.BackupCertificateResult)
...
func TestCRUD(t *testing.T) {
	...
	certName := getName(t, "")
	createParams := azcertificates.CreateCertificateParameters{CertificatePolicy: &selfSignedPolicy}
	internal.TestSerde(t, &createParams)
	...
	internal.TestSerde(t, &getResp.Certificate)
...
func TestDisableChallengeResourceVerification(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			client, err := azcertificates.NewClient(vaultURL, &internal.FakeCredential{}, options)
			require.NoError(t, err)
...
func TestImportCertificate(t *testing.T) {
	...
	internal.TestSerde(t, &importParams)
...
func TestIssuerCRUD(t *testing.T) {
	...
	internal.TestSerde(t, &setResp.Issuer)
	...
	for pager.More() {
		...
		internal.TestSerde(t, &page.IssuerPropertiesListResult)
		require.NoError(t, err)
		for _, issuer := range page.Value {
			internal.TestSerde(t, issuer)
...
func TestListCertificates(t *testing.T) {
	...
	for listCertsPager.More() {
		...
		require.NoError(t, err)
		internal.TestSerde(t, &page.CertificatePropertiesListResult)
		for _, cert := range page.Value {
			internal.TestSerde(t, cert)
	...
	for listDeletedCertsPager.More() {
		...
		require.NoError(t, err)
		internal.TestSerde(t, &page.DeletedCertificatePropertiesListResult)
		for _, cert := range page.Value {
			internal.TestSerde(t, cert)
			if value, ok := cert.Tags[tag]; ok && *value == "yes" {
				count--
...
func TestMergeCertificate(t *testing.T) {
	...
	internal.TestSerde(t, &mergeParams)
