...
// SecureUnwrapKey - Creates a new 256 bit AES key within the trusted execution environment and wraps a symmetric key using
// a specified key.
//
...
func (client *Client) SecureUnwrapKey(ctx context.Context, name string, parameters SecureKeyUnWrapOperationParameters, options *SecureUnwrapKeyOptions) (SecureUnwrapKeyResponse, error) {
	var err error
	const operationName = "Client.SecureUnwrapKey"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.secureUnwrapKeyCreateRequest(ctx, name, parameters, options)
	if err != nil {
...
func (client *Client) secureUnwrapKeyCreateRequest(ctx context.Context, name string, parameters SecureKeyUnWrapOperationParameters, options *SecureUnwrapKeyOptions) (*policy.Request, error) {
	...
	if options != nil && options.Version != nil {
		optionalVersion = *options.Version
	}
	urlPath = strings.ReplaceAll(urlPath, "{key-version}", url.PathEscape(optionalVersion))
	req, err := runtime.NewRequest(ctx, http.MethodPost, runtime.JoinPaths(client.vaultBaseUrl, urlPath))
	if err != nil {
		return nil, err
	}
...
func (client *Client) SecureWrapKey(ctx context.Context, name string, parameters SecureKeyWrapOperationParameters, options *SecureWrapKeyOptions) (SecureWrapKeyResponse, error) {
	var err error
	const operationName = "Client.SecureWrapKey"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.secureWrapKeyCreateRequest(ctx, name, parameters, options)
	if err != nil {
...
func (client *Client) secureWrapKeyCreateRequest(ctx context.Context, name string, parameters SecureKeyWrapOperationParameters, options *SecureWrapKeyOptions) (*policy.Request, error) {
