...
func (s *BlobRecordedTestsSuite) TestBlobDownloadBufferDataLessThanCount() {
	_require := require.New(s.T())
	testName := s.T().Name()
	svcClient, err := testcommon.GetServiceClient(s.T(), testcommon.TestAccountDefault, nil)
	_require.NoError(err)
	...
	options := blob.DownloadBufferOptions{
		Range: blob.HTTPRange{
			Count: 100,
		},
	}

	newBuff := make([]byte, 100)

	value, err := bbClient.DownloadBuffer(context.Background(), newBuff, &options)
	_require.NoError(err)
	_require.Equal(value, int64(len(testcommon.BlockBlobDefaultData)))
}

func TestBlobDownloadBufferLargeData(t *testing.T) {
	_require := require.New(t)
	...
	newBuff := make([]byte, count)

	value, err := blobClient.DownloadBuffer(context.Background(), newBuff, &options)

	_require.NoError(err)
	_require.Equal(value, count)
}
