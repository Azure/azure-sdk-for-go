...
func (s *Client) GetUserDelegationCredential(ctx context.Context, info KeyInfo, userDelegationKeyOptions *GetUserDelegationKeyOptions) (*UserDelegationCredential, error) {
	url, err := blob.ParseURL(s.URL())
	if err != nil {
		return nil, err
	}

	getUserDelegationKeyOptions := userDelegationKeyOptions.format()
