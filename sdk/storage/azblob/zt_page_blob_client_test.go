...
func (s *azblobTestSuite) TestPutGetPages() {
	...
	pager := pbClient.GetPageRanges(&PageBlobGetPageRangesOptions{Range: &HttpRange{Offset: 0, Count: 1023}})
...
//  }
//  _assert.Nil(err)
//  pageListResp := resp.PageList.Range
...
//  resp, err := pbClient.GetPageRanges(ctx,NewHttpRange(0, 0), nil)
//  _assert.Nil(err)
//  pageListResp := resp.PageList.Range
...
//  defer deleteContainer(_assert, containerClient)
//
//  _, err := pbClient.ClearPages(ctx,NewHttpRange(0, PageBlobPageBytes + 1}, nil)
//  _assert.NotNil(err)
//}
//
...
//      },
//  }
//  _, err = pbClient.ClearPages(ctx,NewHttpRange(0, PageBlobPageBytes}, &clearPageOptions)
//  _assert.Nil(err)
//
...
//      },
//  }
//  _, err := pbClient.ClearPages(ctx,NewHttpRange(0, PageBlobPageBytes}, &clearPageOptions) // This will cause the library to set the value of the header to 0
//  _assert.NotNil(err)
//
//  validateStorageError(_assert, err, StorageErrorCodeInvalidInput)
//}
...
//  _assert.Nil(err)
//  _assert.NotNil(resp.Range)
//  _assert.Len(resp.Range, 1)
//  start, end := int64(0), int64(PageBlobPageBytes-1)
//  rawStart, rawEnd := (resp.Range)[0].Raw()
//  _assert.Equal(rawStart, start)
//  _assert.Equal(rawEnd, end)
//}
//
...
//  resp, err := pbClient.GetPageRanges(ctx,NewHttpRange(0, 0), nil)
//  _assert.Nil(err)
//  _assert.Nil(resp.PageList.Range)
//}
//
...
//  defer deleteContainer(_assert, containerClient)
//
//  _, err := pbClient.GetPageRanges(ctx,NewHttpRange(-2, 500}, nil)
//  _assert.Nil(err)
//}
//
...
//  defer deleteContainer(_assert, containerClient)
//
//  resp, err := pbClient.GetPageRanges(ctx,NewHttpRange(0, 2000}, nil)
//  _assert.Nil(err)
//  validateBasicGetPageRanges(_assert, resp.PageList, err)
//}
//
...
//  resp2, err := snapshotURL.GetPageRanges(ctx,NewHttpRange(0, 0), nil)
//  _assert.Nil(err)
...
//  currentTime := getRelativeTimeFromAnchor(getPropertiesResp.Date, -10)
//
//  getPageRangesOptions := PageBlobGetPageRangesOptions{
//      BlobAccessConditions: &BlobAccessConditions{
//          ModifiedAccessConditions: &ModifiedAccessConditions{
//              IfModifiedSince: &currentTime,
//          },
//      },
//  }
//  resp, err := pbClient.GetPageRanges(ctx,NewHttpRange(0, 0), &getPageRangesOptions)
//  _assert.Nil(err)
//  validateBasicGetPageRanges(_assert, resp.PageList, err)
//}
//
...
//      },
//  }
//  _, err = pbClient.GetPageRanges(ctx,NewHttpRange(0, 0), &getPageRangesOptions)
//  _assert.NotNil(err)
//
...
//      },
//  }
//  resp2, err := pbClient.GetPageRanges(ctx,NewHttpRange(0, 0), &getPageRangesOptions)
//  _assert.Nil(err)
//  validateBasicGetPageRanges(_assert, resp2.PageList, err)
//}
//
...
func setupDiffPageRangesTest(_assert *assert.Assertions, testName string) (containerClient *ContainerClient, pbClient *PageBlobClient, snapshot string) {
	...
	offset, count := int64(0), int64(PageBlobPageBytes)
	uploadPagesOptions := PageBlobUploadPagesOptions{
		Range: &HttpRange{Offset: offset, Count: count},
	}
	_, err = pbClient.UploadPages(ctx, r, &uploadPagesOptions)
	_assert.Nil(err)
...
func (s *azblobUnrecordedTestSuite) TestBlobDiffPageRangeInvalidRange() {
	...
	containerClient, pbClient, snapshot := setupDiffPageRangesTest(_assert, testName)
	defer deleteContainer(_assert, containerClient)
	pager := pbClient.GetPageRangesDiff(&PageBlobGetPageRangesDiffOptions{Range: NewHttpRange(-22, 14), Snapshot: &snapshot})
	for pager.NextPage(ctx) {
		err := pager.Err()
		_assert.Nil(err)
	}
