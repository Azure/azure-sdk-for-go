...
type FileSystemClientListPathsOptions struct {
	...
	// in the x-ms-owner, x-ms-group, and x-ms-acl response headers will be transformed from Azure Active Directory Object IDs
	// to User Principal Names. If "false", the values will be returned as Azure Active Directory Object IDs. The default value
	// is false. Note that group and application Object IDs are not translated because they do not have unique friendly names.
	Upn *bool
}
...
type PathClientAppendDataOptions struct {
	...
	// is performed with the root account encryption key.
	...
	// the lease only on flush. If "acquire-release" it will acquire & complete the operation & release the lease once operation
	// is done.
	...
	// file. To flush, the previously uploaded data must be contiguous, the position parameter must be specified and equal to
	// the length of the file after all data has been written, and there must not be a request entity body included with the request.
	Position *int64
...
type PathClientCreateOptions struct {
	...
	// is specified, the properties will overwrite the existing properties; otherwise, the existing properties will be preserved.
	// This value must be a URL percent-encoded string. Note that the string may only contain ASCII characters in the ISO-8859-1
	// character set.
	RenameSource *string
	...
	// parent folder does not have a default ACL, the umask restricts the permissions of the file or directory to be created.
	// The resulting permission is given by p bitwise and not u, where p is the permission and u is the umask. For example, if
	// p is 0777 and u is 0057, then the resulting permission is 0720. The default permission is 0777 for a directory and 0666
	// for a file. The default umask is 0027. The umask must be specified in 4-digit octal notation (e.g. 0766).
	Umask *string
}
...
type PathClientDeleteOptions struct {
	...
	// and Delete in an atomic operation once the ACL checks are completed. If false or missing, normal default behavior will
	// kick in, which may timeout in case of very large directories due to recursive ACL checks. This new parameter is introduced
...
type PathClientFlushDataOptions struct {
	...
	// flush operation completes successfully, the service raises a file change notification with a property indicating that this
	// is the final update (the file stream has been closed). If "false" a change notification is raised indicating the file has
	// changed. The default is false. This query parameter is set to true by the Hadoop ABFS driver to indicate that the file
...
type PathClientGetPropertiesOptions struct {
	...
	// the access control list is returned in the response headers (Hierarchical Namespace must be enabled for the account), otherwise
...
type PathClientSetAccessControlRecursiveOptions struct {
	...
	// user errors (4XX). If true, the operation will ignore user errors and proceed with the operation on other sub-entities
	// of the directory. Continuation token will only be returned when forceFlag is true in case of user errors. If not set the
...
type PathClientUpdateOptions struct {
	...
	// this limit, a continuation token is returned in the response header x-ms-continuation. When a continuation token is returned
	// in the response, it must be percent-encoded and specified in a subsequent invocation of setAccessControlRecursive operation.
	Continuation *string
	...
	// the acl change will be applied. If omitted or greater than 2,000, the request will process up to 2,000 items.
	MaxRecords *int32
