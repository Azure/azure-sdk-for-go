...
func TestFileUploadRangeStructuredMessageBody(t *testing.T) {
	...
	encoder := storageinternal.NewSMEncoder(streaming.NopCloser(bytes.NewReader(content)), int64(contentSize), segmentSize)
	...
	_require.Equal(storageinternal.SMHeaderValue, sbHeader[0])
