...
func (s *RecordedTestSuite) TestServiceListQueuesPaged() {
	...
	for pager.More() {
		resp, err := pager.NextPage(context.Background())
		_require.Nil(err)
		for _, ctnr := range resp.QueuesList {
...
func (s *UnrecordedTestSuite) TestSASQueueClient() {
	_require := require.New(s.T())
	testName := s.T().Name()
	accountName := os.Getenv("AZURE_STORAGE_ACCOUNT_NAME")
	accountKey := os.Getenv("AZURE_STORAGE_ACCOUNT_KEY")
...
func (s *UnrecordedTestSuite) TestSASQueueClient2() {
	_require := require.New(s.T())
	testName := s.T().Name()
	accountName := os.Getenv("AZURE_STORAGE_ACCOUNT_NAME")
	accountKey := os.Getenv("AZURE_STORAGE_ACCOUNT_KEY")
	...
	sasUrlRCWL, err := queueClient.GetSASURL(sas.QueuePermissions{Add: true, Read: true, Update: true, Process: true}, time.Now().Add(time.Hour), &opts)
	_require.Nil(err)

	queueClient2, err := azqueue.NewQueueClientWithNoCredential(sasUrlRCWL, nil)
	_require.Nil(err)

	// queues can't be created, deleted, or listed with SAS auth
