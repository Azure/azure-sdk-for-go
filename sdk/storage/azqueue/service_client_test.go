...
// TestServiceClientGetSASURLPreservesCustomQueryParams is a regression test for GetSASURL()
...
func TestServiceClientGetSASURLPreservesCustomQueryParams(t *testing.T) {
	...
	serviceURL := fmt.Sprintf("https://%s.queue.core.windows.net/?customparam=value", accountName)
	svcClient, err := azqueue.NewServiceClientWithSharedKeyCredential(serviceURL, cred, nil)
	_require.NoError(err)
...
// TestServiceClientGetSASURLNoTrailingSlashBeforeQuery is a regression test for GetSASURL()
// corrupting the query string when the client's URL has a query string but no trailing slash
// after the account path (e.g. https://acct.queue.core.windows.net?customparam=value). The
// trailing slash added for consistency with the portal must be appended to the account path,
// not to the query string value.
func TestServiceClientGetSASURLNoTrailingSlashBeforeQuery(t *testing.T) {
	_require := require.New(t)
	const accountName = "fakestorageaccount"
	// base64-encoded fake key; not a real secret.
	const accountKey = "PSA7dl59RwZBFEBhBEtdrsq/g7VpjMFeSPzdC4SoBiQI3xVLg2y8HRoAF3PidfB8/i9v67QCNSAdVdJdKrmqSw=="
	...
	_require.True(strings.HasPrefix(sasURL, fmt.Sprintf("https://%s.queue.core.windows.net/?", accountName)), "trailing slash must be appended to the account path: %s", sasURL)
}
