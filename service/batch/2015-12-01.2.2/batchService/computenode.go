...
// is specifies the user account to be created. timeout is sets the maximum
...
// the name of the user account to delete. timeout is sets the maximum time
// that the server can spend processing the request, in seconds. The default is
// 30 seconds. clientRequestID is caller generated request identity, in the
...
// file. timeout is sets the maximum time that the server can spend processing
// the request, in seconds. The default is 30 seconds. clientRequestID is
...
//
// poolID is the id of the pool from which you want to list nodes. filter is
// sets an OData $filter clause. selectParameter is sets an OData $select
// clause. maxResults is sets the maximum number of items to return in the
// response. timeout is sets the maximum time that the server can spend
...
// parameters for the request. timeout is sets the maximum time that the server
...
func (client ComputeNodeClient) UpdateUser(poolID string, nodeID string, userName string, nodeUpdateUserParameter NodeUpdateUserParameter, timeout *int32, clientRequestID string, returnClientRequestID *bool, ocpDate *date.TimeRFC1123) (result autorest.Response, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewErrorWithValidationError(err, "batchservice.ComputeNodeClient", "UpdateUser")
	}
