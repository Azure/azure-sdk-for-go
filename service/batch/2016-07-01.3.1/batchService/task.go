...
// may not return the results for each task in the same order the tasks were
...
// re-issuing a request. For example, you should use the same task ids during a
// retry so that if the prior operation succeeded, the retry will not create
// extra tasks unexpectedly.
//
...
// synchronously to the primary task; subtasks and their files are then deleted
...
// A maximum of 1000 tasks can be returned. timeout is the maximum time that
// the server can spend processing the request, in seconds. The default is 30
// seconds. clientRequestID is the caller-generated request identity, in the
// form of a GUID with no decoration such as curly braces, e.g.
// 9C4D50EE-2D56-4CD3-8152-34347DC9F2B0. returnClientRequestID is whether the
...
func (client TaskClient) ListComplete(jobID string, filter string, selectParameter string, expand string, maxResults *int32, timeout *int32, clientRequestID string, returnClientRequestID *bool, ocpDate *date.TimeRFC1123, cancel <-chan struct{}) (<-chan CloudTask, <-chan error) {
	resultChan := make(chan CloudTask)
	...
	go func() {
		...
		list, err := client.List(jobID, filter, selectParameter, expand, maxResults, timeout, clientRequestID, returnClientRequestID, ocpDate)
		if err != nil {
			errChan <- err
			return
		}
...
// maximum retry count. The task's state is changed to active. As the task is
// no longer in the completed state, any previous exit code or scheduling error
// is no longer available after reactivation. This will fail for tasks that are
// not completed or that previously completed successfully (with an exit code
// of 0). Additionally, this will fail if the job has completed (or is
// terminating or deleting).
//
// jobID is the ID of the job containing the task. taskID is the ID of the task
// to reactivate. timeout is the maximum time that the server can spend
// processing the request, in seconds. The default is 30 seconds.
// clientRequestID is the caller-generated request identity, in the form of a
// GUID with no decoration such as curly braces, e.g.
// 9C4D50EE-2D56-4CD3-8152-34347DC9F2B0. returnClientRequestID is whether the
...
// state. For multi-instance tasks, the terminate task operation applies
