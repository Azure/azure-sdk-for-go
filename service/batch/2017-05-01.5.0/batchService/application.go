...
func (client ApplicationClient) ListComplete(maxResults *int32, timeout *int32, clientRequestID *uuid.UUID, returnClientRequestID *bool, ocpDate *date.TimeRFC1123, cancel <-chan struct{}) (<-chan ApplicationSummary, <-chan error) {
	resultChan := make(chan ApplicationSummary)
	...
	go func() {
		...
		list, err := client.List(maxResults, timeout, clientRequestID, returnClientRequestID, ocpDate)
		if err != nil {
			errChan <- err
			return
		}
