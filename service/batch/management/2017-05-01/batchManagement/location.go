...
// LocationClient is the client for the Location methods of the Batchmanagement
// service.
...
// CheckNameAvailability checks whether the Batch account name is available in
// the specified region.
//
// locationName is the desired region for the name check. parameters is
...
func (client LocationClient) CheckNameAvailability(locationName string, parameters CheckNameAvailabilityParameters) (result CheckNameAvailabilityResult, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewErrorWithValidationError(err, "batchmanagement.LocationClient", "CheckNameAvailability")
	}

	req, err := client.CheckNameAvailabilityPreparer(locationName, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "batchmanagement.LocationClient", "CheckNameAvailability", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "batchmanagement.LocationClient", "CheckNameAvailability", resp, "Failure sending request")
		return
	}

	result, err = client.CheckNameAvailabilityResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "batchmanagement.LocationClient", "CheckNameAvailability", resp, "Failure responding to request")
	}
...
func (client LocationClient) GetQuotas(locationName string) (result BatchLocationQuota, err error) {
	req, err := client.GetQuotasPreparer(locationName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "batchmanagement.LocationClient", "GetQuotas", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "batchmanagement.LocationClient", "GetQuotas", resp, "Failure sending request")
		return
	}

	result, err = client.GetQuotasResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "batchmanagement.LocationClient", "GetQuotas", resp, "Failure responding to request")
	}
...
// closes the http.Response Body.
func (client LocationClient) GetQuotasResponder(resp *http.Response) (result BatchLocationQuota, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
