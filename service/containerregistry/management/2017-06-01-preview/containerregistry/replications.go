...
// ReplicationsClient is the client for the Replications methods of the
...
// NewReplicationsClientWithBaseURI creates an instance of the
...
// CreateOrUpdate creates or updates a replication for a container registry
// with the specified parameters. This method may poll for completion. Polling
...
// replicationName is the name of the replication. replication is the
// parameters for creating or updating a replication.
func (client ReplicationsClient) CreateOrUpdate(resourceGroupName string, registryName string, replicationName string, replication Replication, cancel <-chan struct{}) (<-chan Replication, <-chan error) {
	...
	if err := validation.Validate([]validation.Validation{
		...
		errChan <- validation.NewErrorWithValidationError(err, "containerregistry.ReplicationsClient", "CreateOrUpdate")
		close(errChan)
		close(resultChan)
	...
	go func() {
		...
		req, err := client.CreateOrUpdatePreparer(resourceGroupName, registryName, replicationName, replication, cancel)
		if err != nil {
			err = autorest.NewErrorWithError(err, "containerregistry.ReplicationsClient", "CreateOrUpdate", nil, "Failure preparing request")
			return
		}
		...
		if err != nil {
			...
			err = autorest.NewErrorWithError(err, "containerregistry.ReplicationsClient", "CreateOrUpdate", resp, "Failure sending request")
			return
		}

		result, err = client.CreateOrUpdateResponder(resp)
		if err != nil {
			err = autorest.NewErrorWithError(err, "containerregistry.ReplicationsClient", "CreateOrUpdate", resp, "Failure responding to request")
		}
	}()
	return resultChan, errChan
}
...
func (client ReplicationsClient) CreateOrUpdatePreparer(resourceGroupName string, registryName string, replicationName string, replication Replication, cancel <-chan struct{}) (*http.Request, error) {
...
func (client ReplicationsClient) CreateOrUpdateSender(req *http.Request) (*http.Response, error) {
...
func (client ReplicationsClient) CreateOrUpdateResponder(resp *http.Response) (result Replication, err error) {
...
// Delete deletes a replication from a container registry. This method may poll
...
// resourceGroupName is the name of the resource group to which the container
// registry belongs. registryName is the name of the container registry.
// replicationName is the name of the replication.
