...
// NewArtifactsClientWithBaseURI creates an instance of the ArtifactsClient
// client.
...
// uploads the required files to a storage account, and validates the generated
// artifact.
//
...
// name of the artifact. generateArmTemplateRequest is parameters for
...
// resourceGroupName is the name of the resource group. labName is the name of
// the lab. artifactSourceName is the name of the artifact source. name is the
// name of the artifact. expand is specify the $expand query. Example:
...
// specify the $expand query. Example: 'properties($select=title)' filter is
...
func (client ArtifactsClient) ListNextResults(lastResults ResponseWithContinuationArtifact) (result ResponseWithContinuationArtifact, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "dtl.ArtifactsClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "dtl.ArtifactsClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dtl.ArtifactsClient", "List", resp, "Failure responding to next results request")
	}
