...
// ServersClient is the the Microsoft Azure management API provides create,
// read, update, and delete functionality for Azure MySQL resources including
// servers, databases, firewall rules, log files and configurations.
...
// CreateOrUpdate creates a new server or updates an existing server. The
// update action will overwrite the existing server. This method may poll for
...
// Delete deletes a server. This method may poll for completion. Polling can be
...
// Update updates an existing server. The request body can contain one to many
// of the properties present in the normal server definition. This method may
