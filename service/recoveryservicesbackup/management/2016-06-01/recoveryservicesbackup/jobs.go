...
// JobsClient is the client for the Jobs methods of the Recoveryservicesbackup
// service.
...
// Export exports all jobs for a given Shared Access Signatures (SAS) URL. The
// SAS URL expires within 15 minutes of its creation.
//
...
// filter is the OData filter options. status eq { InProgress , Completed ,
// Failed , CompletedWithWarnings , Cancelled , Cancelling } and
...
// DeleteBackupData } and jobId eq {guid} and startTime eq { yyyy-mm-dd
// hh:mm:ss PM } and endTime eq { yyyy-mm-dd hh:mm:ss PM }.
...
// vaultName is the name of the Recovery Services vault. resourceGroupName is
// the name of the resource group associated with the Recovery Services vault.
// filter is the following equation can be used to filter the list of jobs
// based on status, type, start date, and end date. status eq { InProgress ,
// Completed , Failed , CompletedWithWarnings , Cancelled , Cancelling } and
// backupManagementType eq {AzureIaasVM, MAB, DPM, AzureBackupServer, AzureSql
// } and operation eq { ConfigureBackup , Backup , Restore , DisableBackup ,
// DeleteBackupData } and jobId eq {guid} and startTime eq { yyyy-mm-dd
// hh:mm:ss PM } and endTime eq { yyyy-mm-dd hh:mm:ss PM }. skipToken is the
// Skip Token filter.
func (client JobsClient) List(vaultName string, resourceGroupName string, filter string, skipToken string) (result JobResourceList, err error) {
	req, err := client.ListPreparer(vaultName, resourceGroupName, filter, skipToken)
	if err != nil {
		err = autorest.NewErrorWithError(err, "recoveryservicesbackup.JobsClient", "List", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "recoveryservicesbackup.JobsClient", "List", resp, "Failure sending request")
		return
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "recoveryservicesbackup.JobsClient", "List", resp, "Failure responding to request")
	}
...
func (client JobsClient) ListPreparer(vaultName string, resourceGroupName string, filter string, skipToken string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
		"vaultName":         autorest.Encode("path", vaultName),
...
func (client JobsClient) ListNextResults(lastResults JobResourceList) (result JobResourceList, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "recoveryservicesbackup.JobsClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "recoveryservicesbackup.JobsClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "recoveryservicesbackup.JobsClient", "List", resp, "Failure responding to next results request")
	}
...
func (client JobsClient) ListComplete(vaultName string, resourceGroupName string, filter string, skipToken string, cancel <-chan struct{}) (<-chan JobResource, <-chan error) {
