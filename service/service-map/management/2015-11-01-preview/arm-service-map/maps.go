...
func (client MapsClient) Generate(resourceGroupName string, workspaceName string, request MapRequest) (result MapResponse, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewErrorWithValidationError(err, "armservicemap.MapsClient", "Generate")
	}

	req, err := client.GeneratePreparer(resourceGroupName, workspaceName, request)
	if err != nil {
		err = autorest.NewErrorWithError(err, "armservicemap.MapsClient", "Generate", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "armservicemap.MapsClient", "Generate", resp, "Failure sending request")
		return
	}

	result, err = client.GenerateResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "armservicemap.MapsClient", "Generate", resp, "Failure responding to request")
	}
