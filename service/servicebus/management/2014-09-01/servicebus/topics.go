...
func (client TopicsClient) ListAllComplete(resourceGroupName string, namespaceName string, cancel <-chan struct{}) (<-chan TopicResource, <-chan error) {
	resultChan := make(chan TopicResource)
	...
	go func() {
		...
		list, err := client.ListAll(resourceGroupName, namespaceName)
		if err != nil {
			errChan <- err
			return
		}
...
func (client TopicsClient) ListAuthorizationRulesComplete(resourceGroupName string, namespaceName string, topicName string, cancel <-chan struct{}) (<-chan SharedAccessAuthorizationRuleResource, <-chan error) {
...
// UpdateAuthorizationRule creates an authorizatio rule for the specified
// topic.
//
// resourceGroupName is name of the Resource group within the Azure
// subscription. namespaceName is the namespace name topicName is the topic
// name. authorizationRuleName is the authorizationrule name. parameters is the
// shared access authorization rule.
func (client TopicsClient) UpdateAuthorizationRule(resourceGroupName string, namespaceName string, topicName string, authorizationRuleName string, parameters SharedAccessAuthorizationRuleCreateOrUpdateParameters) (result SharedAccessAuthorizationRuleResource, err error) {
	if err := validation.Validate([]validation.Validation{
		{TargetValue: resourceGroupName,
			Constraints: []validation.Constraint{{Target: "resourceGroupName", Name: validation.MaxLength, Rule: 90, Chain: nil},
				{Target: "resourceGroupName", Name: validation.MinLength, Rule: 1, Chain: nil}}},
		...
		{TargetValue: parameters,
			Constraints: []validation.Constraint{{Target: "parameters.SharedAccessAuthorizationRuleProperties", Name: validation.Null, Rule: false,
				Chain: []validation.Constraint{{Target: "parameters.SharedAccessAuthorizationRuleProperties.Rights", Name: validation.Null, Rule: true, Chain: nil}}}}}}); err != nil {
		return result, validation.NewErrorWithValidationError(err, "servicebus.TopicsClient", "UpdateAuthorizationRule")
	}

	req, err := client.UpdateAuthorizationRulePreparer(resourceGroupName, namespaceName, topicName, authorizationRuleName, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "servicebus.TopicsClient", "UpdateAuthorizationRule", nil, "Failure preparing request")
		return
	}

	resp, err := client.UpdateAuthorizationRuleSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "servicebus.TopicsClient", "UpdateAuthorizationRule", resp, "Failure sending request")
		return
	}

	result, err = client.UpdateAuthorizationRuleResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "servicebus.TopicsClient", "UpdateAuthorizationRule", resp, "Failure responding to request")
	}

	return
}

// UpdateAuthorizationRulePreparer prepares the UpdateAuthorizationRule request.
func (client TopicsClient) UpdateAuthorizationRulePreparer(resourceGroupName string, namespaceName string, topicName string, authorizationRuleName string, parameters SharedAccessAuthorizationRuleCreateOrUpdateParameters) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"authorizationRuleName": autorest.Encode("path", authorizationRuleName),
		"namespaceName":         autorest.Encode("path", namespaceName),
		"resourceGroupName":     autorest.Encode("path", resourceGroupName),
...
// UpdateAuthorizationRuleSender sends the UpdateAuthorizationRule request. The method will close the
// http.Response Body if it receives an error.
func (client TopicsClient) UpdateAuthorizationRuleSender(req *http.Request) (*http.Response, error) {
	return autorest.SendWithSender(client, req)
}

// UpdateAuthorizationRuleResponder handles the response to the UpdateAuthorizationRule request. The method always
// closes the http.Response Body.
func (client TopicsClient) UpdateAuthorizationRuleResponder(resp *http.Response) (result SharedAccessAuthorizationRuleResource, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
