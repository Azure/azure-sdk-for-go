...
func (client DatabaseAdvisorsClient) CreateOrUpdate(resourceGroupName string, serverName string, databaseName string, advisorName string, parameters Advisor) (result Advisor, err error) {
	req, err := client.CreateOrUpdatePreparer(resourceGroupName, serverName, databaseName, advisorName, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "advisors.DatabaseAdvisorsClient", "CreateOrUpdate", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "advisors.DatabaseAdvisorsClient", "CreateOrUpdate", resp, "Failure sending request")
		return
	}

	result, err = client.CreateOrUpdateResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "advisors.DatabaseAdvisorsClient", "CreateOrUpdate", resp, "Failure responding to request")
	}
...
func (client DatabaseAdvisorsClient) CreateOrUpdatePreparer(resourceGroupName string, serverName string, databaseName string, advisorName string, parameters Advisor) (*http.Request, error) {
...
// the portal. serverName is the name of the server. databaseName is the name
// of the database.
func (client DatabaseAdvisorsClient) ListByDatabase(resourceGroupName string, serverName string, databaseName string) (result ListResult, err error) {
...
// closes the http.Response Body.
func (client DatabaseAdvisorsClient) ListByDatabaseResponder(resp *http.Response) (result ListResult, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
