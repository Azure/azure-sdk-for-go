...
// ServerConnectionPoliciesClient is the the Azure SQL Server management API
// provides a RESTful set of web services that interact with Azure SQL Server
// services to manage your databases. The API enables users update server
// connection policy.
type ServerConnectionPoliciesClient struct {
	ManagementClient
}

// NewServerConnectionPoliciesClient creates an instance of the
// ServerConnectionPoliciesClient client.
...
// NewServerConnectionPoliciesClientWithBaseURI creates an instance of the
// ServerConnectionPoliciesClient client.
...
// the portal. serverName is the name of the server. connectionPolicyName is
// the name of the connection policy. parameters is the required parameters for
...
func (client ServerConnectionPoliciesClient) CreateOrUpdate(resourceGroupName string, serverName string, connectionPolicyName string, parameters ServerConnectionPolicy) (result ServerConnectionPolicy, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "connectionpolicies.ServerConnectionPoliciesClient", "CreateOrUpdate", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "connectionpolicies.ServerConnectionPoliciesClient", "CreateOrUpdate", resp, "Failure sending request")
		return
	}

	result, err = client.CreateOrUpdateResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "connectionpolicies.ServerConnectionPoliciesClient", "CreateOrUpdate", resp, "Failure responding to request")
	}
...
func (client ServerConnectionPoliciesClient) Get(resourceGroupName string, serverName string, connectionPolicyName string) (result ServerConnectionPolicy, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "connectionpolicies.ServerConnectionPoliciesClient", "Get", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "connectionpolicies.ServerConnectionPoliciesClient", "Get", resp, "Failure sending request")
		return
	}

	result, err = client.GetResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "connectionpolicies.ServerConnectionPoliciesClient", "Get", resp, "Failure responding to request")
	}
