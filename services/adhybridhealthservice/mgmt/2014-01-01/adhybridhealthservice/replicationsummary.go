...
// not. query is the custom query. nextPartitionKey is the next partition key to query for. filter is the server
// property filter to apply. nextRowKey is the next row key to query for. takeCount is the take count , which
