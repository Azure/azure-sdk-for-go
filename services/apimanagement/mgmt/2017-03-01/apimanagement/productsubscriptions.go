...
func (client ProductSubscriptionsClient) List(resourceGroupName string, serviceName string, productID string, filter string, top *int32, skip *int32) (result SubscriptionCollection, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewErrorWithValidationError(err, "apimanagement.ProductSubscriptionsClient", "List")
	}

	req, err := client.ListPreparer(resourceGroupName, serviceName, productID, filter, top, skip)
	if err != nil {
...
func (client ProductSubscriptionsClient) ListPreparer(resourceGroupName string, serviceName string, productID string, filter string, top *int32, skip *int32) (*http.Request, error) {
...
func (client ProductSubscriptionsClient) ListNextResults(lastResults SubscriptionCollection) (result SubscriptionCollection, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "apimanagement.ProductSubscriptionsClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "apimanagement.ProductSubscriptionsClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "apimanagement.ProductSubscriptionsClient", "List", resp, "Failure responding to next results request")
	}
...
func (client ProductSubscriptionsClient) ListComplete(resourceGroupName string, serviceName string, productID string, filter string, top *int32, skip *int32, cancel <-chan struct{}) (<-chan SubscriptionContract, <-chan error) {
	...
	go func() {
		...
		list, err := client.List(resourceGroupName, serviceName, productID, filter, top, skip)
		if err != nil {
			errChan <- err
			return
		}
