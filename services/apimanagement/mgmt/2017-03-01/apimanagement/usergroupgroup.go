...
func (client UserGroupClient) List(resourceGroupName string, serviceName string, UID string, filter string, top *int32, skip *int32) (result GroupCollection, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewErrorWithValidationError(err, "apimanagement.UserGroupClient", "List")
	}

	req, err := client.ListPreparer(resourceGroupName, serviceName, UID, filter, top, skip)
	if err != nil {
...
func (client UserGroupClient) ListPreparer(resourceGroupName string, serviceName string, UID string, filter string, top *int32, skip *int32) (*http.Request, error) {
...
func (client UserGroupClient) ListNextResults(lastResults GroupCollection) (result GroupCollection, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "apimanagement.UserGroupClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "apimanagement.UserGroupClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "apimanagement.UserGroupClient", "List", resp, "Failure responding to next results request")
	}
...
func (client UserGroupClient) ListComplete(resourceGroupName string, serviceName string, UID string, filter string, top *int32, skip *int32, cancel <-chan struct{}) (<-chan GroupContract, <-chan error) {
	...
	go func() {
		...
		list, err := client.List(resourceGroupName, serviceName, UID, filter, top, skip)
		if err != nil {
			errChan <- err
			return
		}
