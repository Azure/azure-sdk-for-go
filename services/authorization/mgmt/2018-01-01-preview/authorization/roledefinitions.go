...
func NewRoleDefinitionsClient(subscriptionID string, resourceGroupName string, resourceProviderNamespace string) RoleDefinitionsClient {
	return NewRoleDefinitionsClientWithBaseURI(DefaultBaseURI, subscriptionID, resourceGroupName, resourceProviderNamespace)
}

// NewRoleDefinitionsClientWithBaseURI creates an instance of the RoleDefinitionsClient client.
func NewRoleDefinitionsClientWithBaseURI(baseURI string, subscriptionID string, resourceGroupName string, resourceProviderNamespace string) RoleDefinitionsClient {
	return RoleDefinitionsClient{NewWithBaseURI(baseURI, subscriptionID, resourceGroupName, resourceProviderNamespace)}
}
...
// GetByID1 gets a role definition by ID.
//
// roleDefinitionID is the fully qualified role definition ID. Use the format,
// /subscriptions/{guid}/providers/Microsoft.Authorization/roleDefinitions/{roleDefinitionId} for subscription level
// role definitions, or /providers/Microsoft.Authorization/roleDefinitions/{roleDefinitionId} for tenant level role
// definitions.
func (client RoleDefinitionsClient) GetByID1(ctx context.Context, roleDefinitionID string) (result RoleDefinition, err error) {
	req, err := client.GetByID1Preparer(ctx, roleDefinitionID)
	if err != nil {
		err = autorest.NewErrorWithError(err, "authorization.RoleDefinitionsClient", "GetByID1", nil, "Failure preparing request")
		return
	}

	resp, err := client.GetByID1Sender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "authorization.RoleDefinitionsClient", "GetByID1", resp, "Failure sending request")
		return
	}

	result, err = client.GetByID1Responder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "authorization.RoleDefinitionsClient", "GetByID1", resp, "Failure responding to request")
	}

	return
}

// GetByID1Preparer prepares the GetByID1 request.
func (client RoleDefinitionsClient) GetByID1Preparer(ctx context.Context, roleDefinitionID string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"roleDefinitionId": roleDefinitionID,
	}
...
// GetByID1Sender sends the GetByID1 request. The method will close the
// http.Response Body if it receives an error.
func (client RoleDefinitionsClient) GetByID1Sender(req *http.Request) (*http.Response, error) {
	return autorest.SendWithSender(client, req,
		autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
}

// GetByID1Responder handles the response to the GetByID1 request. The method always
// closes the http.Response Body.
func (client RoleDefinitionsClient) GetByID1Responder(resp *http.Response) (result RoleDefinition, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
