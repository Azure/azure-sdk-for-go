...
//
// automationAccountName is the name of the automation account. jobName is the job name. jobStreamID is the job
// stream id. clientRequestID is identifies this specific client request.
func (client JobStreamClient) Get(ctx context.Context, automationAccountName string, jobName string, jobStreamID string, clientRequestID string) (result JobStream, err error) {
	...
	req, err := client.GetPreparer(ctx, automationAccountName, jobName, jobStreamID, clientRequestID)
	if err != nil {
...
func (client JobStreamClient) GetPreparer(ctx context.Context, automationAccountName string, jobName string, jobStreamID string, clientRequestID string) (*http.Request, error) {
...
//
// automationAccountName is the name of the automation account. jobName is the job name. filter is the filter to
// apply on the operation. clientRequestID is identifies this specific client request.
func (client JobStreamClient) ListByJob(ctx context.Context, automationAccountName string, jobName string, filter string, clientRequestID string) (result JobStreamListResultPage, err error) {
	if err := validation.Validate([]validation.Validation{
		{TargetValue: client.ResourceGroupName,
			Constraints: []validation.Constraint{{Target: "client.ResourceGroupName", Name: validation.Pattern, Rule: `^[-\w\._]+$`, Chain: nil}}}}); err != nil {
	...
	req, err := client.ListByJobPreparer(ctx, automationAccountName, jobName, filter, clientRequestID)
	if err != nil {
...
func (client JobStreamClient) ListByJobPreparer(ctx context.Context, automationAccountName string, jobName string, filter string, clientRequestID string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"automationAccountName": autorest.Encode("path", automationAccountName),
		"jobName":               autorest.Encode("path", jobName),
		"resourceGroupName":     autorest.Encode("path", client.ResourceGroupName),
...
func (client JobStreamClient) ListByJobComplete(ctx context.Context, automationAccountName string, jobName string, filter string, clientRequestID string) (result JobStreamListResultIterator, err error) {
	result.page, err = client.ListByJob(ctx, automationAccountName, jobName, filter, clientRequestID)
	return
}
