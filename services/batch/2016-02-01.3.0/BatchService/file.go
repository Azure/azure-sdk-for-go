...
// the path to the file that you want to delete. recursive is whether to delete
// children of a directory. If the fileName parameter represents a directory
// instead of a file, you can set Recursive to true to delete the directory and
// all of the files and subdirectories in it. If Recursive is false then the
// directory must be empty or deletion will fail. timeout is the maximum time
...
// that you want to delete. recursive is whether to delete children of a
...
// must be empty or deletion will fail. timeout is the maximum time that the
...
// task file that you want to get the content of. timeout is the maximum time
// that the server can spend processing the request, in seconds. The default is
// 30 seconds. clientRequestID is the caller-generated request identity, in the
// form of a GUID with no decoration such as curly braces, e.g.
// 9C4D50EE-2D56-4CD3-8152-34347DC9F2B0. returnClientRequestID is whether the
...
// is the byte range to be retrieved. The default is to retrieve the entire
// file. The format is startRange-endRange. ifModifiedSince is specify this
// header to perform the operation only if the resource has been modified since
// the specified date/time. ifUnmodifiedSince is specify this header to perform
// the operation only if the resource has not been modified since the specified
// date/time.
...
// that you want to get the content of. timeout is the maximum time that the
// server can spend processing the request, in seconds. The default is 30
// seconds. clientRequestID is the caller-generated request identity, in the
// form of a GUID with no decoration such as curly braces, e.g.
// 9C4D50EE-2D56-4CD3-8152-34347DC9F2B0. returnClientRequestID is whether the
...
// poolID is the id of the pool that contains the compute node. nodeID is the
// id of the compute node that contains the file. fileName is the path to the
// compute node file that you want to get the properties of. timeout is the
...
// the task file that you want to get the properties of. timeout is the maximum
...
//
// poolID is the id of the pool that contains the compute node. nodeID is the
// id of the compute node whose files you want to list. filter is an OData
// $filter clause. recursive is whether to list children of a directory.
// maxResults is the maximum number of items to return in the response. timeout
...
//
// jobID is the id of the job that contains the task. taskID is the id of the
// task whose files you want to list. filter is an OData $filter clause.
// recursive is whether to list children of a directory. maxResults is the
