...
// CancelDeletion if you try to delete a Certificate that is being used by a pool or Node, the status of the
...
// existing Nodes in that pool until they restart), and no running Tasks depend on the Certificate. If you try to
// delete a Certificate that is in use, the deletion fails. The Certificate status changes to deleteFailed. You can use
// Cancel Delete Certificate to set the status back to active if you decide that you want to continue using the
// Certificate.
// Parameters:
// thumbprintAlgorithm - the algorithm used to derive the thumbprint parameter. This must be sha1.
