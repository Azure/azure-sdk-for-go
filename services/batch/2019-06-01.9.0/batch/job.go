...
// the retention period for Task data; that is, if the Job contains Tasks which are still retained on Nodes, the Batch
// services deletes those Tasks' working directories and all their contents.  When a Delete Job request is received,
// the Batch service sets the Job to the deleting state. All update operations on a Job that is in deleting state will
// fail with status code 409 (Conflict), with additional information indicating that the Job is being deleted.
// Parameters:
...
// ListPreparationAndReleaseTaskStatus this API returns the Job Preparation and Job Release Task status on all Nodes
// that have run the Job Preparation or Job Release Task. This includes nodes which have since been removed from the
// pool. If this API is invoked on a Job which has no Job Preparation or Job Release Task, the Batch service returns
// HTTP status code 409 (Conflict) with an error code of JobPreparationTaskNotSpecified.
// Parameters:
// jobID - the ID of the Job.
// filter - an OData $filter clause. For more information on constructing this filter, see
