...
// Add](https://docs.microsoft.com/rest/api/faceapi/largefacelist/addfacefromurl) to import the faces and
// [LargeFaceList - Train](https://docs.microsoft.com/rest/api/faceapi/largefacelist/train) to make it ready for [Face
// - Find Similar](https://docs.microsoft.com/rest/api/faceapi/face/findsimilar). No image will be stored. Only the
// extracted face features are stored on server until [LargeFaceList -
// Delete](https://docs.microsoft.com/rest/api/faceapi/largefacelist/delete) is called.
...
// extracted by another version of recognition model. Please refer to [Specify a recognition
...
// returnRecognitionModel - a value indicating whether the operation should return 'recognitionModel' in
// response.
// start - starting large face list id to return (used to list a range of large face lists).
// top - number of large face lists to return starting with the large face list id indicated by the 'start'
// parameter.
func (client LargeFaceListClient) List(ctx context.Context, returnRecognitionModel *bool, start string, top *int32) (result ListLargeFaceList, err error) {
	if tracing.IsEnabled() {
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("face.LargeFaceListClient", "List", err.Error())
	}

	req, err := client.ListPreparer(ctx, returnRecognitionModel, start, top)
	if err != nil {
...
func (client LargeFaceListClient) ListPreparer(ctx context.Context, returnRecognitionModel *bool, start string, top *int32) (*http.Request, error) {
	urlParameters := map[string]interface{}{
		"Endpoint": client.Endpoint,
	}
