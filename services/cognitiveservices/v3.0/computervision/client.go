...
// Parameters:
// imageURL - a JSON document with a URL pointing to the image that is to be analyzed.
// language - the BCP-47 language code of the text in the document. Currently, only English ('en'), Dutch
// (‘nl’), French (‘fr’), German (‘de’), Italian (‘it’), Portuguese (‘pt), and Spanish ('es') are supported.
// Read supports auto language identification and multilanguage documents, so only provide a language code if
// you would like to force the documented to be processed as that specific language.
