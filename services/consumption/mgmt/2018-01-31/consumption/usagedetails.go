...
func NewUsageDetailsClient(subscriptionID string, name string) UsageDetailsClient {
	return NewUsageDetailsClientWithBaseURI(DefaultBaseURI, subscriptionID, name)
}

// NewUsageDetailsClientWithBaseURI creates an instance of the UsageDetailsClient client.
func NewUsageDetailsClientWithBaseURI(baseURI string, subscriptionID string, name string) UsageDetailsClient {
	return UsageDetailsClient{NewWithBaseURI(baseURI, subscriptionID, name)}
}
...
// period. expand is may be used to expand the properties/additionalProperties or properties/meterDetails within a list
// of usage details. By default, these fields are not included when listing usage details. filter is may be used to
// filter usageDetails by properties/usageEnd (Utc time), properties/usageStart (Utc time), properties/resourceGroup,
// properties/instanceName or properties/instanceId. The filter supports 'eq', 'lt', 'gt', 'le', 'ge', and 'and'. It
// does not currently support 'ne', 'or', or 'not'. skiptoken is skiptoken is only used if a previous operation
// returned a partial result. If a previous response contains a nextLink element, the value of the nextLink element
// will include a skiptoken parameter that specifies a starting point to use for subsequent calls. top is may be used
// to limit the number of results to the most recent N usageDetails.
