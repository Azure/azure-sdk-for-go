...
func NewContainerLogsClient(p pipeline.Pipeline) ContainerLogsClient {
	return ContainerLogsClient{NewManagementClient(p)}
}
...
// are shown up to 4mb.
func (client ContainerLogsClient) List(ctx context.Context, resourceGroupName string, containerName string, containerGroupName string, tail *int32) (*Logs, error) {
...
func (client ContainerLogsClient) listPreparer(resourceGroupName string, containerName string, containerGroupName string, tail *int32) (pipeline.Request, error) {
	u := client.url
	u.Path = "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ContainerInstance/containerGroups/{containerGroupName}/containers/{containerName}/logs"
	...
	if tail != nil {
		params.Set("tail", fmt.Sprintf("%v", *tail))
	}
...
func (client ContainerLogsClient) listResponder(resp pipeline.Response) (pipeline.Response, error) {
	err := validateResponse(resp, http.StatusOK)
	if resp == nil {
		return nil, err
	}
	result := &Logs{rawResponse: resp.Response()}
	if err != nil {
		return result, err
	}
