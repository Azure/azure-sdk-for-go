...
// MarshalXML implements the xml.Marshaler interface for DataLakeStoreAccountProperties.
func (dlsap DataLakeStoreAccountProperties) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if reflect.TypeOf((*DataLakeStoreAccountProperties)(nil)).Elem().Size() != reflect.TypeOf((*dataLakeStoreAccountProperties)(nil)).Elem().Size() {
		panic("size mismatch between DataLakeStoreAccountProperties and dataLakeStoreAccountProperties")
	}
	dlsap2 := (*dataLakeStoreAccountProperties)(unsafe.Pointer(&dlsap))
	return e.EncodeElement(*dlsap2, start)
}

// UnmarshalXML implements the xml.Unmarshaler interface for DataLakeStoreAccountProperties.
func (dlsap *DataLakeStoreAccountProperties) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	if reflect.TypeOf((*DataLakeStoreAccountProperties)(nil)).Elem().Size() != reflect.TypeOf((*dataLakeStoreAccountProperties)(nil)).Elem().Size() {
		panic("size mismatch between DataLakeStoreAccountProperties and dataLakeStoreAccountProperties")
	}
	dlsap2 := (*dataLakeStoreAccountProperties)(unsafe.Pointer(dlsap))
	return d.DecodeElement(dlsap2, &start)
}
