...
func (client DevicesClient) CreateOrUpdateExtendedInfo(ctx context.Context, deviceName string, parameters DataBoxEdgeDeviceExtendedInfo, resourceGroupName string) (result DataBoxEdgeDeviceExtendedInfo, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/DevicesClient.CreateOrUpdateExtendedInfo")
		defer func() {
			sc := -1
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: parameters,
			Constraints: []validation.Constraint{{Target: "parameters.DataBoxEdgeDeviceExtendedInfoProperties", Name: validation.Null, Rule: false,
				Chain: []validation.Constraint{{Target: "parameters.DataBoxEdgeDeviceExtendedInfoProperties.IntegrityKey", Name: validation.Null, Rule: true, Chain: nil}}}}}}); err != nil {
		return result, validation.NewError("edgegateway.DevicesClient", "CreateOrUpdateExtendedInfo", err.Error())
	}

	req, err := client.CreateOrUpdateExtendedInfoPreparer(ctx, deviceName, parameters, resourceGroupName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "edgegateway.DevicesClient", "CreateOrUpdateExtendedInfo", nil, "Failure preparing request")
		return
	}

	resp, err := client.CreateOrUpdateExtendedInfoSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "edgegateway.DevicesClient", "CreateOrUpdateExtendedInfo", resp, "Failure sending request")
		return
	}

	result, err = client.CreateOrUpdateExtendedInfoResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "edgegateway.DevicesClient", "CreateOrUpdateExtendedInfo", resp, "Failure responding to request")
	}

	return
}

// CreateOrUpdateExtendedInfoPreparer prepares the CreateOrUpdateExtendedInfo request.
func (client DevicesClient) CreateOrUpdateExtendedInfoPreparer(ctx context.Context, deviceName string, parameters DataBoxEdgeDeviceExtendedInfo, resourceGroupName string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"deviceName":        autorest.Encode("path", deviceName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
...
// CreateOrUpdateExtendedInfoSender sends the CreateOrUpdateExtendedInfo request. The method will close the
// http.Response Body if it receives an error.
func (client DevicesClient) CreateOrUpdateExtendedInfoSender(req *http.Request) (*http.Response, error) {
	return autorest.SendWithSender(client, req,
		azure.DoRetryWithRegistration(client.Client))
}

// CreateOrUpdateExtendedInfoResponder handles the response to the CreateOrUpdateExtendedInfo request. The method always
// closes the http.Response Body.
func (client DevicesClient) CreateOrUpdateExtendedInfoResponder(resp *http.Response) (result DataBoxEdgeDeviceExtendedInfo, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
