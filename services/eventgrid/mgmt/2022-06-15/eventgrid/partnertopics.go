...
func (client PartnerTopicsClient) CreateOrUpdate(ctx context.Context, resourceGroupName string, partnerTopicName string, partnerTopicInfo PartnerTopic) (result PartnerTopic, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/PartnerTopicsClient.CreateOrUpdate")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	req, err := client.CreateOrUpdatePreparer(ctx, resourceGroupName, partnerTopicName, partnerTopicInfo)
	if err != nil {
		err = autorest.NewErrorWithError(err, "eventgrid.PartnerTopicsClient", "CreateOrUpdate", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "eventgrid.PartnerTopicsClient", "CreateOrUpdate", resp, "Failure sending request")
		return
	}

	result, err = client.CreateOrUpdateResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "eventgrid.PartnerTopicsClient", "CreateOrUpdate", resp, "Failure responding to request")
		return
	}
...
func (client PartnerTopicsClient) CreateOrUpdatePreparer(ctx context.Context, resourceGroupName string, partnerTopicName string, partnerTopicInfo PartnerTopic) (*http.Request, error) {
	...
	partnerTopicInfo.SystemData = nil
	preparer := autorest.CreatePreparer(
		...
		autorest.WithJSON(partnerTopicInfo),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}
