...
// resourceGroupName - name of the resource group within the azure subscription.
// namespaceName - the Namespace name
func (client NamespacesClient) ListNetworkRuleSet(ctx context.Context, resourceGroupName string, namespaceName string) (result NetworkRuleSetListResult, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/NamespacesClient.ListNetworkRuleSet")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	if err := validation.Validate([]validation.Validation{
		...
		{TargetValue: namespaceName,
			Constraints: []validation.Constraint{{Target: "namespaceName", Name: validation.MaxLength, Rule: 50, Chain: nil},
				{Target: "namespaceName", Name: validation.MinLength, Rule: 6, Chain: nil}}}}); err != nil {
		return result, validation.NewError("eventhub.NamespacesClient", "ListNetworkRuleSet", err.Error())
	}

	req, err := client.ListNetworkRuleSetPreparer(ctx, resourceGroupName, namespaceName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "eventhub.NamespacesClient", "ListNetworkRuleSet", nil, "Failure preparing request")
		return
	}

	resp, err := client.ListNetworkRuleSetSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "eventhub.NamespacesClient", "ListNetworkRuleSet", resp, "Failure sending request")
		return
	}

	result, err = client.ListNetworkRuleSetResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "eventhub.NamespacesClient", "ListNetworkRuleSet", resp, "Failure responding to request")
		return
	}
...
// ListNetworkRuleSetPreparer prepares the ListNetworkRuleSet request.
func (client NamespacesClient) ListNetworkRuleSetPreparer(ctx context.Context, resourceGroupName string, namespaceName string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"namespaceName":     autorest.Encode("path", namespaceName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
...
// ListNetworkRuleSetSender sends the ListNetworkRuleSet request. The method will close the
// http.Response Body if it receives an error.
func (client NamespacesClient) ListNetworkRuleSetSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// ListNetworkRuleSetResponder handles the response to the ListNetworkRuleSet request. The method always
// closes the http.Response Body.
func (client NamespacesClient) ListNetworkRuleSetResponder(resp *http.Response) (result NetworkRuleSetListResult, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByUnmarshallingJSON(&result),
