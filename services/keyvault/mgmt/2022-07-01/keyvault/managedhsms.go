...
func (client ManagedHsmsClient) CheckMhsmNameAvailability(ctx context.Context, mhsmName CheckMhsmNameAvailabilityParameters) (result CheckMhsmNameAvailabilityResult, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ManagedHsmsClient.CheckMhsmNameAvailability")
		defer func() {
			sc := -1
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: mhsmName,
			Constraints: []validation.Constraint{{Target: "mhsmName.Name", Name: validation.Null, Rule: true, Chain: nil}}}}); err != nil {
		return result, validation.NewError("keyvault.ManagedHsmsClient", "CheckMhsmNameAvailability", err.Error())
	}

	req, err := client.CheckMhsmNameAvailabilityPreparer(ctx, mhsmName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "keyvault.ManagedHsmsClient", "CheckMhsmNameAvailability", nil, "Failure preparing request")
		return
	}

	resp, err := client.CheckMhsmNameAvailabilitySender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "keyvault.ManagedHsmsClient", "CheckMhsmNameAvailability", resp, "Failure sending request")
		return
	}

	result, err = client.CheckMhsmNameAvailabilityResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "keyvault.ManagedHsmsClient", "CheckMhsmNameAvailability", resp, "Failure responding to request")
		return
	}
...
func (client ManagedHsmsClient) CheckMhsmNameAvailabilityPreparer(ctx context.Context, mhsmName CheckMhsmNameAvailabilityParameters) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/providers/Microsoft.KeyVault/checkMhsmNameAvailability", pathParameters),
		autorest.WithJSON(mhsmName),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// CheckMhsmNameAvailabilitySender sends the CheckMhsmNameAvailability request. The method will close the
// http.Response Body if it receives an error.
func (client ManagedHsmsClient) CheckMhsmNameAvailabilitySender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// CheckMhsmNameAvailabilityResponder handles the response to the CheckMhsmNameAvailability request. The method always
