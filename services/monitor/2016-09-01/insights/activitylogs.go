...
// ActivityLogsClient is the monitor Client
...
// 'resourceGroupName'.<br>- *List events for resource*: $filter=eventTimestamp ge '2014-07-16T04:36:37.6407898Z' and
// eventTimestamp le '2014-07-20T04:36:37.6407898Z' and resourceUri eq 'resourceURI'.<br>- *List events for a
...
// 'correlationID'.<br><br>**NOTE**: No other syntax is allowed. selectParameter is used to fetch events with only the
...
func (client ActivityLogsClient) ListNextResults(lastResults EventDataCollection) (result EventDataCollection, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "insights.ActivityLogsClient", "List", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "insights.ActivityLogsClient", "List", resp, "Failure sending next results request")
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "insights.ActivityLogsClient", "List", resp, "Failure responding to next results request")
	}
