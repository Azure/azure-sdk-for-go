...
// MetricsClient is the client for the Metrics methods of the Metricsapi
// service.
...
// data collected.<br>The filter is optional. If present it must contain a list
// of metric names to retrieve of the form: *(name.value eq 'metricName' [or
// name.value eq 'metricName' or ...])*. Optionally, the filter can contain
// conditions for the following attributes *aggregationType*, *startTime*,
// *endTime*, and *timeGrain* of the form *attributeName operator value*. Where
// operator is one of *ne*, *eq*, *gt*, *lt*.<br>Several conditions can be
// combined with parentheses and logical operators, e.g: *and*, *or*.<br>Some
// example filter expressions are:<br>- $filter=(name.value eq 'RunsSucceeded')
// and aggregationType eq 'Total' and startTime eq 2016-02-20 and endTime eq
...
func (client MetricsClient) List(resourceURI string, filter string) (result MetricCollection, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "metricsapi.MetricsClient", "List", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "metricsapi.MetricsClient", "List", resp, "Failure sending request")
		return
	}

	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "metricsapi.MetricsClient", "List", resp, "Failure responding to request")
	}
