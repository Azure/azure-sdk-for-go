...
func (client TenantActivityLogsClient) List(ctx context.Context, filter string, selectParameter string) (result EventDataCollectionPage, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "activitylogs.TenantActivityLogsClient", "List", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "activitylogs.TenantActivityLogsClient", "List", resp, "Failure sending request")
		return
	}

	result.edc, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "activitylogs.TenantActivityLogsClient", "List", resp, "Failure responding to request")
		return
	}
...
func (client TenantActivityLogsClient) listNextResults(ctx context.Context, lastResults EventDataCollection) (result EventDataCollection, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "activitylogs.TenantActivityLogsClient", "listNextResults", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "activitylogs.TenantActivityLogsClient", "listNextResults", resp, "Failure sending next results request")
	}
	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "activitylogs.TenantActivityLogsClient", "listNextResults", resp, "Failure responding to next results request")
	}
	return
}
