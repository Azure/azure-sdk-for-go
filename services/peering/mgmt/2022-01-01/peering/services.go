...
func (client ServicesClient) InitializeConnectionMonitor(ctx context.Context) (result autorest.Response, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ServicesClient.InitializeConnectionMonitor")
		defer func() {
			sc := -1
			if result.Response != nil {
				sc = result.Response.StatusCode
	...
	req, err := client.InitializeConnectionMonitorPreparer(ctx)
	if err != nil {
		err = autorest.NewErrorWithError(err, "peering.ServicesClient", "InitializeConnectionMonitor", nil, "Failure preparing request")
		return
	}

	resp, err := client.InitializeConnectionMonitorSender(req)
	if err != nil {
		result.Response = resp
		err = autorest.NewErrorWithError(err, "peering.ServicesClient", "InitializeConnectionMonitor", resp, "Failure sending request")
		return
	}

	result, err = client.InitializeConnectionMonitorResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "peering.ServicesClient", "InitializeConnectionMonitor", resp, "Failure responding to request")
		return
	}
...
// InitializeConnectionMonitorPreparer prepares the InitializeConnectionMonitor request.
func (client ServicesClient) InitializeConnectionMonitorPreparer(ctx context.Context) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"subscriptionId": autorest.Encode("path", client.SubscriptionID),
	}
	...
	preparer := autorest.CreatePreparer(
		autorest.AsPost(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/providers/Microsoft.Peering/initializeConnectionMonitor", pathParameters),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// InitializeConnectionMonitorSender sends the InitializeConnectionMonitor request. The method will close the
// http.Response Body if it receives an error.
func (client ServicesClient) InitializeConnectionMonitorSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// InitializeConnectionMonitorResponder handles the response to the InitializeConnectionMonitor request. The method always
// closes the http.Response Body.
func (client ServicesClient) InitializeConnectionMonitorResponder(resp *http.Response) (result autorest.Response, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByClosing())
