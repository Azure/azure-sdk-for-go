...
// checkNameAvailabilityParameters - the object containing information for the availability request.
func (client OperationsClient) RegionalCheckNameAvailability(ctx context.Context, location string, checkNameAvailabilityParameters CheckNameAvailabilityParameters) (result NameAvailabilityStatus, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/OperationsClient.RegionalCheckNameAvailability")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: checkNameAvailabilityParameters,
			Constraints: []validation.Constraint{{Target: "checkNameAvailabilityParameters.Name", Name: validation.Null, Rule: true, Chain: nil},
				{Target: "checkNameAvailabilityParameters.Type", Name: validation.Null, Rule: true, Chain: nil}}}}); err != nil {
		return result, validation.NewError("appconfiguration.OperationsClient", "RegionalCheckNameAvailability", err.Error())
	}

	req, err := client.RegionalCheckNameAvailabilityPreparer(ctx, location, checkNameAvailabilityParameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "appconfiguration.OperationsClient", "RegionalCheckNameAvailability", nil, "Failure preparing request")
		return
	}

	resp, err := client.RegionalCheckNameAvailabilitySender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "appconfiguration.OperationsClient", "RegionalCheckNameAvailability", resp, "Failure sending request")
		return
	}

	result, err = client.RegionalCheckNameAvailabilityResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "appconfiguration.OperationsClient", "RegionalCheckNameAvailability", resp, "Failure responding to request")
		return
	}
...
// RegionalCheckNameAvailabilityPreparer prepares the RegionalCheckNameAvailability request.
func (client OperationsClient) RegionalCheckNameAvailabilityPreparer(ctx context.Context, location string, checkNameAvailabilityParameters CheckNameAvailabilityParameters) (*http.Request, error) {
	pathParameters := map[string]interface{}{
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.AsPost(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/providers/Microsoft.AppConfiguration/locations/{location}/checkNameAvailability", pathParameters),
		autorest.WithJSON(checkNameAvailabilityParameters),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// RegionalCheckNameAvailabilitySender sends the RegionalCheckNameAvailability request. The method will close the
// http.Response Body if it receives an error.
func (client OperationsClient) RegionalCheckNameAvailabilitySender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// RegionalCheckNameAvailabilityResponder handles the response to the RegionalCheckNameAvailability request. The method always
// closes the http.Response Body.
func (client OperationsClient) RegionalCheckNameAvailabilityResponder(resp *http.Response) (result NameAvailabilityStatus, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByUnmarshallingJSON(&result),
