...
func NewLiveTokenClient(subscriptionID string) LiveTokenClient {
	return NewLiveTokenClientWithBaseURI(DefaultBaseURI, subscriptionID)
}

// NewLiveTokenClientWithBaseURI creates an instance of the LiveTokenClient client using a custom endpoint.  Use this
// when interacting with an Azure cloud that uses a non-standard base URI (sovereign clouds, Azure stack).
func NewLiveTokenClientWithBaseURI(baseURI string, subscriptionID string) LiveTokenClient {
	return LiveTokenClient{NewWithBaseURI(baseURI, subscriptionID)}
}
...
func (client LiveTokenClient) Get(ctx context.Context, resourceURI string) (result LiveTokenResponse, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/LiveTokenClient.Get")
		defer func() {
			sc := -1
	...
	req, err := client.GetPreparer(ctx, resourceURI)
	if err != nil {
		err = autorest.NewErrorWithError(err, "insights.LiveTokenClient", "Get", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "insights.LiveTokenClient", "Get", resp, "Failure sending request")
		return
	}

	result, err = client.GetResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "insights.LiveTokenClient", "Get", resp, "Failure responding to request")
		return
	}
...
func (client LiveTokenClient) GetPreparer(ctx context.Context, resourceURI string) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPathParameters("/{resourceUri}/providers/microsoft.insights/generatelivetoken", pathParameters),
