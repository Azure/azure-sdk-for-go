...
// PaymentMethodsByBillingProfileClient is the billing client provides access to billing resources for Azure
// subscriptions.
...
func NewPaymentMethodsByBillingProfileClient(subscriptionID string) PaymentMethodsByBillingProfileClient {
	return NewPaymentMethodsByBillingProfileClientWithBaseURI(DefaultBaseURI, subscriptionID)
}

// NewPaymentMethodsByBillingProfileClientWithBaseURI creates an instance of the PaymentMethodsByBillingProfileClient
// client.
func NewPaymentMethodsByBillingProfileClientWithBaseURI(baseURI string, subscriptionID string) PaymentMethodsByBillingProfileClient {
	return PaymentMethodsByBillingProfileClient{NewWithBaseURI(baseURI, subscriptionID)}
}
...
func (client PaymentMethodsByBillingProfileClient) List(ctx context.Context, billingAccountName string, billingProfileName string) (result PaymentMethodsListResultPage, err error) {
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "billing.PaymentMethodsByBillingProfileClient", "List", resp, "Failure sending request")
		return
	}

	result.pmlr, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "billing.PaymentMethodsByBillingProfileClient", "List", resp, "Failure responding to request")
	}
...
func (client PaymentMethodsByBillingProfileClient) ListSender(req *http.Request) (*http.Response, error) {
...
func (client PaymentMethodsByBillingProfileClient) listNextResults(ctx context.Context, lastResults PaymentMethodsListResult) (result PaymentMethodsListResult, err error) {
	req, err := lastResults.paymentMethodsListResultPreparer(ctx)
	if err != nil {
		return result, autorest.NewErrorWithError(err, "billing.PaymentMethodsByBillingProfileClient", "listNextResults", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "billing.PaymentMethodsByBillingProfileClient", "listNextResults", resp, "Failure sending next results request")
	}
	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "billing.PaymentMethodsByBillingProfileClient", "listNextResults", resp, "Failure responding to next results request")
	}
	return
}
