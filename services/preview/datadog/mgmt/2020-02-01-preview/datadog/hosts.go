...
func (client HostsClient) List(ctx context.Context, resourceGroupName string, monitorName string) (result HostListResponsePage, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "datadog.HostsClient", "List", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "datadog.HostsClient", "List", resp, "Failure sending request")
		return
	}
...
func (client HostsClient) listNextResults(ctx context.Context, lastResults HostListResponse) (result HostListResponse, err error) {
	req, err := lastResults.hostListResponsePreparer(ctx)
	if err != nil {
		return result, autorest.NewErrorWithError(err, "datadog.HostsClient", "listNextResults", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "datadog.HostsClient", "listNextResults", resp, "Failure sending next results request")
	}
