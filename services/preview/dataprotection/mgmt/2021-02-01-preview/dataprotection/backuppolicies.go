...
func (client BackupPoliciesClient) Put(ctx context.Context, vaultName string, resourceGroupName string, backupPolicyName string, parameters BaseBackupPolicyResource) (result BaseBackupPolicyResource, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/BackupPoliciesClient.Put")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	req, err := client.PutPreparer(ctx, vaultName, resourceGroupName, backupPolicyName, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataprotection.BackupPoliciesClient", "Put", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "dataprotection.BackupPoliciesClient", "Put", resp, "Failure sending request")
		return
	}

	result, err = client.PutResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataprotection.BackupPoliciesClient", "Put", resp, "Failure responding to request")
		return
	}
...
func (client BackupPoliciesClient) PutPreparer(ctx context.Context, vaultName string, resourceGroupName string, backupPolicyName string, parameters BaseBackupPolicyResource) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"backupPolicyName":  autorest.Encode("path", backupPolicyName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
...
// http.Response Body if it receives an error.
func (client BackupPoliciesClient) PutSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}

// PutResponder handles the response to the Put request. The method always
// closes the http.Response Body.
func (client BackupPoliciesClient) PutResponder(resp *http.Response) (result BaseBackupPolicyResource, err error) {
	err = autorest.Respond(
		resp,
		azure.WithErrorUnlessStatusCode(http.StatusOK),
		autorest.ByUnmarshallingJSON(&result),
