...
func (client BackupVaultsClient) PatchSender(req *http.Request) (future BackupVaultsPatchFuture, err error) {
	...
	future.Result = func(client BackupVaultsClient) (bvr BackupVaultResource, err error) {
		var done bool
		...
		bvr.Response.Response, err = future.GetResult(sender)
		if bvr.Response.Response == nil && err == nil {
			err = autorest.NewErrorWithError(err, "dataprotection.BackupVaultsPatchFuture", "Result", nil, "received nil response and error")
		}
		if err == nil && bvr.Response.Response.StatusCode != http.StatusNoContent {
...
// Put creates or updates a BackupVault resource belonging to a resource group.
// Parameters:
// vaultName - the name of the backup vault.
// resourceGroupName - the name of the resource group where the backup vault is present.
// parameters - request body for operation
func (client BackupVaultsClient) Put(ctx context.Context, vaultName string, resourceGroupName string, parameters BackupVaultResource) (result BackupVaultsPutFuture, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/BackupVaultsClient.Put")
		defer func() {
			sc := -1
			if result.FutureAPI != nil && result.FutureAPI.Response() != nil {
				sc = result.FutureAPI.Response().StatusCode
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataprotection.BackupVaultsClient", "Put", nil, "Failure preparing request")
		return
	}

	result, err = client.PutSender(req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataprotection.BackupVaultsClient", "Put", nil, "Failure sending request")
		return
	}
...
func (client BackupVaultsClient) PutPreparer(ctx context.Context, vaultName string, resourceGroupName string, parameters BackupVaultResource) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
		"vaultName":         autorest.Encode("path", vaultName),
...
// http.Response Body if it receives an error.
func (client BackupVaultsClient) PutSender(req *http.Request) (future BackupVaultsPutFuture, err error) {
	var resp *http.Response
	resp, err = client.Send(req, azure.DoRetryWithRegistration(client.Client))
	if err != nil {
		return
	...
	future.Result = func(client BackupVaultsClient) (bvr BackupVaultResource, err error) {
		...
		if err != nil {
			err = autorest.NewErrorWithError(err, "dataprotection.BackupVaultsPutFuture", "Result", future.Response(), "Polling failure")
			return
		}
		if !done {
			err = azure.NewAsyncOpIncompleteError("dataprotection.BackupVaultsPutFuture")
			return
		}
		sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
		bvr.Response.Response, err = future.GetResult(sender)
		if bvr.Response.Response == nil && err == nil {
			err = autorest.NewErrorWithError(err, "dataprotection.BackupVaultsPutFuture", "Result", nil, "received nil response and error")
		}
		if err == nil && bvr.Response.Response.StatusCode != http.StatusNoContent {
			bvr, err = client.PutResponder(bvr.Response.Response)
			if err != nil {
				err = autorest.NewErrorWithError(err, "dataprotection.BackupVaultsPutFuture", "Result", bvr.Response.Response, "Failure responding to request")
			}
		}
		return
	}
...
// closes the http.Response Body.
func (client BackupVaultsClient) PutResponder(resp *http.Response) (result BackupVaultResource, err error) {
	err = autorest.Respond(
		resp,
