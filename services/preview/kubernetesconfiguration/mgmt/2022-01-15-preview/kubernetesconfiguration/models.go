...
const fqdn = "github.com/Azure/azure-sdk-for-go/services/preview/kubernetesconfiguration/mgmt/2022-01-15-preview/kubernetesconfiguration"
...
type ExtensionTypeProperties struct {
	...
	// ClusterTypes - READ-ONLY; Cluster types
...
type ExtensionVersionList struct {
	autorest.Response `json:"-"`
	// Value - Versions available for this Extension Type
	Value *[]ExtensionVersionListValueItem `json:"value,omitempty"`
	// NextLink - The link to fetch the next page of Extension Types
	NextLink *string `json:"nextLink,omitempty"`
	// SystemData - READ-ONLY; Metadata pertaining to creation and last modification of the resource.
	SystemData *SystemData `json:"systemData,omitempty"`
...
// ExtensionVersionListIterator provides access to a complete listing of ExtensionVersionListValueItem
// values.
...
// Value returns the current value or a zero-initialized value if the
// iterator has advanced beyond the end of the collection.
func (iter ExtensionVersionListIterator) Value() ExtensionVersionListValueItem {
	if !iter.page.NotDone() {
		return ExtensionVersionListValueItem{}
	}
	return iter.page.Values()[iter.i]
}
...
func (evl ExtensionVersionList) IsEmpty() bool {
	return evl.Value == nil || len(*evl.Value) == 0
}
...
// ExtensionVersionListPage contains a page of ExtensionVersionListValueItem values.
...
// Values returns the slice of values for the current page or nil if there are no values.
func (page ExtensionVersionListPage) Values() []ExtensionVersionListValueItem {
	if page.evl.IsEmpty() {
		return nil
	}
	return *page.evl.Value
}
...
type ExtensionVersionListValueItem struct {
...
type FluxConfigurationPatchProperties struct {
	// SourceKind - Source Kind to pull the configuration data from. Possible values include: 'GitRepository', 'Bucket'
...
type FluxConfigurationProperties struct {
	...
	// LastSourceUpdatedCommitID - READ-ONLY; Branch and SHA of the last source commit synced with the cluster.
	LastSourceUpdatedCommitID *string `json:"lastSourceUpdatedCommitId,omitempty"`
	// LastSourceUpdatedAt - READ-ONLY; Datetime the fluxConfiguration last synced its source on the cluster.
	LastSourceUpdatedAt *date.Time `json:"lastSourceUpdatedAt,omitempty"`
