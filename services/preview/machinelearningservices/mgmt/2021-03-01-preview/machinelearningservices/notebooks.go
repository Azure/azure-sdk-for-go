...
func (client NotebooksClient) ListKeys(ctx context.Context, resourceGroupName string, workspaceName string) (result ListNotebookKeysResult, err error) {
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("machinelearningservices.NotebooksClient", "ListKeys", err.Error())
	}
	...
	if err := validation.Validate([]validation.Validation{
		...
		{TargetValue: resourceGroupName,
			Constraints: []validation.Constraint{{Target: "resourceGroupName", Name: validation.MaxLength, Rule: 90, Chain: nil},
				{Target: "resourceGroupName", Name: validation.MinLength, Rule: 1, Chain: nil},
				{Target: "resourceGroupName", Name: validation.Pattern, Rule: `^[-\w\._\(\)]+$`, Chain: nil}}}}); err != nil {
		return result, validation.NewError("machinelearningservices.NotebooksClient", "Prepare", err.Error())
	}
