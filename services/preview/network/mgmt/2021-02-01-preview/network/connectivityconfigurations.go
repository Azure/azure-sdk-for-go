...
// networkManagerName - the name of the network manager.
// configurationName - the name of the network manager connectivity configuration.
func (client ConnectivityConfigurationsClient) Delete(ctx context.Context, resourceGroupName string, networkManagerName string, configurationName string) (result autorest.Response, err error) {
	if tracing.IsEnabled() {
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "network.ConnectivityConfigurationsClient", "Delete", resp, "Failure sending request")
		return
	}

	result, err = client.DeleteResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "network.ConnectivityConfigurationsClient", "Delete", resp, "Failure responding to request")
		return
	}
...
// http.Response Body if it receives an error.
func (client ConnectivityConfigurationsClient) DeleteSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, azure.DoRetryWithRegistration(client.Client))
}
