...
func (client RecommendedActionsClient) ListByServer(ctx context.Context, resourceGroupName string, serverName string, advisorName string, sessionID string) (result RecommendationActionsResultListPage, err error) {
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("postgresqlflexibleservers.RecommendedActionsClient", "ListByServer", err.Error())
	}

	result.fn = client.listByServerNextResults
	req, err := client.ListByServerPreparer(ctx, resourceGroupName, serverName, advisorName, sessionID)
	if err != nil {
		err = autorest.NewErrorWithError(err, "postgresqlflexibleservers.RecommendedActionsClient", "ListByServer", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "postgresqlflexibleservers.RecommendedActionsClient", "ListByServer", resp, "Failure sending request")
		return
	}

	result.rarl, err = client.ListByServerResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "postgresqlflexibleservers.RecommendedActionsClient", "ListByServer", resp, "Failure responding to request")
		return
	}
...
func (client RecommendedActionsClient) ListByServerPreparer(ctx context.Context, resourceGroupName string, serverName string, advisorName string, sessionID string) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DBforPostgreSQL/flexibleServers/{serverName}/advisors/{advisorName}/recommendedActions", pathParameters),
...
func (client RecommendedActionsClient) listByServerNextResults(ctx context.Context, lastResults RecommendationActionsResultList) (result RecommendationActionsResultList, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "postgresqlflexibleservers.RecommendedActionsClient", "listByServerNextResults", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "postgresqlflexibleservers.RecommendedActionsClient", "listByServerNextResults", resp, "Failure sending next results request")
	}
	result, err = client.ListByServerResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "postgresqlflexibleservers.RecommendedActionsClient", "listByServerNextResults", resp, "Failure responding to next results request")
	}
	return
}
