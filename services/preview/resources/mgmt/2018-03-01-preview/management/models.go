...
func (future EntitiesListAllFuture) Result(client EntitiesClient) (elrp EntityListResultPage, err error) {
	...
	if !done {
		return elrp, azure.NewAsyncOpIncompleteError("managementgroups.EntitiesListAllFuture")
	}
	if future.PollingMethod() == azure.PollingLocation {
		elrp, err = client.ListResponder(future.Response())
		if err != nil {
			err = autorest.NewErrorWithError(err, "managementgroups.EntitiesListAllFuture", "Result", future.Response(), "Failure responding to request")
		}
		return
	}
	var req *http.Request
	...
	resp, err = autorest.SendWithSender(client, req,
		autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if err != nil {
		err = autorest.NewErrorWithError(err, "managementgroups.EntitiesListAllFuture", "Result", resp, "Failure sending request")
		return
	}
	elrp, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "managementgroups.EntitiesListAllFuture", "Result", resp, "Failure responding to request")
	}
	return
}
...
func (future EntitiesListFuture) Result(client EntitiesClient) (elrp EntityListResultPage, err error) {
	...
	if !done {
		return elrp, azure.NewAsyncOpIncompleteError("managementgroups.EntitiesListFuture")
	}
	if future.PollingMethod() == azure.PollingLocation {
		elrp, err = client.ListResponder(future.Response())
		if err != nil {
			err = autorest.NewErrorWithError(err, "managementgroups.EntitiesListFuture", "Result", future.Response(), "Failure responding to request")
		}
		return
	}
	var req *http.Request
	...
	resp, err = autorest.SendWithSender(client, req,
		autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if err != nil {
		err = autorest.NewErrorWithError(err, "managementgroups.EntitiesListFuture", "Result", resp, "Failure sending request")
		return
	}
	elrp, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "managementgroups.EntitiesListFuture", "Result", resp, "Failure responding to request")
	}
	return
}
