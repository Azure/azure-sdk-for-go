...
type SecureScoreControlDefinitionSource struct {
	// SourceType - BuiltIn if the control is built-in from Azure Security Center managed assessments, Custom (Future) if the assessment based on custom Azure Policy definition, CustomerManaged (future) for customers who build their own controls. Possible values include: 'BuiltIn', 'Custom'
