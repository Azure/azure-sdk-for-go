...
// Create create a security assessment on your resource. An assessment metadata that describes this assessment must be
...
// body - calculated assessment on a pre-defined assessment metadata
func (client AssessmentsClient) Create(ctx context.Context, resourceID string, assessmentName string, body Assessment) (result Assessment, err error) {
	...
	if err := validation.Validate([]validation.Validation{
		{TargetValue: body,
			Constraints: []validation.Constraint{{Target: "body.AssessmentProperties", Name: validation.Null, Rule: false,
				Chain: []validation.Constraint{{Target: "body.AssessmentProperties.ResourceDetails", Name: validation.Null, Rule: true, Chain: nil},
					{Target: "body.AssessmentProperties.Status", Name: validation.Null, Rule: true, Chain: nil},
				}}}}}); err != nil {
		return result, validation.NewError("security.AssessmentsClient", "Create", err.Error())
	}

	req, err := client.CreatePreparer(ctx, resourceID, assessmentName, body)
	if err != nil {
		err = autorest.NewErrorWithError(err, "security.AssessmentsClient", "Create", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "security.AssessmentsClient", "Create", resp, "Failure sending request")
		return
	}

	result, err = client.CreateResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "security.AssessmentsClient", "Create", resp, "Failure responding to request")
	}
...
func (client AssessmentsClient) CreatePreparer(ctx context.Context, resourceID string, assessmentName string, body Assessment) (*http.Request, error) {
