...
// BabylonConfiguration babylon Configuration
type BabylonConfiguration struct {
	// BabylonResourceID - Babylon Resource ID
...
type BigDataPoolResourceProperties struct {
	...
	// SessionLevelPackagesEnabled - Whether session level library/package management is enabled or not.
...
// WorkspaceManagedqlServerBlobAuditingPoliciesCreateOrUpdateFuture an abstraction for monitoring and
// retrieving the results of a long-running operation.
type WorkspaceManagedqlServerBlobAuditingPoliciesCreateOrUpdateFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *WorkspaceManagedqlServerBlobAuditingPoliciesCreateOrUpdateFuture) Result(client WorkspaceManagedqlServerBlobAuditingPoliciesClient) (sbap ServerBlobAuditingPolicy, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "synapse.WorkspaceManagedqlServerBlobAuditingPoliciesCreateOrUpdateFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("synapse.WorkspaceManagedqlServerBlobAuditingPoliciesCreateOrUpdateFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if sbap.Response.Response, err = future.GetResult(sender); err == nil && sbap.Response.Response.StatusCode != http.StatusNoContent {
		...
		if err != nil {
			err = autorest.NewErrorWithError(err, "synapse.WorkspaceManagedqlServerBlobAuditingPoliciesCreateOrUpdateFuture", "Result", sbap.Response.Response, "Failure responding to request")
		}
	}
	return
}
...
func (wpp WorkspacePatchProperties) MarshalJSON() ([]byte, error) {
	...
	if wpp.BabylonConfiguration != nil {
		objectMap["babylonConfiguration"] = wpp.BabylonConfiguration
	}
	return json.Marshal(objectMap)
}
...
func (wp WorkspaceProperties) MarshalJSON() ([]byte, error) {
	...
	if wp.BabylonConfiguration != nil {
		objectMap["babylonConfiguration"] = wp.BabylonConfiguration
	}
	return json.Marshal(objectMap)
}
