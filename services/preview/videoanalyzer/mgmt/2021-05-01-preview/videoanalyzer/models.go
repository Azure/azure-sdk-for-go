...
type AccessPolicyProperties struct {
	// Role - Defines the access level granted by this policy. Possible values include: 'Reader'
...
type EccTokenKey struct {
	// Alg - Elliptical curve algorithm to be used: ES256, ES384 or ES512. Possible values include: 'ES256', 'ES384', 'ES512'
	...
	// Type - Possible values include: 'TypeTokenKey', 'TypeMicrosoftVideoAnalyzerRsaTokenKey', 'TypeMicrosoftVideoAnalyzerEccTokenKey'
...
func (etk EccTokenKey) MarshalJSON() ([]byte, error) {
	etk.Type = TypeMicrosoftVideoAnalyzerEccTokenKey
	objectMap := make(map[string]interface{})
	if etk.Alg != "" {
...
type RsaTokenKey struct {
	// Alg - RSA algorithm to be used: RS256, RS384 or RS512. Possible values include: 'RS256', 'RS384', 'RS512'
...
func (rtk RsaTokenKey) MarshalJSON() ([]byte, error) {
	rtk.Type = TypeMicrosoftVideoAnalyzerRsaTokenKey
	objectMap := make(map[string]interface{})
	if rtk.Alg != "" {
...
func unmarshalBasicTokenKey(body []byte) (BasicTokenKey, error) {
	...
	switch m["@type"] {
	case string(TypeMicrosoftVideoAnalyzerRsaTokenKey):
		var rtk RsaTokenKey
		err := json.Unmarshal(body, &rtk)
		return rtk, err
	case string(TypeMicrosoftVideoAnalyzerEccTokenKey):
		var etk EccTokenKey
...
// VideoAnalyzer a Video Analyzer account.
...
// MarshalJSON is the custom marshaler for VideoAnalyzer.
func (va VideoAnalyzer) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]interface{})
	if va.PropertiesType != nil {
		objectMap["properties"] = va.PropertiesType
	}
	if va.Identity != nil {
...
// UnmarshalJSON is the custom unmarshaler for VideoAnalyzer struct.
func (va *VideoAnalyzer) UnmarshalJSON(body []byte) error {
	var m map[string]*json.RawMessage
	err := json.Unmarshal(body, &m)
	if err != nil {
		return err
	...
	for k, v := range m {
		...
		case "properties":
			if v != nil {
				...
				va.PropertiesType = &propertiesType
			}
		case "systemData":
			if v != nil {
				var systemData SystemData
...
type VideoProperties struct {
	...
	Description *string `json:"description,omitempty"`
	// Type - READ-ONLY; Type of the video archive. Different archive formats provide different capabilities. Possible values include: 'Archive'
