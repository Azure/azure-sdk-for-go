...
// BMSPrepareDataMoveFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type BMSPrepareDataMoveFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *BMSPrepareDataMoveFuture) Result(client BaseClient) (ar autorest.Response, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "backup.BMSPrepareDataMoveFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("backup.BMSPrepareDataMoveFuture")
		return
	}
...
// BMSPrepareDataMoveOperationResultGetFuture an abstraction for monitoring and retrieving the results of a
// long-running operation.
type BMSPrepareDataMoveOperationResultGetFuture struct {
	azure.Future
}

// Result returns the result of the asynchronous operation.
// If the operation has not completed it will return an error.
func (future *BMSPrepareDataMoveOperationResultGetFuture) Result(client BMSPrepareDataMoveOperationResultClient) (vscorrm VaultStorageConfigOperationResultResponseModel, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "backup.BMSPrepareDataMoveOperationResultGetFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("backup.BMSPrepareDataMoveOperationResultGetFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if vscorrm.Response.Response, err = future.GetResult(sender); err == nil && vscorrm.Response.Response.StatusCode != http.StatusNoContent {
		vscorrm, err = client.GetResponder(vscorrm.Response.Response)
		if err != nil {
			err = autorest.NewErrorWithError(err, "backup.BMSPrepareDataMoveOperationResultGetFuture", "Result", vscorrm.Response.Response, "Failure responding to request")
		}
	}
	return
}
