...
func (client ReplicationProtectionIntentsClient) List(ctx context.Context, skipToken string, takeToken string) (result ReplicationProtectionIntentCollectionPage, err error) {
	if tracing.IsEnabled() {
	...
	req, err := client.ListPreparer(ctx, skipToken, takeToken)
	if err != nil {
...
func (client ReplicationProtectionIntentsClient) ListPreparer(ctx context.Context, skipToken string, takeToken string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"resourceGroupName": autorest.Encode("path", client.ResourceGroupName),
		"resourceName":      autorest.Encode("path", client.ResourceName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
...
func (client ReplicationProtectionIntentsClient) ListComplete(ctx context.Context, skipToken string, takeToken string) (result ReplicationProtectionIntentCollectionIterator, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ReplicationProtectionIntentsClient.List")
		defer func() {
			sc := -1
	...
	result.page, err = client.List(ctx, skipToken, takeToken)
	return
}
