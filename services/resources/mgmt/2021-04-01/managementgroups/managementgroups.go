...
// cacheControl - indicates whether the request should utilize any caches. Populate the header with 'no-cache'
// value to bypass existing caches.
func (client Client) Get(ctx context.Context, groupID string, expand ManagementGroupExpandType, recurse *bool, filter string, cacheControl string) (result ManagementGroup, err error) {
	if tracing.IsEnabled() {
...
func (client Client) GetPreparer(ctx context.Context, groupID string, expand ManagementGroupExpandType, recurse *bool, filter string, cacheControl string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"groupId": autorest.Encode("path", groupID),
	}
