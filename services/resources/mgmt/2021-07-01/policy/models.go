...
const fqdn = "github.com/Azure/azure-sdk-for-go/services/resources/mgmt/2021-07-01/policy"
...
// Pricing the policy pricing.
type Pricing struct {
	autorest.Response `json:"-"`
	// PricingProperties - Properties for the policy pricing.
	*PricingProperties `json:"properties,omitempty"`
	// SystemData - READ-ONLY; Azure Resource Manager metadata containing createdBy and modifiedBy information.
	SystemData *SystemData `json:"systemData,omitempty"`
	// ID - READ-ONLY; The ID of the policy pricing.
	ID *string `json:"id,omitempty"`
	// Name - READ-ONLY; The name of the policy pricing.
	Name *string `json:"name,omitempty"`
	// Type - READ-ONLY; The type of the resource (Microsoft.Authorization/policyPricings).
	Type *string `json:"type,omitempty"`
}
...
// PricingListResult list of policy pricings.
type PricingListResult struct {
	autorest.Response `json:"-"`
	// Value - An array of policy pricings.
	Value *[]Pricing `json:"value,omitempty"`
	// NextLink - READ-ONLY; The URL to use for getting the next set of results.
	NextLink *string `json:"nextLink,omitempty"`
}

// MarshalJSON is the custom marshaler for PricingListResult.
func (plr PricingListResult) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]interface{})
	if plr.Value != nil {
...
// PricingListResultIterator provides access to a complete listing of Pricing values.
type PricingListResultIterator struct {
	i    int
	page PricingListResultPage
}

// NextWithContext advances to the next value.  If there was an error making
// the request the iterator does not advance and the error is returned.
func (iter *PricingListResultIterator) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/PricingListResultIterator.NextWithContext")
		defer func() {
			sc := -1
			if iter.Response().Response.Response != nil {
				sc = iter.Response().Response.Response.StatusCode
...
// the request the iterator does not advance and the error is returned.
// Deprecated: Use NextWithContext() instead.
func (iter *PricingListResultIterator) Next() error {
	return iter.NextWithContext(context.Background())
}

// NotDone returns true if the enumeration should be started or is not yet complete.
func (iter PricingListResultIterator) NotDone() bool {
	return iter.page.NotDone() && iter.i < len(iter.page.Values())
}

// Response returns the raw server response from the last page request.
func (iter PricingListResultIterator) Response() PricingListResult {
	return iter.page.Response()
}

// Value returns the current value or a zero-initialized value if the
// iterator has advanced beyond the end of the collection.
func (iter PricingListResultIterator) Value() Pricing {
	if !iter.page.NotDone() {
		return Pricing{}
	}
	return iter.page.Values()[iter.i]
...
// Creates a new instance of the PricingListResultIterator type.
func NewPricingListResultIterator(page PricingListResultPage) PricingListResultIterator {
	return PricingListResultIterator{page: page}
}

// IsEmpty returns true if the ListResult contains no values.
func (plr PricingListResult) IsEmpty() bool {
	return plr.Value == nil || len(*plr.Value) == 0
}

// hasNextLink returns true if the NextLink is not empty.
func (plr PricingListResult) hasNextLink() bool {
	return plr.NextLink != nil && len(*plr.NextLink) != 0
}

// pricingListResultPreparer prepares a request to retrieve the next set of results.
// It returns nil if no more results exist.
func (plr PricingListResult) pricingListResultPreparer(ctx context.Context) (*http.Request, error) {
	if !plr.hasNextLink() {
		return nil, nil
	}
	return autorest.Prepare((&http.Request{}).WithContext(ctx),
...
// PricingListResultPage contains a page of Pricing values.
type PricingListResultPage struct {
	fn  func(context.Context, PricingListResult) (PricingListResult, error)
	plr PricingListResult
}

// NextWithContext advances to the next page of values.  If there was an error making
// the request the page does not advance and the error is returned.
func (page *PricingListResultPage) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/PricingListResultPage.NextWithContext")
		defer func() {
			sc := -1
			if page.Response().Response.Response != nil {
				sc = page.Response().Response.Response.StatusCode
...
// the request the page does not advance and the error is returned.
// Deprecated: Use NextWithContext() instead.
func (page *PricingListResultPage) Next() error {
	return page.NextWithContext(context.Background())
}

// NotDone returns true if the page enumeration should be started or is not yet complete.
func (page PricingListResultPage) NotDone() bool {
	return !page.plr.IsEmpty()
}

// Response returns the raw server response from the last page request.
func (page PricingListResultPage) Response() PricingListResult {
	return page.plr
}

// Values returns the slice of values for the current page or nil if there are no values.
func (page PricingListResultPage) Values() []Pricing {
	if page.plr.IsEmpty() {
		return nil
	}
...
// Creates a new instance of the PricingListResultPage type.
func NewPricingListResultPage(cur PricingListResult, getNextPage func(context.Context, PricingListResult) (PricingListResult, error)) PricingListResultPage {
	return PricingListResultPage{
		fn:  getNextPage,
		plr: cur,
	}
}

// PricingProperties the policy pricing properties.
type PricingProperties struct {
	// PricingTier - The policy pricing tier. Possible values are Advanced and Defender. Possible values include: 'Advanced', 'Defender'
	PricingTier PricingTier `json:"pricingTier,omitempty"`
	// ProvisioningState - READ-ONLY; The provisioning state of the policy pricing resource. Possible values include: 'Accepted', 'Succeeded', 'Deleting'
...
// PricingsCreateOrUpdateFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type PricingsCreateOrUpdateFuture struct {
	azure.FutureAPI
	// Result returns the result of the asynchronous operation.
	// If the operation has not completed it will return an error.
	Result func(PricingsClient) (Pricing, error)
}

// UnmarshalJSON is the custom unmarshaller for CreateFuture.
func (future *PricingsCreateOrUpdateFuture) UnmarshalJSON(body []byte) error {
...
// result is the default implementation for PricingsCreateOrUpdateFuture.Result.
func (future *PricingsCreateOrUpdateFuture) result(client PricingsClient) (p Pricing, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "policy.PricingsCreateOrUpdateFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		p.Response.Response = future.Response()
		err = azure.NewAsyncOpIncompleteError("policy.PricingsCreateOrUpdateFuture")
		return
	}
	...
	if p.Response.Response, err = future.GetResult(sender); err == nil && p.Response.Response.StatusCode != http.StatusNoContent {
		...
		if err != nil {
			err = autorest.NewErrorWithError(err, "policy.PricingsCreateOrUpdateFuture", "Result", p.Response.Response, "Failure responding to request")
		}
	}
	return
}

// PricingsDeleteFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type PricingsDeleteFuture struct {
	azure.FutureAPI
	// Result returns the result of the asynchronous operation.
	// If the operation has not completed it will return an error.
	Result func(PricingsClient) (autorest.Response, error)
}

// UnmarshalJSON is the custom unmarshaller for CreateFuture.
func (future *PricingsDeleteFuture) UnmarshalJSON(body []byte) error {
	var azFuture azure.Future
	if err := json.Unmarshal(body, &azFuture); err != nil {
		return err
	}
...
// result is the default implementation for PricingsDeleteFuture.Result.
func (future *PricingsDeleteFuture) result(client PricingsClient) (ar autorest.Response, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "policy.PricingsDeleteFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		ar.Response = future.Response()
		err = azure.NewAsyncOpIncompleteError("policy.PricingsDeleteFuture")
		return
	}
	ar.Response = future.Response()
	return
