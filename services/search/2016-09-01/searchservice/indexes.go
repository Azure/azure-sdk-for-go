...
//
// indexName is the name of the index for which to test an analyzer. request is
// the text and analyzer or analysis components to test. clientRequestID is the
...
func (client IndexesClient) Analyze(indexName string, request AnalyzeRequest, clientRequestID *uuid.UUID) (result AnalyzeResult, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewErrorWithValidationError(err, "searchservice.IndexesClient", "Analyze")
	}
...
// is the definition of the index to create or update. allowIndexDowntime is
// allows new analyzers, tokenizers, token filters, or char filters to be added
// to an index by taking the index offline for at least a few seconds. This
// temporarily causes indexing and query requests to fail. Performance and
// write availability of the index can be impaired for several minutes after
// the index is updated, or longer for very large indexes. clientRequestID is
...
//
// indexName is the name of the index to delete. clientRequestID is the
...
//
// indexName is the name of the index to retrieve. clientRequestID is the
// tracking ID sent with the request to help with debugging.
...
// for all properties. The default is all properties. clientRequestID is the
// tracking ID sent with the request to help with debugging.
