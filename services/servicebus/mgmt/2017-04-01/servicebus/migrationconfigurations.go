	...
	if err := validation.Validate([]validation.Validation{
		...
		{TargetValue: parameters,
			Constraints: []validation.Constraint{{Target: "parameters.MigrationConfigPropertiesProperties", Name: validation.Null, Rule: false,
				Chain: []validation.Constraint{{Target: "parameters.MigrationConfigPropertiesProperties.PrimaryNamespaceName", Name: validation.Null, Rule: true, Chain: nil},
...
// DisableMigration this operation disables the Migration
//
// resourceGroupName is name of the Resource group within the Azure subscription. namespaceName is the namespace
// name
func (client MigrationConfigurationsClient) DisableMigration(ctx context.Context, resourceGroupName string, namespaceName string) (result autorest.Response, err error) {
	if err := validation.Validate([]validation.Validation{
		{TargetValue: resourceGroupName,
			Constraints: []validation.Constraint{{Target: "resourceGroupName", Name: validation.MaxLength, Rule: 90, Chain: nil},
				{Target: "resourceGroupName", Name: validation.MinLength, Rule: 1, Chain: nil}}},
		{TargetValue: namespaceName,
			Constraints: []validation.Constraint{{Target: "namespaceName", Name: validation.MaxLength, Rule: 50, Chain: nil},
				{Target: "namespaceName", Name: validation.MinLength, Rule: 6, Chain: nil}}}}); err != nil {
		return result, validation.NewError("servicebus.MigrationConfigurationsClient", "DisableMigration", err.Error())
	}

	req, err := client.DisableMigrationPreparer(ctx, resourceGroupName, namespaceName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "servicebus.MigrationConfigurationsClient", "DisableMigration", nil, "Failure preparing request")
		return
	}

	resp, err := client.DisableMigrationSender(req)
	if err != nil {
		result.Response = resp
		err = autorest.NewErrorWithError(err, "servicebus.MigrationConfigurationsClient", "DisableMigration", resp, "Failure sending request")
		return
	}

	result, err = client.DisableMigrationResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "servicebus.MigrationConfigurationsClient", "DisableMigration", resp, "Failure responding to request")
	}

	return
}

// DisableMigrationPreparer prepares the DisableMigration request.
func (client MigrationConfigurationsClient) DisableMigrationPreparer(ctx context.Context, resourceGroupName string, namespaceName string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"configName":        autorest.Encode("path", "$default"),
		"namespaceName":     autorest.Encode("path", namespaceName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
...
// DisableMigrationSender sends the DisableMigration request. The method will close the
// http.Response Body if it receives an error.
func (client MigrationConfigurationsClient) DisableMigrationSender(req *http.Request) (*http.Response, error) {
	return autorest.SendWithSender(client, req,
		azure.DoRetryWithRegistration(client.Client))
}

// DisableMigrationResponder handles the response to the DisableMigration request. The method always
// closes the http.Response Body.
func (client MigrationConfigurationsClient) DisableMigrationResponder(resp *http.Response) (result autorest.Response, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
