...
// GetUpgradeProgress gets the upgrade progress information about the Application resource with the given name. The
// information include percentage of completion and other upgrade state information of the Application resource.
// Parameters:
// applicationResourceName - the identity of the application.
func (client MeshApplicationClient) GetUpgradeProgress(ctx context.Context, applicationResourceName string) (result ApplicationResourceUpgradeProgressInfo, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/MeshApplicationClient.GetUpgradeProgress")
		defer func() {
			sc := -1
			if result.Response.Response != nil {
				sc = result.Response.Response.StatusCode
	...
	req, err := client.GetUpgradeProgressPreparer(ctx, applicationResourceName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "servicefabric.MeshApplicationClient", "GetUpgradeProgress", nil, "Failure preparing request")
		return
	}

	resp, err := client.GetUpgradeProgressSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "servicefabric.MeshApplicationClient", "GetUpgradeProgress", resp, "Failure sending request")
		return
	}

	result, err = client.GetUpgradeProgressResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "servicefabric.MeshApplicationClient", "GetUpgradeProgress", resp, "Failure responding to request")
	}

	return
}

// GetUpgradeProgressPreparer prepares the GetUpgradeProgress request.
func (client MeshApplicationClient) GetUpgradeProgressPreparer(ctx context.Context, applicationResourceName string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"applicationResourceName": applicationResourceName,
	}
	...
	preparer := autorest.CreatePreparer(
		autorest.AsGet(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/Resources/Applications/{applicationResourceName}/$/GetUpgradeProgress", pathParameters),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// GetUpgradeProgressSender sends the GetUpgradeProgress request. The method will close the
// http.Response Body if it receives an error.
func (client MeshApplicationClient) GetUpgradeProgressSender(req *http.Request) (*http.Response, error) {
	sd := autorest.GetSendDecorators(req.Context(), autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	return autorest.SendWithSender(client, req, sd...)
}

// GetUpgradeProgressResponder handles the response to the GetUpgradeProgress request. The method always
// closes the http.Response Body.
