...
const (
	// Wave0 Cluster upgrade starts immediately after a new version is rolled out. Recommended for Test/Dev
	// clusters.
	Wave0 ClusterUpgradeCadence = "Wave0"
	// Wave1 Cluster upgrade starts 7 days after a new version is rolled out. Recommended for Pre-prod
	// clusters.
	Wave1 ClusterUpgradeCadence = "Wave1"
	// Wave2 Cluster upgrade starts 14 days after a new version is rolled out. Recommended for Production
	// clusters.
	Wave2 ClusterUpgradeCadence = "Wave2"
)
...
func PossibleClusterUpgradeCadenceValues() []ClusterUpgradeCadence {
	return []ClusterUpgradeCadence{Wave0, Wave1, Wave2}
}
...
const (
	// None Indicates that no identity is associated with the resource.
	None ManagedIdentityType = "None"
	// SystemAssigned Indicates that system assigned identity is associated with the resource.
	SystemAssigned ManagedIdentityType = "SystemAssigned"
	// SystemAssignedUserAssigned Indicates that both system assigned and user assigned identity are associated
	// with the resource.
	SystemAssignedUserAssigned ManagedIdentityType = "SystemAssigned, UserAssigned"
	// UserAssigned Indicates that user assigned identity is associated with the resource.
...
const (
	// EmailSubscription For subscription receivers. In this case, the parameter receivers should be a list of
	// roles of the subscription for the cluster (eg. Owner, AccountAdmin, etc) that will receive the
	// notifications.
	EmailSubscription NotificationChannel = "EmailSubscription"
	// EmailUser For email user receivers. In this case, the parameter receivers should be a list of email
	// addresses that will receive the notifications.
	EmailUser NotificationChannel = "EmailUser"
)
...
func PossibleNotificationChannelValues() []NotificationChannel {
	return []NotificationChannel{EmailSubscription, EmailUser}
}
...
func PossibleNotificationLevelValues() []NotificationLevel {
	return []NotificationLevel{All, Critical}
}
...
const (
	// Hierarchical If this value is omitted or set to Hierarchical, VMs are grouped to reflect the zonal
	// distribution in up to 15 UDs. Each of the three zones has five UDs. This ensures that the zones are
	// updated one at a time, moving to next zone only after completing five UDs within the first zone. This
	// update process is safer for the cluster and the user application.
	Hierarchical SfZonalUpgradeMode = "Hierarchical"
	// Parallel VMs under the node type are grouped into UDs and ignore the zone info in five UDs. This setting
	// causes UDs across all zones to be upgraded at the same time. This deployment mode is faster for
	// upgrades, we don't recommend it because it goes against the SDP guidelines, which state that the updates
	// should be applied to one zone at a time.
	Parallel SfZonalUpgradeMode = "Parallel"
)
...
func PossibleSfZonalUpgradeModeValues() []SfZonalUpgradeMode {
	return []SfZonalUpgradeMode{Hierarchical, Parallel}
}
