...
func (client DatabasesGroupClient) UpgradeDataWarehouse(ctx context.Context, resourceGroupName string, serverName string, databaseName string) (result DatabasesGroupUpgradeDataWarehouseFuture, err error) {
	req, err := client.UpgradeDataWarehousePreparer(ctx, resourceGroupName, serverName, databaseName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "sql.DatabasesGroupClient", "UpgradeDataWarehouse", nil, "Failure preparing request")
		return
	}

	result, err = client.UpgradeDataWarehouseSender(req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "sql.DatabasesGroupClient", "UpgradeDataWarehouse", result.Response(), "Failure sending request")
		return
	}
...
func (client DatabasesGroupClient) UpgradeDataWarehousePreparer(ctx context.Context, resourceGroupName string, serverName string, databaseName string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"databaseName":      autorest.Encode("path", databaseName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"serverName":        autorest.Encode("path", serverName),
...
// http.Response Body if it receives an error.
func (client DatabasesGroupClient) UpgradeDataWarehouseSender(req *http.Request) (future DatabasesGroupUpgradeDataWarehouseFuture, err error) {
	sender := autorest.DecorateSender(client, azure.DoRetryWithRegistration(client.Client))
	future.Future = azure.NewFuture(req)
	future.req = req
	_, err = future.Done(sender)
...
// closes the http.Response Body.
func (client DatabasesGroupClient) UpgradeDataWarehouseResponder(resp *http.Response) (result autorest.Response, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK, http.StatusAccepted),
