...
const fqdn = "github.com/Azure/azure-sdk-for-go/services/storagecache/mgmt/2022-01-01/storagecache"
...
type ResourceUsage struct {
	// Limit - READ-ONLY; The limit (quota) for this resource.
	Limit *int32 `json:"limit,omitempty"`
	// Unit - READ-ONLY; Unit that the limit and usages are expressed in, such as 'Count'.
	Unit *string `json:"unit,omitempty"`
	// CurrentValue - READ-ONLY; The current usage of this resource.
	CurrentValue *int32 `json:"currentValue,omitempty"`
	// Name - READ-ONLY; Naming information for this resource type.
...
type ResourceUsageName struct {
	// Value - Canonical name for this resource type.
	Value *string `json:"value,omitempty"`
	// LocalizedValue - Localized name for this resource type.
	LocalizedValue *string `json:"localizedValue,omitempty"`
}

// ResourceUsagesListResult result of the request to list resource usages. It contains a list of resource
// usages & limits and a URL link to get the next set of results.
type ResourceUsagesListResult struct {
	autorest.Response `json:"-"`
	// NextLink - READ-ONLY; URL to get the next set of resource usage list results if there are any.
	NextLink *string `json:"nextLink,omitempty"`
	// Value - READ-ONLY; List of usages and limits for resources controlled by the Microsoft.StorageCache resource provider.
...
type ResourceUsagesListResultIterator struct {
	i    int
	page ResourceUsagesListResultPage
}
...
func (iter *ResourceUsagesListResultIterator) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ResourceUsagesListResultIterator.NextWithContext")
		defer func() {
			sc := -1
			if iter.Response().Response.Response != nil {
				sc = iter.Response().Response.Response.StatusCode
...
func NewResourceUsagesListResultIterator(page ResourceUsagesListResultPage) ResourceUsagesListResultIterator {
	return ResourceUsagesListResultIterator{page: page}
}
...
// hasNextLink returns true if the NextLink is not empty.
func (rulr ResourceUsagesListResult) hasNextLink() bool {
	return rulr.NextLink != nil && len(*rulr.NextLink) != 0
}

// resourceUsagesListResultPreparer prepares a request to retrieve the next set of results.
// It returns nil if no more results exist.
func (rulr ResourceUsagesListResult) resourceUsagesListResultPreparer(ctx context.Context) (*http.Request, error) {
	if !rulr.hasNextLink() {
		return nil, nil
	}
	return autorest.Prepare((&http.Request{}).WithContext(ctx),
...
type ResourceUsagesListResultPage struct {
	fn   func(context.Context, ResourceUsagesListResult) (ResourceUsagesListResult, error)
	rulr ResourceUsagesListResult
}
...
func (page *ResourceUsagesListResultPage) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ResourceUsagesListResultPage.NextWithContext")
		defer func() {
			sc := -1
			if page.Response().Response.Response != nil {
				sc = page.Response().Response.Response.StatusCode
...
// StorageTargetInvalidateFuture an abstraction for monitoring and retrieving the results of a long-running
// operation.
type StorageTargetInvalidateFuture struct {
	azure.FutureAPI
	// Result returns the result of the asynchronous operation.
	// If the operation has not completed it will return an error.
	Result func(StorageTargetClient) (autorest.Response, error)
...
// result is the default implementation for StorageTargetInvalidateFuture.Result.
func (future *StorageTargetInvalidateFuture) result(client StorageTargetClient) (ar autorest.Response, err error) {
	var done bool
	done, err = future.DoneWithContext(context.Background(), client)
	if err != nil {
		err = autorest.NewErrorWithError(err, "storagecache.StorageTargetInvalidateFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		ar.Response = future.Response()
		err = azure.NewAsyncOpIncompleteError("storagecache.StorageTargetInvalidateFuture")
		return
	}
	ar.Response = future.Response()
	return
