...
func (client WorkflowsGroupClient) Abort(ctx context.Context, resourceGroupName string, storageSyncServiceName string, workflowID string) (result autorest.Response, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("storagesyncmanagement.WorkflowsGroupClient", "Abort", err.Error())
	}

	req, err := client.AbortPreparer(ctx, resourceGroupName, storageSyncServiceName, workflowID)
	if err != nil {
		err = autorest.NewErrorWithError(err, "storagesyncmanagement.WorkflowsGroupClient", "Abort", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "storagesyncmanagement.WorkflowsGroupClient", "Abort", resp, "Failure sending request")
		return
	}

	result, err = client.AbortResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "storagesyncmanagement.WorkflowsGroupClient", "Abort", resp, "Failure responding to request")
	}
...
func (client WorkflowsGroupClient) Get(ctx context.Context, resourceGroupName string, storageSyncServiceName string, workflowID string) (result Workflow, err error) {
	if err := validation.Validate([]validation.Validation{
		...
		{TargetValue: resourceGroupName,
			Constraints: []validation.Constraint{{Target: "resourceGroupName", Name: validation.MaxLength, Rule: 90, Chain: nil},
				{Target: "resourceGroupName", Name: validation.MinLength, Rule: 1, Chain: nil},
				{Target: "resourceGroupName", Name: validation.Pattern, Rule: `^[-\w\._\(\)]+$`, Chain: nil}}}}); err != nil {
		return result, validation.NewError("storagesyncmanagement.WorkflowsGroupClient", "Get", err.Error())
	}

	req, err := client.GetPreparer(ctx, resourceGroupName, storageSyncServiceName, workflowID)
	if err != nil {
		err = autorest.NewErrorWithError(err, "storagesyncmanagement.WorkflowsGroupClient", "Get", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "storagesyncmanagement.WorkflowsGroupClient", "Get", resp, "Failure sending request")
		return
	}

	result, err = client.GetResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "storagesyncmanagement.WorkflowsGroupClient", "Get", resp, "Failure responding to request")
	}
