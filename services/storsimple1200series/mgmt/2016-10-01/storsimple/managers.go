...
func (client ManagersClient) GetActivationKeyPreparer(ctx context.Context, resourceGroupName string, managerName string) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		autorest.AsPost(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.StorSimple/managers/{managerName}/getActivationKey", pathParameters),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}
...
// RegenerateRegistrationCertificate returns the Registration Certificate.
// Parameters:
// resourceGroupName - the resource group name
// managerName - the manager name
func (client ManagersClient) RegenerateRegistrationCertificate(ctx context.Context, resourceGroupName string, managerName string) (result RegistrationCertificate, err error) {
	if err := validation.Validate([]validation.Validation{
		{TargetValue: managerName,
			Constraints: []validation.Constraint{{Target: "managerName", Name: validation.MaxLength, Rule: 50, Chain: nil},
				{Target: "managerName", Name: validation.MinLength, Rule: 2, Chain: nil}}}}); err != nil {
		return result, validation.NewError("storsimple.ManagersClient", "RegenerateRegistrationCertificate", err.Error())
	}

	req, err := client.RegenerateRegistrationCertificatePreparer(ctx, resourceGroupName, managerName)
	if err != nil {
		err = autorest.NewErrorWithError(err, "storsimple.ManagersClient", "RegenerateRegistrationCertificate", nil, "Failure preparing request")
		return
	}

	resp, err := client.RegenerateRegistrationCertificateSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "storsimple.ManagersClient", "RegenerateRegistrationCertificate", resp, "Failure sending request")
		return
	}

	result, err = client.RegenerateRegistrationCertificateResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "storsimple.ManagersClient", "RegenerateRegistrationCertificate", resp, "Failure responding to request")
	}

	return
}

// RegenerateRegistrationCertificatePreparer prepares the RegenerateRegistrationCertificate request.
func (client ManagersClient) RegenerateRegistrationCertificatePreparer(ctx context.Context, resourceGroupName string, managerName string) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"managerName":       autorest.Encode("path", managerName),
		"resourceGroupName": autorest.Encode("path", resourceGroupName),
		"subscriptionId":    autorest.Encode("path", client.SubscriptionID),
	...
	preparer := autorest.CreatePreparer(
		autorest.AsPost(),
		autorest.WithBaseURL(client.BaseURI),
		autorest.WithPathParameters("/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.StorSimple/managers/{managerName}/regenarateRegistationCertificate", pathParameters),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// RegenerateRegistrationCertificateSender sends the RegenerateRegistrationCertificate request. The method will close the
// http.Response Body if it receives an error.
func (client ManagersClient) RegenerateRegistrationCertificateSender(req *http.Request) (*http.Response, error) {
	return autorest.SendWithSender(client, req,
		azure.DoRetryWithRegistration(client.Client))
}

// RegenerateRegistrationCertificateResponder handles the response to the RegenerateRegistrationCertificate request. The method always
// closes the http.Response Body.
func (client ManagersClient) RegenerateRegistrationCertificateResponder(resp *http.Response) (result RegistrationCertificate, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
