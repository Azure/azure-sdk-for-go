...
func main() {
	...
	packages = generateAliasPackages(packages)
	products := packages.ParallelSelect(writeAliasPackages)
...
func generateAliasPackages(packages collection.Enumerator) collection.Enumerator {
	packages = packages.ParallelSelect(generateIntermediaryAliasPackage)
	packages = packages.Select(updateAliasPackageUserAgent)
	return packages
}
//updateAliasPackageUserAgent updates the "UserAgent" function in the generated profile, if it is present.
func updateAliasPackageUserAgent(x interface{}) interface{} {
	cast, ok := x.(*alias)
...
// generateIntermediaryAliasPackage generates the alias package from the originally parsed one.
func generateIntermediaryAliasPackage(x interface{}) interface{} {
	var err error
...
//writeAliasPackages adds the MSFT Copyright Header, then writes the alias package to disk.
func writeAliasPackages(x interface{}) interface{} {
	cast, ok := x.(*alias)
	if !ok {
		return false
	}
